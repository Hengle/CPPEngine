...
if [[ " ${arr[@]} " =~ " ${mainphoneproject} " ]]; then
	cp -fr "$CURDIR/../../../../IndieLib_resources/$mainphoneproject" "$TOPDIR_/."
fi
...
if [[ " ${arr[@]} " =~ " ${mainphoneproject} " ]]; then
	cp -fr "$CURDIR/../../../../IndieLib_resources/skeleton" "$TOPDIR_/."
fi

arr=("a_10_IND_Light" "a_14_3d_Camera")
if [[ " ${arr[@]} " =~ " ${mainphoneproject} " ]]; then
	cp -fr "$CURDIR/../../../../IndieLib_resources/trex dx" "$TOPDIR_/."
fi
arr=("Test" "SimpleTexture" "Lesson01")
if [[ " ${arr[@]} " =~ " ${mainphoneproject} " ]]; then
	cp -f "$CURDIR/../../../../IndieLib_resources/test.bmp" "$TOPDIR_/."
fi

arr=("GLEssentialsiOS" "GLEssentialsOSX")
if [[ " ${arr[@]} " =~ " ${mainphoneproject} " ]]; then
	cp -f "$CURDIR/../../../../IndieLib_resources/GLEssentials" "$1/Documents/Data"
fi

if [ "$mainphoneproject" == "2DAnimation" ]; then
	cp -fr "$CURDIR/../../../../IndieLib_resources/2DAnimation" "$TOPDIR_/."
	cp -fr "$CURDIR/../../../../IndieLib_resources/IndieLib-Animation-Collision" "$TOPDIR_/."
fi
