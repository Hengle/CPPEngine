...
class ProjectLNX():
	...
	def project2gcc(self, a):
	    ...
	    v.append('-I'+ccc.getINDIELIBROOT()+'/Libraries/assimp-3.2/include')
...
ifeq ($(FILEEXT), ".so")
$(TARGET): $(OBJS)
	$(GCC) -shared $(LDFLAGS) '''+ccc.getG()+''' '''+ccc.getO()+''' $(OBJS) '''+alibs+''' $(FRAMEWORKS) $(OPENGL) $(SDL) $(CGLIBS_) -lCg -lCgGL $(PTHREAD) $(X11) '''+LIBCPP+''' -lz -lm -ldl -o ./lib/lib$(NAME).so
else ifeq ($(FILEEXT), ".a")
$(TARGET): $(OBJS)
	$(AR) -cr $@ $^
	$(RANLIB) $(TARGET)
...
$(TARGET): $(OBJS)
	mkdir -p $(TARGETDIR)
	$(GCC) $(LDFLAGS) '''+ccc.getG()+''' '''+ccc.getO()+''' $(OBJS) '''+alibs+''' $(FRAMEWORKS) $(OPENGL) $(SDL) $(CGLIBS_) $(PTHREAD) $(X11) '''+LIBCPP+''' -lz -lm -ldl -o $@
endif
...
class ProjectNMAKE:
	...
	def mkx(self, a,name,type):
		...
		for i in a.getincs():
		    ...
		    r+=' /I\".\\Libraries\\assimp-3.2\\include\" '
...
class ProjectVC:
	...
	def project2pro(self, a):
		...
		r+=';..\\..\\..\\Libraries\\assimp-3.2\\include'
...
class ProjectNDK:
	...
	def project2pro(self, pre,a, prebuiltlib, NDKarch):
		...
		r=r+'$(LOCAL_PATH)/../Libraries/assimp-3.2/include \\\n\t\t\t'
