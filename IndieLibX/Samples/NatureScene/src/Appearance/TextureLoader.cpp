...

{
  ...
  if(!name)
    ...
    {Logger::writeErrorLog(NSString("2D NVTexture Shell must have a valid name"));return false;}
  ...
  //Logger::writeInfoLog(NSString("Loading new 2D Shell: width = ") + int(width7)
 ...
 */
  ...
  depth        = 1;
	...
	Logger::writeInfoLog(NSString("Loading new 2D Shell: width = ")+int(width7)+NSString(", height = ")+int(height7));
...
id=IRenderer::GetRendererInstance()->addImageLibTexture(
	...
	Logger::writeInfoLog(NSString("Loaded Texture2D file at -> /NatureScene/Water/water.png"));
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Water/water.png:"+id);
...
id=IRenderer::GetRendererInstance()->addImageLibTexture(
	...
	Logger::writeInfoLog(NSString("Loaded Texture2D file at -> /NatureScene/Sky/clouds.png"));
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Sky/clouds.png:"+id);
...
id=IRenderer::GetRendererInstance()->addImageLibTexture(
		...
		,false,
		IRenderer::GetRendererInstance()->Getlinear());
	Logger::writeInfoLog(NSString("Loaded Texture2D file at -> ")+name);
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+name+":"+id);
	}
