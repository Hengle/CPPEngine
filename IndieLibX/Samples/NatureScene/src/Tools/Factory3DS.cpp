...

{
  ...
  stx_memset(TriangleRefs, 0, sizeof(long)*32*vertexIndex.count);
...

{
  ...
  if(!filePath)
    ...
    return  NatureScene::Logger::writeErrorLog(NSString("Failed to load the 3DS model -> ") + strFileName);
...
//	  texture.load2D(Model->Material[objectPointer->group[0].Material].TextureID);
	  ...
	  stx_strlcat(buf,str,1024);
		  ...
		  texture = IRenderer::GetRendererInstance()->addImageLibTexture(
			  buf, false, IRenderer::GetRendererInstance()->Getlinear());
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+buf+":"+texture);
...
*/
  ...
  NatureScene::Logger::writeInfoLog(NSString("Loaded 3DS model -> ") + filePath);
