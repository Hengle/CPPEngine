...
int GetFVFOffsetD3D(DWORD FVF, DWORD FVFComponent, int TEXComponent)
{
	int FVFOffset=0;
	// Mask position
...
DWORD	 Geometry::getIndicesCount()	     { return indicesCount;  }
...
DWORD*	 Geometry::getIndices()    { return indices;	       }
...

{
  ...
  {NatureScene::Logger::writeErrorLog(NSString("Couldn't activate the shadow info");return false;}
  ...
  */
  {
    ...
    indices   = new DWORD[indicesCount];
...
#if 0
void Geometry::setIndices(DWORD * ind, bool byRef){
...

{
  ...
  NatureScene::Logger::writeInfoLog(NSString("Cleaning up geometry after compilation -> ") + name);
...

{
  ...
  NatureScene::Logger::writeInfoLog(NSString("Compiling geometry -> ") + name);



//  if(!GLEE_ARB_vertex_buffer_object)

//    {NatureScene::Logger::writeErrorLog(NSString("Failed: VBO extension not supported");return;}
  ...
  if(!getVertices())
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Failed: 0 vertices"));return false;}
...
/*  else
   ...
   {NatureScene::Logger::writeErrorLog(NSString("Not enough memory for the interleaved geometry arrays");return;}
...
/*
...

    {
      ...
      {NatureScene::Logger::writeErrorLog(NSString("Error creating index buffer");return;}
...
/*
...

  {
	...
	indicesVBOID=IRenderer::GetRendererInstance()->addIndexBuffer(getIndicesCount(),sizeof(DWORD), STATIC,i0);
	setIndicesVBOID(indicesVBOID);

	LOG_FNLN;
	LOG_PRINT("getIndicesCount()=%d\n",getIndicesCount());

	//for(DWORD i = 0; i < getIndicesCount(); i++)
	//	LOG_PRINT("sky i0=%d\n",i0[i]);
...

{
...

  {
	...
	{NatureScene::Logger::writeErrorLog(NSString("Unknown Geometry type -> ") + node->getValue());return false;}
	...
	NatureScene::Logger::writeErrorLog(NSString("XML node <TextureID> must have an <id> child"));
	...
	NatureScene::Logger::writeErrorLog(NSString("XML node <TextureID> must have a <size> child"));
	...
	NatureScene::Logger::writeErrorLog(NSString("XML node <TextureID> must have a <RawData> child"));
	...
	NatureScene::Logger::writeErrorLog(NSString("XML node <Vertices> must have a <RawData> child"));
	...
	NatureScene::Logger::writeErrorLog(NSString("XML node <Normals> must have a <RawData> child"));
	...
	NatureScene::Logger::writeErrorLog(NSString("XML node <Indices> must have a <RawData> child"));
...

{
  ...
  if(!xmlFile.is_open())
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Cannot export Geometry to XML: file not ready."));return false;}



  if(bRESIDE_ON_GFX_DEVICE)

    {NatureScene::Logger::writeErrorLog(NSString("Cannot export Geometry to XML: compiled."));return false;}
...

  {
    ...
    default:
     ...
     {NatureScene::Logger::writeErrorLog(NSString("Cannot export Geometry to XML: unknown type."));return false;}
