...

{
  ...
  if(!element || !shapePath)
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Invalid XMLElement or shapePath -> Shape::loadXMLSettings "));return false;}
  ...
  if(!geometryNode || !appearanceNode)
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Can't locate <Appearance> or <Geometry> in <Shape> nodel -> ") + shapePath);return false;}
...

  {
...

    {
      ...
      {NatureScene::Logger::writeErrorLog(NSString("Failed to load XML geometry -> ") + shapePath);return false;}
  ...
  if(!appearance->loadXMLSettings(appearanceNode))
    ...
    NatureScene::Logger::writeErrorLog(NSString("Failed to load XML appearance -> ") + shapePath);
...

{
  ...
  if(!xmlFile.is_open())
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Cannot export Shape to XML: file not ready."));return false;}



  if(!geometry)

    {NatureScene::Logger::writeErrorLog(NSString("Cannot export Shape to XML: 0 geometry."));return false;}



  if(!appearance)

    {NatureScene::Logger::writeErrorLog(NSString("Cannot export Shape to XML: 0 appearance."));return false;}
