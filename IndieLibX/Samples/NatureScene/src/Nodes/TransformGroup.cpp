...

{
  ...
  if(!child)
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Couldn't add shape to TransformGroup: NULL"));return false;}



  if(!child->getGeometry() || !child->getAppearance())

    {NatureScene::Logger::writeErrorLog(NSString("Couldn't add shape to TransformGroup: NULL Geometry or Appearance"));return false;}



  if(child->getParent())

    {NatureScene::Logger::writeErrorLog(NSString("Couldn't add shape to TransformGroup: Already has a parent"));return false;}
  ...
  NatureScene::Logger::writeInfoLog(NSString("Adding shape to TransformGroup"));
...

{
  ...
  if(!child)
    ...
    return NatureScene::Logger::writeErrorLog(NSString("Couldn't add Group to TransformGroup: NULL"));



  if(this == child)

    return NatureScene::Logger::writeErrorLog(NSString("Couldn't add shape to TransformGroup: parent is same as child"));



  if(child->getParent())

    return NatureScene::Logger::writeErrorLog(NSString("Couldn't add shape to TransformGroup: Already has a parent"));



  NatureScene::Logger::writeInfoLog(NSString("Adding Group to TransformGroup"));
...

{
  ...
  if(!xmlFile.is_open())
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Cannot export Group to XML: file not ready."));return false;}
...

{
  ...
  {    NatureScene::Logger::writeErrorLog(NSString("NULL Shape node"));return false;}



  if(element->getName() != "Shape")

    {NatureScene::Logger::writeErrorLog(NSString("Need a <Shape> tag in the XML Node"));return false;}
  ...
  if(!(child = element->getChildByName("path")))
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Need a path child in <Shape>"));return false;}
  ...
  if(!verifiedPath)
    ...
    {NatureScene::Logger::writeErrorLog(NSString("Couldn't locate Shape file at ->") + child->getValuec());return false;}
...

  {
	...
	NatureScene::Logger::writeErrorLog(NSString("Failed to load Shape -> ") + stringBuffer);
