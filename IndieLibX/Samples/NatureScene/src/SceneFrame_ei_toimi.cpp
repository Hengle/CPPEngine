...
#endif
/*
  Logger::writeInfoLog(NSString("Renderer: ") + (char*)glGetString(GL_RENDERER));
  Logger::writeInfoLog(NSString("Version:  ") + (char*)glGetString(GL_VERSION));
  Logger::writeInfoLog(NSString("Vendor:   ") + (char*)glGetString(GL_VENDOR));
*/
...
*/
  ...
  grass = IRenderer::GetRendererInstance()->addImageLibTexture("/NatureScene/Terrain/grasslayer.png", true, IRenderer::GetRendererInstance()->Getlinear());
  grassPack = IRenderer::GetRendererInstance()->addImageLibTexture("/NatureScene/Grass/grassPack.png", true, IRenderer::GetRendererInstance()->GetlinearClamp());


  	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Terrain/grasslayer.png:"+grass);
	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Grass/grassPack.png:"+grassPack);
...
noise=IRenderer::GetRendererInstance()->addImageLibTexture(
  	...
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Sky/clouds.png:"+noise);
...
waterDepth=IRenderer::GetRendererInstance()->addImageLibTexture(
  	...
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Terrain/watermap.jpg:"+waterDepth);
#endif
...
bool SceneFrame::run()
{
  ...
  return false;//???benchmark.running();
}
 ...
 */
  ...
  if(randommap==-1)
  {
  	randommap=IRenderer::GetRendererInstance()->addImageLibTexture("/NatureScene/randommap.tga",true);
	
  	Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/randommap.tga:"+randommap);
  }
...
void SceneFrame::keyReleased()
{
  ...
  //if(STX_Service::GetInputInstance()->IsKeyPressed(KEY_F8))
      //window.scheduleSnapshot();
  if(STX_Service::GetInputInstance()->IsKeyPressed(KEY_PAUSE))
      paused = !paused;
}
...
void SceneFrame::keyPressed ()
{
  ...
  if(STX_Service::GetInputInstance()->IsKeyPressed(KEY_h))
     if(rectangle = guiFrame->getWidgetByCallbackString("User Controls"))
       rectangle->setVisible(!rectangle->isVisible());
  if(STX_Service::GetInputInstance()->IsKeyPressed(KEY_ESCAPE))
	active = false;
}
#endif
bool SceneFrame::loadXMLSettings(XMLElement *element)
{
  ...
  if(guiFrame->GUIPanel::loadXMLSettings("/NatureScene/GUILayout.xml"))
  {
    GUISlider *slider = 0;
