...
{    
	static D3DXFROMWINEMATRIX proj,								// This will hold our projection matrix
	                w,v,modl,								// This will hold our modelview matrix
  ...
  D3DXFROMWINEVECTOR4        *p;
	...
	IRenderer::GetRendererInstance()->GetTransform( TS_PROJECTION, proj );

	// By passing in GL_MODELVIEW_MATRIX, we can abstract our model view matrix.
	// This also stores it in an array of [16].
	//glGetFloatv( GL_MODELVIEW_MATRIX, modl );
	IRenderer::GetRendererInstance()->GetTransform( TS_VIEW, v );
	//IRenderer::GetRendererInstance()->GetTransform( TS_WORLD, w );
	modl=/* w* */v;
