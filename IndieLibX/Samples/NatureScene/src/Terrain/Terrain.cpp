...
#else
  if(!watermap.loadImageLibImage("/NatureScene/Terrain/watermap.png",false))
    return Logger::writeErrorLog("Invalid watermap image");

 if(!coverage.loadImageLibImage("/NatureScene/Grass/coverage.png",false))
    return Logger::writeErrorLog("Invalid coverage image");

  if(!heightmap.loadImageLibImage("/NatureScene/Terrain/heightmap.png",false))
    return Logger::writeErrorLog("Invalid heightmap image");
#endif
  ...
  for(x = (CELL_COLUMN_COUNT - 1); x > -1; x--)
  {
    ...
    if(!portals[index].setup(terrainInfo,  Tuple2i(x*xRatio + xOf, y*yRatio + yOf), randommap, coverage,terrainShader,grassShader))
      return Logger::writeErrorLog(NSString("Error setting up portal ") + index);
  }
  ...
  Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Terrain/dirt.png:"+dirt);
  Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Terrain/fungus.png:"+fungus);
  Logger::writeInfoLog(NSString("addImageLibTexture:")+"/NatureScene/Terrain/watermap.png:"+weight);
...
{
  ubyte *hmBytes  = (ubyte*)dst.getDataBuffer(),
          *wmBytes  = (ubyte*)src.getDataBuffer();
...
void Terrain::createRandomMap(int width, int height, Image3 &image)
{
  ...
  const char* verified = MediaPathManager::lookUpMediaPath("/NatureScene/randommap.tga");

  if(verified)
  {
	//LOG_PRINT("verified=%s\n", verified);
    //image.load(verified);
	image.loadImageLibImage(verified,false);
    return;
  }
  ...
  randommap.open("/NatureScene/randommap.tga", ios::binary);
...
ofstream randommap;
  ...
  ubyte *fDest = image.create(FORMAT_RGBA8, width7, height7, 1, 1);
	...
	memcpy(fDest,data,index*4*sizeof(unsigned char));
