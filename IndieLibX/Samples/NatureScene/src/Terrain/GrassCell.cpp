			...
			const Image3     &coveragemap,ShaderID sha)
{
  ...
  const  ubyte *coverageContent = coveragemap.getDataBuffer(),
...
/*
	LOG_PRINT("vertex.x=%f\n", grass[12*nextGObject+i0].vertex.x);
	LOG_PRINT("vertex.y=%f\n", grass[12*nextGObject+i0].vertex.y);
	LOG_PRINT("vertex.z=%f\n", grass[12*nextGObject+i0].vertex.z);
*/
	  //grass[12*nextGObject+i0].vertex  *= yRandRotation;
...
*/
  if(indexBufferID==-1) indexBufferID = IRenderer::GetRendererInstance()->addIndexBuffer(
	  indexCount,sizeof(DWORD), STATIC, indices);
  ...
  if(!status)
    NatureScene::Logger::writeErrorLog(NSString("Could not generate a geometry buffer object, GRASS"));
...
const DWORD *GrassCell::getIndexStream()     { return indices;          }
