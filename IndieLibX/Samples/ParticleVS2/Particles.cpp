...
{LOG_FNLN;LOG_PRINT("m_pDecl=%x\n", m_pDecl);}
  ...
  for(__WORD__ i=0;i<m_NumParticles;i++) {
	...
	LOG_PRINT("m_VB.size()=%d\n", m_VB.size());
	LOG_PRINT("m_IB.size()=%d\n", m_IB.size());
...
void cParticleEmitter::Render(D3DXFROMWINEMATRIX &matView, D3DXFROMWINEMATRIX &matProj)
{
	Begin(matView, matProj);
  ...
  for(__DWORD__ i=0;i<NUM_PARTICLE_TYPES;i++) {
	...
	*/
	...
	//LOG_PRINT("m_VB.size()=%d, Num=%d\n", m_VB.size(), Num);

        // Copy over particle data
        float HalfSize = Particle->m_Size * 0.5f;
        m_VB[4*Num+0].vecPos    = Particle->m_vecPos;
        m_VB[4*Num+0].vecOffset = D3DXFROMWINEVECTOR2(-HalfSize, HalfSize);
        //m_VB[4*Num+0].Diffuse   = Particle->m_Color;
        m_VB[4*Num+0].uv         = D3DXFROMWINEVECTOR2(0.0f, 0.0f);
        m_VB[4*Num+1].vecPos    = Particle->m_vecPos;
        m_VB[4*Num+1].vecOffset = D3DXFROMWINEVECTOR2(HalfSize, HalfSize);
        //m_VB[4*Num+1].Diffuse   = Particle->m_Color;
        m_VB[4*Num+1].uv         = D3DXFROMWINEVECTOR2(1.0f, 0.0f);
        m_VB[4*Num+2].vecPos    = Particle->m_vecPos;
        m_VB[4*Num+2].vecOffset = D3DXFROMWINEVECTOR2(-HalfSize, -HalfSize);
        //m_VB[4*Num+2].Diffuse   = Particle->m_Color;
        m_VB[4*Num+2].uv         = D3DXFROMWINEVECTOR2(0.0f, 1.0f);
        m_VB[4*Num+3].vecPos    = Particle->m_vecPos;
        m_VB[4*Num+3].vecOffset = D3DXFROMWINEVECTOR2(HalfSize, -HalfSize);
        //m_VB[4*Num+3].Diffuse   = Particle->m_Color;
        m_VB[4*Num+3].uv         = D3DXFROMWINEVECTOR2(1.0f, 1.0f);
...
void cParticleEmitter::HandleSmoke(D3DXFROMWINEVECTOR3 *vecPos, __DWORD__ Elapsed)
{
  ...
  if(Timer > 66) {
    ...
    Add(PARTICLE_SMOKE, &v1,
        10.0f, // 0xFF222222,
        D3DXFROMWINEVECTOR4(1.0f, 1.0f, 1.0f, 1.0f),
        1000, 
        &v2);
  }
}
