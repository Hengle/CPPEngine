...
bool MyModel::Load( const char *name, const char *texturePath)
{
	...
	*/
	...
	if ((m_vertexBuffer = context->CreateVertexBuffer(m_numVertices * sizeof(Vertex_), D3D10_USAGE_IMMUTABLE, vertices)) == 0) return false;
	if ((m_indexBuffer = context->CreateIndexBuffer(m_numTriangles * 3 * sizeof(uint32), D3D10_USAGE_IMMUTABLE, indices)) == 0) return false;
	...
	if ((m_materials[0] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m0)) == 0) return false;
	if ((m_materials[1] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m1)) == 0) return false;
	if ((m_materials[2] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m2)) == 0) return false;
	if ((m_materials[3] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m3)) == 0) return false;
	if ((m_materials[4] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m4)) == 0) return false;
	if ((m_materials[5] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m5)) == 0) return false;
	if ((m_materials[6] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m6)) == 0) return false;
	if ((m_materials[7] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m7)) == 0) return false;
	if ((m_materials[8] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m8)) == 0) return false;
	if ((m_materials[9] = context->CreateConstantBuffer(sizeof(PerObject), D3D10_USAGE_IMMUTABLE, &m9)) == 0) return false;
...
void MyModel::Setup()
{
	...
	IRenderer::GetRendererInstance()->setIndexBuffer(m_indexBuffer, DXGI_FORMAT_R32_UINT, 0);

	IRenderer::GetRendererInstance()->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}
...
bool Create()
{
	...
	if (!m_context->Create(("Global Illumination"), DXGI_FORMAT_R10G10B10A2_UNORM, DXGI_FORMAT_D24_UNORM_S8_UINT, 800, 600, 4, false)) return false;
...
bool CreateSHTextures()
{
	...
	for (int i = 0; i < SH_COEFF_VECTORS; i++)
	{
		...
		if ((m_shCube[i] = m_context->CreateTextureCube(tex0, DXGI_FORMAT_R16G16B16A16_FLOAT, SH_TF_CUBE_SIZE, &m_shCubeSRV[i])) == 0) return false;
		if ((m_shTable[i] = m_context->CreateTexture3D(tex1, DXGI_FORMAT_R16G16B16A16_FLOAT, PROBE_SIZE, PROBE_SIZE, 6, &m_shTableSRV[i])) == 0) return false;
	}
...
bool Load()
{
	...
	if ((m_lighting = m_context->LoadEffect("Lighting.fx", effectDefines)) == 0) return false;

	// Use our own constant buffer management for best performance
	if ((m_perFrameCB = m_context->CreateEffectConstantBuffer(m_lighting, "PerFrame")) == 0) return false;
	if ((m_gsMvpCB    = m_context->CreateEffectConstantBuffer(m_lighting, "GSMvp"   )) == 0) return false;
	if ((m_gsMvpInvCB = m_context->CreateEffectConstantBuffer(m_lighting, "GSMvpInv")) == 0) return false;
	...
	*/
	...
	fclose(file);
	if ((m_texLightGrad = (TextureID ) m_context->CreateTexture1D(grad, DXGI_FORMAT_R8G8B8A8_UNORM, 256, 1, &m_texLightGradSRV)) == 0) return false;


	// Load the model
	m_model = new MyModel();
	if (!m_model->Load(m_context, MODEL_PATH ("GIroom.am"), TEX_PATH)) return false;


	// Define the input layout
	D3D10_INPUT_ELEMENT_DESC layout[] =
	{
		{ "Vertex_", 0, DXGI_FORMAT_R32G32B32_FLOAT,    0, offsetOf(Vertex_, pos),    D3D10_INPUT_PER_VERTEX_DATA, 0 },
		{ "Normal", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetOf(Vertex_, normal), D3D10_INPUT_PER_VERTEX_DATA, 0 },
	};
	if ((m_lightingIL = m_context->CreateInputLayout(m_lighting->GetTechniqueByIndex(0)->GetPassByIndex(0), layout, elementsOf(layout))) == 0) return false;

	// Create two rasterizer states so we can switch between back/none face culling.
	if ((m_cullBack = m_context->CreateRasterizerState(D3D10_CULL_BACK)) == 0) return false;
	if ((m_cullNone = m_context->CreateRasterizerState(D3D10_CULL_NONE)) == 0) return false;
	...
	if ((m_shEffect = m_context->LoadEffect(("SH.fx"), shDefines)) == 0) return false;
	...
	if ((m_shCoeffIL = m_context->CreateInputLayout(m_shEffect->GetTechniqueByName("SH")->GetPassByName("Main"), shLayout, elementsOf(shLayout))) == 0) return false;
	...
	if ((m_shadowMap = m_context->CreateRenderTargetCube(DXGI_FORMAT_R16_UNORM, SHADOWMAP_SIZE, 1, 1, &m_shadowMapRTV, 0, &m_shadowMapSRV)) == 0) return false;
	if ((m_shadowMapDepth = m_context->CreateDepthTargetCube(DXGI_FORMAT_D16_UNORM, SHADOWMAP_SIZE, 1, 1, &m_shadowMapDepthDSV, 0)) == 0) return false;

	// Comparison filters are broken in current Nvidia drivers
	if (useNvidiaWorkaround)
	{
		if ((m_shadowMapSS = m_context->CreateSamplerState(D3D10_FILTER_MIN_MAG_LINEAR_MIP_POINT, D3D10_TEXTURE_ADDRESS_CLAMP, 0.0f)) == 0) return false;
	}
	else
	{
		if ((m_shadowMapSS = m_context->CreateSamplerState(D3D10_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT, D3D10_TEXTURE_ADDRESS_CLAMP, 0.0f)) == 0) return false;
	}
	...
	for (int i = 0; i < SH_COEFF_VECTORS; i++)
	{
		for (int c = 0; c < 3; c++)
		{
			if ((m_shCoeffs[c][i] = m_context->CreateRenderTarget3D(DXGI_FORMAT_R16G16B16A16_FLOAT, SIZE_X, SIZE_Y, SIZE_Z, 1, &m_shCoeffsRTV[c][i], &m_shCoeffsSRV[c][i])) == 0) return false;
		}
	}
	if ((m_shCoeffsSS = m_context->CreateSamplerState(D3D10_FILTER_MIN_MAG_LINEAR_MIP_POINT, D3D10_TEXTURE_ADDRESS_CLAMP, 0.0f)) == 0) return false;
	...
	if ((m_shCoeffVB = m_context->CreateVertexBuffer((m_probeCount + PROBE_CUBES_PER_FRAME - 1) * sizeof(ShVertex), D3D10_USAGE_IMMUTABLE, coords)) == 0) return false;

	// Create the sampler states for base textures and lightmaps
	if ((m_baseSS = m_context->CreateSamplerState(D3D10_FILTER_ANISOTROPIC, D3D10_TEXTURE_ADDRESS_WRAP)) == 0) return false;
	if ((m_lightMapSS = m_context->CreateSamplerState(D3D10_FILTER_MIN_MAG_LINEAR_MIP_POINT, D3D10_TEXTURE_ADDRESS_CLAMP, 0.0f)) == 0) return false;
	...
	*/
	if ((m_probes = m_context->CreateRenderTarget3D(DXGI_FORMAT_R11G11B10_FLOAT, PROBE_SIZE, PROBE_SIZE, PROBE_PASSES_PER_FRAME * PROBE_SLICES_PER_PASS, 1, &m_probesRTV, &m_probesSRV)) == 0) return false;
	// Make an RTV for each chunk of probes that we'll render per pass
	for (int i = 0; i < PROBE_PASSES_PER_FRAME; i++)
	{
		if ((m_probesChunkRTVs[i] = m_context->CreateRenderTargetView3D(m_probes, DXGI_FORMAT_R11G11B10_FLOAT, i * PROBE_SLICES_PER_PASS, PROBE_SLICES_PER_PASS)) == 0) return false;
	}

	// Make a depth buffer for the light probes
	if ((m_probesDepth = m_context->CreateDepthTarget2D(DXGI_FORMAT_D16_UNORM, PROBE_SIZE, PROBE_SIZE, PROBE_SLICES_PER_PASS * PROBE_PASSES_PER_FRAME, 1, 1, &m_probesDepthDSV)) == 0) return false;
	for (int i = 0; i < PROBE_PASSES_PER_FRAME; i++)
	{
		if ((m_probesDepthChunkDSVs[i] = m_context->CreateDepthStencilView2D(m_probesDepth, DXGI_FORMAT_D16_UNORM, i * PROBE_SLICES_PER_PASS, PROBE_SLICES_PER_PASS)) == 0) return false;
	}
	...
	*/
	ID3D10Device *IRenderer::GetRendererInstance() = m_context->GetDevice();
	...
	m_context->Clear(D3DXFROMWINEVECTOR4(0, 0, 0, 0), D3D10_CLEAR_DEPTH, 1.0f, 0);
...
void SetupScene(const int passIndex)
{
	...
	m_model->Setup(IRenderer::GetRendererInstance());
...
void RenderScene(const int passIndex, const D3DXFROMWINEMATRIX *mvp, const D3DXFROMWINEMATRIX *mvpInv, const int matrixCount)
{
	...
	if (mvp)
	{
		m_gsMvpCB->Map(D3D10_MAP_WRITE_DISCARD, 0, (void **) &mvpArray);
			memcpy(mvpArray, mvp, matrixCount * sizeof(D3DXFROMWINEMATRIX));
		m_gsMvpCB->Unmap();
	}
	if (mvpInv)
	{
		m_gsMvpInvCB->Map(D3D10_MAP_WRITE_DISCARD, 0, (void **) &mvpArray);
			memcpy(mvpArray, mvpInv, matrixCount * sizeof(D3DXFROMWINEMATRIX));
		m_gsMvpInvCB->Unmap();
	}
	...
	if (passIndex >= PROBE_PASS)
	{
		...
		*/
		...
		// Hand
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 9);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 0);

		// Ball + Horse + Sci-Fi weirdo + Bench + Frame
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 4);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 1);

		// Horse stand + Sci-Fi weirdo stand + Globe stand
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 7);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 2);

		// Ceiling + Pillars + Stands + Wall lights
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 5);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 3);

		// Walls
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), m_currWallMaterial);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 4);

		IRenderer::GetRendererInstance()->RSSetState(m_cullNone);

		// Teapot
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 8);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 5);
		...
		*/
		...
		// Globe
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 6);
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 4);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 6);
		...
		*/
		...
		// Ball-horse
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 0);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 7);

		// Head
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 1);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 8);

		// Picture
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 2);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 9);

		// Floor
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 3);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 10);
		...
		*/
		...
		// Ceiling
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), 5);
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 6);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 11);

		IRenderer::GetRendererInstance()->PSSetShader(m_lightingPS[passIndex + 4]);

		// Wall light quads
		m_model->SetupMaterial(IRenderer::GetRendererInstance(), m_currWallMaterial);
		m_model->SetupTexture(IRenderer::GetRendererInstance(), 5);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 12);
	}
	else
	{
		/*
		...
		*/
		...
		// Teapot + Globe
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 5, 2);
		...
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 1, 4);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 7, 6);
		m_model->RenderMaterial(IRenderer::GetRendererInstance(), 0);
	}
}
...
void OnRender()
{
	...
	*/
	if (m_lightIntensity > 0.0f)
	{
		...
		IRenderer::GetRendererInstance()->OMSetRenderTargets(1, &m_shadowMapRTV, m_shadowMapDepthDSV);
	...
	*/
	...
	for (int n = 0; n < PROBE_PASSES_PER_FRAME; n++)
	{
		...
		IRenderer::GetRendererInstance()->OMSetRenderTargets(1, &m_probesChunkRTVs[n], m_probesDepthChunkDSVs[n]);
	...
	*/
	...
	IRenderer::GetRendererInstance()->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_POINTLIST);
