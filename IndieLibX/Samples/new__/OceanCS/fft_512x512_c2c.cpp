					 ...
					 ShaderResourceViewID pSRV_Src)
{
	...
	ID3D11DeviceContext* pd3dContext = fft_plan->IRenderer::GetRendererInstance();
...
void fft512x512_create_plan(CSFFT512x512_Plan* plan, unsigned int slices)
{
	...
	IRenderer::GetRendererInstance()->CreateUnorderedAccessView(plan->pBuffer_Tmp, &uav_desc, &plan->pUAV_Tmp);
	...
	IRenderer::GetRendererInstance()->CreateShaderResourceView(plan->pBuffer_Tmp, &srv_desc, &plan->pSRV_Tmp);
