						...
						VertexBufferID* ppBuffer, ID3D11UnorderedAccessView** ppUAV, ShaderResourceViewID* ppSRV)
{
	...
	->CreateUnorderedAccessView(*ppBuffer, &uav_desc, ppUAV);
	assert(*ppUAV);
	...
	->CreateShaderResourceView(*ppBuffer, &srv_desc, ppSRV);
	assert(*ppSRV);
}

void createTextureAndViews(unsigned int width, unsigned int height, DXGI_FORMAT format,
						   TextureID* ppTex, ShaderResourceViewID* ppSRV, RenderTargetViewID * ppRTV)
{
	...
	->CreateTexture2D(&tex_desc, 0, ppTex);
	assert(*ppTex);
	...
	if (ppSRV)
	{
		...
		->CreateShaderResourceView(*ppTex, &srv_desc, ppSRV);
		assert(*ppSRV);
	}
	...
	if (ppRTV)
	{
		...
		->CreateRenderTargetView(*ppTex, &rtv_desc, ppRTV);
		assert(*ppRTV);
	}
}
...
OceanSimulator::OceanSimulator(OceanParameter& params, )
{
    ...
    m_pd3dDevice->CreateComputeShader(pBlobUpdateSpectrumCS->GetBufferPointer(), pBlobUpdateSpectrumCS->GetBufferSize(), 0, &m_pUpdateSpectrumCS);
    ...
    m_pd3dDevice->CreateVertexShader(pBlobQuadVS->GetBufferPointer(), pBlobQuadVS->GetBufferSize(), 0, &m_pQuadVS);
    m_pd3dDevice->CreatePixelShader(pBlobUpdateDisplacementPS->GetBufferPointer(), pBlobUpdateDisplacementPS->GetBufferSize(), 0, &m_pUpdateDisplacementPS);
    m_pd3dDevice->CreatePixelShader(pBlobGenGradientFoldingPS->GetBufferPointer(), pBlobGenGradientFoldingPS->GetBufferSize(), 0, &m_pGenGradientFoldingPS);
	assert(m_pQuadVS);
	...
	m_pd3dDevice->CreateBuffer(&cb_desc, 0, &m_pPerFrameCB);
	...
	m_pd3dDevice->CreateBuffer(&buf_desc, 0, &m_pDebugBuffer);
...
void OceanSimulator::updateDisplacementMap(float time)
{
	// ---------------------------- H(0) -> H(t), D(x, t), D(y, t) --------------------------------
	// Compute shader
	m_pd3dImmediateContext->CSSetShader(m_pUpdateSpectrumCS, 0, 0);
	...
	m_pd3dImmediateContext->CSSetUnorderedAccessViews(0, 1, cs0_uavs, (unsigned int*)(&cs0_uavs[0]));
	...
	m_pd3dImmediateContext->OMSetRenderTargets(1, rt_views, 0);

	// VS & PS
	m_pd3dImmediateContext->VSSetShader(m_pQuadVS, 0, 0);
	m_pd3dImmediateContext->PSSetShader(m_pUpdateDisplacementPS, 0, 0);
	...
	m_pd3dImmediateContext->setVertexBuffer(0, 1, &vbs[0], &strides[0], &offsets[0]);

	m_pd3dImmediateContext->setVertexFormat(m_pQuadLayout);
	m_pd3dImmediateContext->DrawPrimitive(PRIM_TRIANGLE_STRIP);
	...
	// VS & PS
	m_pd3dImmediateContext->VSSetShader(m_pQuadVS, 0, 0);
	m_pd3dImmediateContext->PSSetShader(m_pGenGradientFoldingPS, 0, 0);
...
int CompileShaderFromFile( char* szFileName, std::string szEntryPoint, std::string szShaderModel, ID3DBlob** ppBlobOut )
{
    ...
    // open the file
    void* hFile = CreateFile( str, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
    ...
    // read the data in
    __DWORD__ BytesRead;
    if( !ReadFile( hFile, pFileData, FileSize.LowPart, &BytesRead, 0 ) )
        return 1; 
    ...
    WideCharToMultiByte(CP_ACP, 0, str, -1, pFilePathName, MAX_PATH, 0, 0);
    ID3DBlob* pErrorBlob;
    hr = D3DCompile( pFileData, FileSize.LowPart, pFilePathName, 0, 0, szEntryPoint, szShaderModel, D3D10_SHADER_ENABLE_STRICTNESS, 0, ppBlobOut, &pErrorBlob );
