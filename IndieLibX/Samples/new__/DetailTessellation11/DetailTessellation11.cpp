...
int  OnD3D11CreateDevice()
{
    ...
    for ( int i=0; i<(int)g_dwNumTextures; ++i )
    {
        ...
        g_pDetailTessellationHeightTextureRV[i] = IRenderer::GetRendererInstance()->addImageLibTexture(DDetailTessellationTextures[i].NormalHeightMap, false);
        ...
        
        {
            ...
            hr = IRenderer::GetRendererInstance()->CreateShaderResourceView( pDensityMap, &dsrvd, &g_pDetailTessellationDensityTextureRV[i] );
        }
        ...
        hr = IRenderer::GetRendererInstance()->CreateShaderResourceView(g_pGridDensityVB[i], &SRVBufferDesc, &g_pGridDensityVBSRV[i]);
    }

    // Load light texture
    char path[256];

    DXUTFindDXSDKMediaFileCch(path, 256, "media/Particle.dds" );
    hr = D3DXFROMWINE11CreateShaderResourceViewFromFile( pd3dDevice, path, 
                                                 0, 0, &g_pLightTextureRV, 0 );
...
//--------------------------------------------------------------------------------------
void  OnD3D11FrameRender(double fTime, float fElapsedTime)
{
    int                     hr;
    ...
    if ( g_bDetailTessellationShadersNeedReloading )
    {
        ...
        D3D10_SHADER_MACRO    pDetailTessellationDefines[] = 
            { "DENSITY_BASED_TESSELLATION", g_bDensityBasedDetailTessellation ? "1" : "0", 
              ...
              0, 0 };
        char wcPath[256];
        DXUTFindDXSDKMediaFileCch( wcPath, 256, "DetailTessellation11.hlsl" );
        CreateVertexShaderFromFile(pd3dDevice, wcPath, pDetailTessellationDefines, 0, 
                "VS_NoTessellation", "vs_5_0", 0, 0, 0, &g_pNoTessellationVS);
        CreateVertexShaderFromFile(pd3dDevice, wcPath, pDetailTessellationDefines, 0, 
                "VS","vs_5_0", 0, 0, 0, &g_pDetailTessellationVS);
        CreateHullShaderFromFile(pd3dDevice,   wcPath, pDetailTessellationDefines, 0, 
                "HS", "hs_5_0", 0, 0, 0, &g_pDetailTessellationHS);
        CreateDomainShaderFromFile(pd3dDevice, wcPath, pDetailTessellationDefines, 0, 
                "DS", "ds_5_0", 0, 0, 0, &g_pDetailTessellationDS);
        CreatePixelShaderFromFile(pd3dDevice,  wcPath, pDetailTessellationDefines, 0, 
    ...
    DepthStateID  pDSV = DXUTGetD3D11DepthStencilView();
    IRenderer::GetRendererInstance()->ClearDepthStencilView( pDSV, D3D11_CLEAR_DEPTH, 1.0f, 0 );
    ...
    // Set index buffer
    IRenderer::GetRendererInstance()->setIndexBuffer( g_pGridTangentSpaceIB, DXGI_FORMAT_R16_UINT, 0 );
    ...
    switch (g_nTessellationMethod)
    {
        case TESSELLATIONMETHOD_DISABLED:
        {
            ...
            IRenderer::GetRendererInstance()->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
        ...
        case TESSELLATIONMETHOD_DETAIL_TESSELLATION:
        {
            ...
            // Set primitive topology
            IRenderer::GetRendererInstance()->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST );
    ...
    if ( g_bDrawLightSource )
    {
        ...
        // Set primitive topology
        IRenderer::GetRendererInstance()->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_POINTLIST );
    ...
    if ( s_dwFrameNumber == g_dwFrameNumberToDump )
    {
        ...
        if ( TexDesc.SampleDesc.Count > 1 )
        {
            ...
            TextureID pSingleSampleTexture;
            hr = IRenderer::GetRendererInstance()->CreateTexture2D( &TexDesc, 0, &pSingleSampleTexture );
            ...
            hr = D3DXFROMWINE11SaveTextureToFile( IRenderer::GetRendererInstance(), pSingleSampleTexture, D3DXFROMWINE11_IFF_BMP, "RTOutput.bmp" );
        ...
        {
            // Single sample case
            hr = D3DXFROMWINE11SaveTextureToFile( IRenderer::GetRendererInstance(), RTTexture, D3DXFROMWINE11_IFF_BMP, "RTOutput.bmp" );
        }
                                    ...
                                    float fBaseTextureRepeat )
{
    ...
    if ( !bCanBeRead )
    {
        ...
        IRenderer::GetRendererInstance()->CreateTexture2D( &Tex2DDesc, 0, &pDensityMapReadFrom );
    ...
    bd.ByteWidth =        dwNumTriangles * sizeof( D3DXFROMWINEVECTOR4 );
    bd.BindFlags =        D3D11_BIND_VERTEX_BUFFER | D3D11_BIND_SHADER_RESOURCE;
    bd.CPUAccessFlags =   0;
    bd.MiscFlags =        0;
    hr = IRenderer::GetRendererInstance()->CreateBuffer( &bd, &InitData, ppEdgeDensityVertexStream );
    if( FAILED( hr ) )
    {
