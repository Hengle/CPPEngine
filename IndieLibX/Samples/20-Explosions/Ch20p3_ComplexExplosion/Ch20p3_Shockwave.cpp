...
int CShockwave::RestoreDeviceObjects(LPDIRECT3DDEVICE8 pDev, 
																			...
																			float fLifetime)
{
	...
	// create shockwave
	{
	  ...
	  VERTEX_XYZ_DIFFUSE_TEX1* pVertices;
		if(FAILED(m_pVB->Lock(0, 0, (BYTE**)&pVertices, NULL))) return hr;
...
{
	VERTEX_XYZ_DIFFUSE_TEX1* pVertices;
	if(FAILED(m_pVB->Lock(0, 0, (BYTE**)&pVertices, NULL))) return;
...
{
	D3DXMATRIX matWorld, matScale, mat;
  D3DXMatrixIdentity( &matWorld );
	D3DXMatrixIdentity( &matScale );
	D3DXMatrixTranslation(&matWorld, m_vPos.x, m_vPos.y, m_vPos.z);
	D3DXMatrixScaling(&matScale, m_fScale, 1.0f, m_fScale);
