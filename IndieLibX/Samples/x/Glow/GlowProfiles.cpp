...
void GlowProfiles::GetProfile( int num_profile, vector< D3DXVECTOR4 > * pV4Coefs, 
								vector< D3DXVECTOR4 > * pV4Coords, KernelType type )
{
	RET_IF( pV4Coefs == NULL );
	RET_IF( pV4Coords == NULL );
	RET_IF( num_profile >= GetNumProfiles() );
	vector< float > vf;
	...
	case KT_HORIZONTAL:
		...
		for( i=0; i < vf.size(); i++ )
		{
			...
			{
				pV4Coefs->push_back( D3DXVECTOR4( v, v, v, v ));
				pV4Coords->push_back( D3DXVECTOR4( x, y, 0.0f, 0.0f ));
			}
		}
		break;
	case KT_VERTICAL:
