...
#define TEXN_GLOW_01	TEXT("textures\\2D\\d3d_set1\\Glow_01.dds")
	...
	for( i=0; i < nNumClear; i++ )
	{
		...
		if( pv != NULL )
		{
			...
			for( n=0; n < pv->size(); n++ )
			{
				if( pv->at(n) != NULL )
				{
					// delete the mesh, set ptr to NULL
					SAFE_DELETE( *(pv->at(n)));
					// delete the ptr
					SAFE_DELETE( pv->at(n) );
				}
...
HRESULT GlowScene::Initialize( IDirect3DDevice9 * pDev )
{
	HRESULT hr = S_OK;
	Free();
	FAIL_IF_NULL( pDev );
	...
	float diag = 0.3f;
	gc.InitCylinder( &msh, D3DXVECTOR3( -diag, -diag, -fHalfSize ), D3DXVECTOR3( diag, diag, fHalfSize ),
						0.20f, 18, 0, 2, 0 );
	...
	*ppMeshVB = new MeshVB;
	...
	int num_lat = 12;
	D3DXMATRIX matRotY;
	D3DXMatrixRotationY( &matRotY, 3.14159f * 2.0f / ((float)num_long) );
	Mesh mesh_array;
	int i;
	for( i=0; i < num_lat; i++ )
	{
		...
		gc.InitCylinder( &msh, D3DXVECTOR3( sx, sy, 0.0f ), D3DXVECTOR3( ex, ey, 0.0f ),
						radius, 5, 0, 0, 0 );
...
HRESULT GlowScene::RenderObjects()
{
	HRESULT hr = S_OK;
	const int nNum = 5;
...
HRESULT GlowScene::RenderObjectsWithGlowStrengthInAlpha()
{
	HRESULT hr = S_OK;
