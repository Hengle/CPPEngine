...
//ID3D10ShaderResourceView * g_pDepthBufferSRV = 0;
//ID3D10DepthStencilView * g_pDepthBufferDSV = 0;
...
//ID3D10DepthStencilView * g_pCubeMapDepthViewArray[6] = { 0, 0, 0, 0, 0, 0 };
//ID3D10ShaderResourceView * g_pCubeMapTextureRV = 0;
...
int PrepareCubeMap()
{
    ...
    if( FAILED( pd3dDevice->CreateTexture2D( &dstex, 0, &g_pCubeMapDepth ) ) )
    {
...
void InitCubeMatrices( D3DXFROMWINEVECTOR3 cubeCenter )
{
	...
	#if 1
	D3DXFROMWINEMatrixIdentity( &cubeViewMatrices[0]);
	D3DXFROMWINEMatrixIdentity( &cubeViewMatrices[1]);
	D3DXFROMWINEMatrixIdentity( &cubeViewMatrices[2]);
	D3DXFROMWINEMatrixIdentity( &cubeViewMatrices[3]);
	D3DXFROMWINEMatrixIdentity( &cubeViewMatrices[4]);
	D3DXFROMWINEMatrixIdentity( &cubeViewMatrices[5]);
	return;
	#endif
...
void InitApp()
{
    ...
    //g_Camera.SetModelCenter( D3DXVECTOR3(0, 7.0f, 0 ) );
...
/*
    ...
    pComboBox->AddItem( L"4D simplex noise", g_pPerlinFire4D );
    pComboBox->AddItem( L"3D flow noise", g_pPerlinFire3DFlow );
    pComboBox->AddItem( L"3D simplex noise", g_pPerlinFire3D );
*/
    g_pCurrentTechnique = g_pPerlinFire3D;
	g_pCurrentLayout = g_pPerlinFire3DLayout;
/*
...
*/
    ...
    g_pMesh.CreateSphere(1.0f, eShaderNone);
...
virtual void render()
{
    ...
    D3DXFROMWINEMatrixInverse( &mWorldViewInv, NULL, &mWorldView );
...
{
#if 0//def LINUX
	IRenderer* r=IRenderer::GetRendererInstance("PerlinFire2", eGLGLSL_1_1, eDebugInit);
#else
	IRenderer* r=IRenderer::GetRendererInstance("PerlinFire2");
#endif	
