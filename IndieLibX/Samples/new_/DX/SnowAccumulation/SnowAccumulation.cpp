...
stx_SkyBox				g_skybox;
stx_Effect*            g_pEffect = NULL;       // D3DXFROMWINE effect interface
#if 0
...
SurfaceID			g_pBBSurface = NULL;	// original swap chain surface
SurfaceID			g_pBBDepthSurface = NULL;	// original swap chain depth surface
TextureID			g_pExposureTexture = NULL;
TextureID			g_pExposureTextureRGBA = NULL;
TextureID			g_pExposureTextureR32 = NULL;
SurfaceID			g_pExposureSurface = NULL;
TextureID			g_pExposureDepthTexture = NULL;
SurfaceID			g_pExposureDepthSurface = NULL;	// original swap chain depth surface
TextureID	g_pNoiseTexture = NULL;

stx_Effect*				g_pExposureCalcEffect = NULL;
...
int  OnCreateDevice()
{
    ...
    g_pEffect=IRenderer::GetRendererInstance()->addEffectFromFile("/new/x/x/SnowAccumulation/Snow.fx");

	// Seperate fx file for the exposure map calculation, and set its projection matrix
	g_pExposureCalcEffect=IRenderer::GetRendererInstance()->addEffectFromFile("/new/x/x/SnowAccumulation/OrthoDepth.fx");
	...
	{PlacedMesh Mesh("/new/x/x/SnowAccumulation/aster1.x",mWorld,true,true,15.f, "/new/x/x/SnowAccumulation/terrainRock.bmp");
	...
	{PlacedMesh Mesh("/new/x/x/SnowAccumulation/snowAccumulation_tree.x",mWorld,true,true,8.f,"/new/x/x/SnowAccumulation/bark.tga");
	 g_SceneMeshList.push_back(Mesh);}
#endif
	...
	mWorld = mScale*mTrans;
	{PlacedMesh Mesh("/new/x/x/SnowAccumulation/tree_0.x",mWorld,true,true,4.f);
	g_SceneMeshList.push_back(Mesh);}
	...
	{PlacedMesh Mesh("/new/x/x/SnowAccumulation/snowman.x",mWorld,false,true,0.f);
	g_SceneMeshList.push_back(Mesh);}
#endif
...
#if 1
    {PlacedMesh Mesh("/new/x/x/SnowAccumulation/snowAccumulation_terrain.x",mWorld,false,true,0.f);
	g_SceneMeshList.push_back(Mesh);}
#else
	{PlacedMesh Mesh("/new/x/x/SnowAccumulation/snowAccumulation_terrain.x",mWorld,true,true,25.f);
...
int  OnResetDevice()
{
	...
	TextureID cubeTex;
	g_skybox.init("/new/x/x/SnowAccumulation/CloudyHillsCubemap.dds");
    ...
    g_pNoiseTexture=IRenderer::GetRendererInstance()->addImageLibTexture("/new/x/x/SnowAccumulation/noiseL8_32x32x32.dds", false);
...
void RenderScene( bool accum)
{
	...
	D3DXFROMWINEMatrixInverse(&mViewInv,NULL,&mView);
	...
	for (iPass = 0; iPass < cPasses; iPass++)
	{
		...
		for(std::list<PlacedMesh>::iterator i=g_SceneMeshList.begin();i!= g_SceneMeshList.end();i++)
		{
			...
			if(accum == (*i).bAccumSnow)
			{
				...
				D3DXFROMWINEMatrixInverse(&mInvWorld,NULL,&mWorld);
				...
				D3DXFROMWINEMatrixTranspose(&mWorldIT,D3DXFROMWINEMatrixInverse(&mWorldIT,NULL,&mWorld));
    ...
    IRenderer::GetRendererInstance()->BeginScene();
    {
		...
		if(g_SampleUI.GetCheckBox(IDC_SHOWEXPMAP)->GetChecked())
		{
			...
			if(g_pExposureTextureR32 != NULL)
			{
				...
				RenderOrthoDepth(IRenderer::GetRendererInstance());
				(IRenderer::GetRendererInstance()->StretchRect(g_pExposureSurface,NULL,g_pBBSurface,&destRect,D3DTEXF_NONE));
				g_pExposureSurface->Release();
