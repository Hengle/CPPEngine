...
{
    int hr = 0;
	g_pEffect=IRenderer::GetRendererInstance()->addEffectFromFile("/new/x/x/Rain/Rain.fx");
    ...
    //load the scene
    g_Mesh.Load("/new/x/x/Rain/Bridge/Bridge.x");
    ...
    //load the arrow mesh
    g_MeshArrow.Load("/new/x/x/Rain/Bridge/arrow.x"); 
    ...
    (loadTextureFromFile("/new/x/x/Rain/Bridge/bridge_color.dds","SceneTextureDiffuse")); 
    (loadTextureFromFile("/new/x/x/Rain/Bridge/bridge_spec.dds","SceneTextureSpecular")); 
    (loadTextureFromFile("/new/x/x/Rain/Bridge/bridge_normal.dds","SceneTextureNormal")); 

    //the 3D textures for the rain splashes
    (load3DTextureFromFile("/new/x/x/Rain/rainTextures/splashes/SBumpVolume.dds","SplashBumpTexture"));
    (load3DTextureFromFile("/new/x/x/Rain/rainTextures/splashes/SDiffuseVolume.dds","SplashDiffuseTexture"));
    ...
    TextureID  rainTexture = 0;
    TextureID  textureRV = 0;
    stx_Effect::stx_Variable*   textureArray = g_pEffect->GetVariableByName( "rainTextureArray" );
    ( LoadTextureArray( "/new/x/x/Rain/rainTextures/cv0_vPositive_", 370 , &rainTexture, &textureRV) );
    ...
    if(loadLUTS("/new/x/x/Rain/F_512_data.csv","Ftable",512,512) == 1)
        loadLUTS("/new/x/x/Rain/F_512_data.csv","Ftable",512,512);             
    if(loadLUTS("/new/x/x/Rain/G0_pi_2_64_data.csv","Gtable",64,64) == 1)
        loadLUTS("/new/x/x/Rain/G0_pi_2_64_data.csv","Gtable",64,64); 
    if(loadLUTS("/new/x/x/Rain/G20_pi_2_64_data.csv","G_20table",64,64) == 1)
        loadLUTS("/new/x/x/Rain/G20_pi_2_64_data.csv","G_20table",64,64); 
...
int  OnD3D10ResizedSwapChain()
{
    ...
    // ??? TextureID  pNULLRT = {0};IRenderer::GetRendererInstance()->OMSetRenderTargets( 1, &(pNULLRT), 0 );
...
// of textures on disk.
//--------------------------------------------------------------------------------------
int LoadTextureArray( char* sTexturePrefix, int iNumTextures, TextureID * ppTex2D, TextureID * ppSRV)
{
    int hr = 0;
    char str[MAX_PATH];
    for(int i=0; i<iNumTextures; i++)
    {
        stx_snprintf(str, MAX_PATH, "%S%.4d.dds", sTexturePrefix, i);
    	*(ppTex2D+i)=IRenderer::GetRendererInstance()->addImageLibTexture(str, true, IRenderer::GetRendererInstance()->Getlinear());
