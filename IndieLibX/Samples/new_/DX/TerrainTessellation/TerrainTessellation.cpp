...
int CreateTerrainEffect()
{
    ...
    g_pTerrainEffect=IRenderer::GetRendererInstance()->addEffectFromFile("/TerrainTessellation/TerrainTessellation.fx");
...
{
    g_pDeformEffect=IRenderer::GetRendererInstance()->addEffectFromFile("/Deformation.fx");
...
int  OnCreateDevice()
{
	...
	g_Skybox.init("/TerrainTessellation/MilkyWayCube.dds");
	ReleaseStars();
	CreateStars();
...
void RenderTerrain( const D3DXFROMWINEMATRIX& mProj, D3DFROMWINEVIEWPORT9& vp, const char* passOverride=0)
{
	...
	if (g_HwTessellation)
	{
		// ??? IRenderer::GetRendererInstance()->DrawPrimitive(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
