...
void TileRing::CreateInputLayout(stx_Effect::stx_Technique* pTech)
{
	...
	assert(pTech != 0);	// Load this first.
	D3DXFROMWINE11_PASS_DESC passDesc;
	(pTech->GetPassByIndex(0)->GetDesc(&passDesc));

	(->CreateInputLayout(layout, N_ELEMS, passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &m_pInputLayout));
}
...
void TileRing::CreateInstanceDataVB()
{
    ...
    int hr;
    (->CreateBuffer(&vbDesc, &initData, &m_pPositionsVB));
}
...
void TileRing::SetRenderingState() const
{
	...
	IRenderer::GetRendererInstance()->setVertexBuffer(0, 1, &m_pPositionsVB, uiVertexStrides, uOffsets);
}
