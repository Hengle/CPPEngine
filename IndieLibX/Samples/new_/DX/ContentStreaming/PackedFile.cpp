...
bool CPackedFile::CreatePackedFile( ID3D10Device* pDev10, IDirect3DDevice9* pDev9, char* szFileName,
                                    unsigned int SqrtNumTiles, unsigned int SidesPerTile, float fWorldScale, float fHeightScale )
{
    ...
    if( FAILED( DXUTFindDXSDKMediaFileCch( strTerrainHeight.str, MAX_PATH, L"contentstreaming\\terrain1.bmp" ) ) )
        return false;
    CTerrain Terrain;
    int hr = Terrain.LoadTerrain( strTerrainHeight.str, SqrtNumTiles, SidesPerTile, fWorldScale, fHeightScale,
                                      true );
    if( FAILED( hr ) )
        return false;
