...
// Constructor
//-----------------------------------------------------------------------------
Physics::Physics(IDirect3DDevice9 *d3dDevice, Renderer *renderer)
{
...
void Physics::FillWaterUtilityBuffers(float windSpeed, float windDirection)
{
    ...
    {
        kx = (-((float)WATER_GRID_RESOLUTION) / 2.0f + ((float)i)) * (2.0f * D3DX_PI / WX);

        for (j = 0; j <= WATER_GRID_RESOLUTION; j++)
        {
            ky = (-((float)WATER_GRID_RESOLUTION) / 2.0f + ((float)j)) * (2.0f * D3DX_PI / WY);
    ...
    {
        kx = (-((float)HIGH_FREQ_WATER_GRID_RESOLUTION) / 2.0f + ((float)i)) * (2.0f * D3DX_PI / WX);

        for (j = 0; j <= HIGH_FREQ_WATER_GRID_RESOLUTION; j++)
        {
            ky = (-((float)HIGH_FREQ_WATER_GRID_RESOLUTION) / 2.0f + ((float)j)) * (2.0f * D3DX_PI / WY);
