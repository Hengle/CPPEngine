...
void initGUI()
{
	...
	mainMenu->addWidget(numModelSlider = new Slider(760, 10, 26, 580, false, 1, MAX_NUM_MODELS/4, 2500));
...
bool loadBoneAnimationMap(Model *m, Batch *b, char* name)
{
	...
	fread(&m->numBones, sizeof(__DWORD__), 1, fp);
	fread(&m->numFrames, sizeof(__DWORD__), 1, fp);
	...
	BONE_MATRIX_VALUE_FORMAT* lpC = (BONE_MATRIX_VALUE_FORMAT *)tex.create(BONE_MATRIX_FORMAT, 4*m->numBones, m->numFrames, 1, 1); 
	{
		for (uint f=0; f<m->numFrames; f++)
			for (__DWORD__ i=0; i<m->numBones; i++)
			{
...
bool loadModel(Model *m, Batch *b, char* name)
{
	...
	fread(&m->numVertices, sizeof(__DWORD__), 1, fp);
	if (m->numVertices%2)
	...
	lpUVList = new D3DXFROMWINEVECTOR2[m->numVertices];
	...
	if ((m->vertexFormat = IRenderer::GetRendererInstance()->addVertexFormat(modelVertexAttribs, elementsOf(modelVertexAttribs))) == VF_NONE) return false;
	...
	if ((m->vsVertexFormat = IRenderer::GetRendererInstance()->addVertexFormat(vsModelVertexAttribs, elementsOf(vsModelVertexAttribs))) == VF_NONE) return false;
	...
	for (__DWORD__ i=0; i<m->numVertices; i++)
	{
		fread(dest, sizeof(D3DXFROMWINEVECTOR4), 1, fp);
		dest++;
	...
	float *uvdest = &uv[0];
		fread(lpUVList, sizeof(float)*6, m->numVertices, fp);
		for (uint j=0; j<m->numVertices; j++)
		{
            ...
            *uvdest++;
            *uvdest++;
			*uvdest++ = float(lpUVList[j].x);
			*uvdest++ = float(lpUVList[j].y);
		}
	m->UVBuffer = IRenderer::GetRendererInstance()->addVertexBuffer(uv.size()*sizeof(float)*6, STATIC, &uv[0]);

	// Create index buffer of model
	fread(&m->numTriangles, sizeof(__DWORD__), 1, fp);
	lpTriangleList = new __WORD__[m->numTriangles*3];

	// Create the triangle index buffer
	std::vector<__WORD__> vi(m->numTriangles*3);
	__WORD__ *iDest = &vi[0];
		fread(lpTriangleList, sizeof(__WORD__), m->numTriangles*3, fp);
		memcpy(iDest, lpTriangleList, sizeof(__WORD__)*m->numTriangles*3);
	m->indexBuffer = IRenderer::GetRendererInstance()->addIndexBuffer(vi.size(), 2, STATIC, &vi[0]);
	...
	*/
	...
	if ((b->vertexFormat = IRenderer::GetRendererInstance()->addVertexFormat(modelVertexAttribs, elementsOf(modelVertexAttribs))) == VF_NONE) return false;

    {
	// Create the vertex texture coordinate buffer of models for batch rendering
	std::vector<float> uv(6*b->vertexTextureWidth * b->vertexTextureHeight);
	float *lpCUV = &uv[0];
		...
		{			
			...
			for (uint j=0; j<m->numVertices; j++)
			{
				...
				*uvdest++;
				*uvdest++;
				*uvdest++ = float(lpUVList[j].x+OffsetX);
				*uvdest++ = float(lpUVList[j].y+OffsetY);
			}
			lpCUV += (m->vertexTextureWidth*m->vertexTextureHeight*6);
			uvdest = lpCUV;
		}
	b->UVBuffer = IRenderer::GetRendererInstance()->addVertexBuffer(uv.size()*sizeof(float)*6, STATIC, &uv[0]);
    }
	/*
	...
	*/
	...
	std::vector<__DWORD__> vi(b->numTriangles*3);
	__DWORD__ *dDest = &vi[0];
	for (__DWORD__ i=0; i<maxBatchOnce; i++)
	{
		for (__DWORD__ t=0; t<m->numTriangles*3; t++)
		{
			*dDest++ = (__DWORD__)lpTriangleList[t] + i*m->vertexTextureWidth*m->vertexTextureHeight;
		}
	}
	b->indexBuffer = IRenderer::GetRendererInstance()->addIndexBuffer(vi.size(), 4, STATIC, &vi[0]);
	}
    Image3 tex;
...
{	
	...
	// Load shaders
	animationSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/animation.shd", "main", "main");
	skinningSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/skinning.shd", "main", "main");
	modelSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/model.shd", "main", "main");
	batchAnimationSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/batchAnimation.shd", "main", "main");
	batchSkinningSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/batchSkinning.shd", "main", "main");
	batchModelSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/batchModel.shd", "main", "main");
	vsModelSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Animation/vsModel.shd", "main", "main");
	...
	// Load textures
	model[0].diffuse = IRenderer::GetRendererInstance()->addImageLibTexture("/R2VB/boy.dds", false, IRenderer::GetRendererInstance()->GettrilinearAniso());
	model[1].diffuse = IRenderer::GetRendererInstance()->addImageLibTexture("/R2VB/ruby.dds", false, IRenderer::GetRendererInstance()->GettrilinearAniso());
	
	FormatDesc quadAttribs[] = { 
		0, TYPE_VERTEX, FORMAT_FLOAT, 4,
		0, TYPE_TEXCOORD, FORMAT_FLOAT, 2	};
	QuadVF = IRenderer::GetRendererInstance()->addVertexFormat(quadAttribs, elementsOf(quadAttribs));
	...
	FormatDesc batchQuadAttribs[] = { 
		0, TYPE_VERTEX, FORMAT_FLOAT, 4,
		0, TYPE_TEXCOORD, FORMAT_FLOAT, 2};
	batch_QuadVF = IRenderer::GetRendererInstance()->addVertexFormat(batchQuadAttribs, elementsOf(batchQuadAttribs));
	...
	dest = &vv[0];
		memcpy(dest, batchQuadV, sizeof(float)*4*6);
	batch_QuadVB = IRenderer::GetRendererInstance()->addVertexBuffer(sizeof(float)*6*4, STATIC, dest);

	// Load model data
	if (!loadModel(&model[0], &batch[0], "/R2VB/npc/npc_01.mdl")) return false;
	// Load skinning vertex maps
	if (!loadSkinningVertexMap(&model[0], "/R2VB/npc/npc_01.skn")) return false;
	// Load animation maps
	if (!loadBoneAnimationMap(&model[0], &batch[0], "/R2VB/npc/npc_01.ani")) return false;

	// Load model data
	if (!loadModel(&model[1], &batch[1], "/R2VB/ruby/ruby.mdl")) return false;
	// Load skinning vertex maps
	if (!loadSkinningVertexMap(&model[1], "/R2VB/ruby/ruby.skn")) return false;
	// Load animation maps
	if (!loadBoneAnimationMap(&model[1], &batch[1], "/R2VB/ruby/ruby.ani")) return false;
...
void psCalculateAnimation(float t, float interp)
{
	IRenderer::GetRendererInstance()->changeRenderTarget(model[currentModel].boneMatrix);
		...
		// ??? IRenderer::GetRendererInstance()->setShader();
...
void drawFrame()
{
	...
	//mainFont.drawText( str, 120, 7, 32, 40, 0.17f, -0.12f, HA_LEFT, VA_TOP);
}
private:
	#if 1
