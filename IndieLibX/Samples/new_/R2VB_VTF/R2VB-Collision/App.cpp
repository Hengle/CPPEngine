...
private:
	...
	VertexFormatID	m_pPositionTTexcoordVertexDeclaration;
...
bool load()
{
	...
	// Load shaders
	//
	m_sceneShader				= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/Scene.shd", "main", "main");
	m_scenewireframeShader		= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/Scene_wireframe.shd", "main", "main");
	m_overlayShader			= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/Overlay.shd", "main", "main");
	m_particlesmathShader		= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/ParticlesMath.shd", "main", "main");
	m_particlesrenderShader	= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/ParticlesRender.shd", "main", "main");
	m_rayintersectionShader	= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/RayIntersection.shd", "main", "main");
	m_skyboxShader				= IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-Collision/skybox.shd", "main", "main");
	...
	TerrainTexture = IRenderer::GetRendererInstance()->addImageLibTexture("/R2VB/land_DIS_512.hdr", false, IRenderer::GetRendererInstance()->GetnearestClamp());

	// Terrain diffuse and illumination textures
	TerrainDiffuseTexture = IRenderer::GetRendererInstance()->addImageLibTexture("/R2VB/land_DL.dds", false, IRenderer::GetRendererInstance()->Getbilinear());

	// Load snow texture
	SnowFlakeTexture = IRenderer::GetRendererInstance()->addImageLibTexture("/R2VB/particle.dds", false, IRenderer::GetRendererInstance()->Getbilinear());
	...
	// Noise texture
	NoiseTexture=IRenderer::GetRendererInstance()->addImageLibTexture("/R2VB/noise2D.tga", false, IRenderer::GetRendererInstance()->Getbilinear());
	...
	const FormatDesc positiont16fVertexAttribs[] = {
		0, TYPE_VERTEX,   FORMAT_HALF, 4
	};
	// Pre-transformed position data and 1 pair of texture coordinates
	const FormatDesc positiontTexcoordVertexAttribs[] = {
		0, TYPE_VERTEX,   FORMAT_FLOAT, 4,		
		0, TYPE_TEXCOORD, FORMAT_FLOAT, 2,
	};

	// Create vertex declarations
	m_pPositionVertexDeclaration=IRenderer::GetRendererInstance()->addVertexFormat(position16fVertexAttribs, elementsOf(position16fVertexAttribs), -1);
	m_pPositionTVertexDeclaration=IRenderer::GetRendererInstance()->addVertexFormat(positiont16fVertexAttribs, elementsOf(positiont16fVertexAttribs), -1);
	m_pPositionTTexcoordVertexDeclaration=IRenderer::GetRendererInstance()->addVertexFormat(positiontTexcoordVertexAttribs, elementsOf(positiontTexcoordVertexAttribs), -1);

	//
	// Create index buffer for terrain
	//
	std::vector<__DWORD__> vi((dwTerrainMapWidth-1)*(dwTerrainMapLength-1)*2*3);
#if 0 // ???
	// Lock index buffer
	__DWORD__ *pIndex=&vi[0];
	// Fill index buffer
...
void drawFrame()
{
	...
	if (nFrame==0) {
		...
		DrawScreenAlignedQuad( (float)dwTerrainMapWidth, (float)dwTerrainMapLength, 0.0f);
	}
	...
	if (bIncreaseTerrain || bDecreaseTerrain) {
		...
		IRenderer::GetRendererInstance()->setVertexFormat(m_pPositionTVertexDeclaration);
	...
	// Set vertex declaration
	IRenderer::GetRendererInstance()->setVertexFormat(m_pPositionVertexDeclaration);
	...
	// Draw quad
	DrawScreenAlignedQuad( PARTICLES_TEXTURE_SIZE, PARTICLES_TEXTURE_SIZE, 0.0f);
	...
	if (DemoMenu && DemoMenu->isItemChecked(Display2DView)) {
		...
		// Set vertex declaration
		IRenderer::GetRendererInstance()->setVertexFormat(m_pPositionTTexcoordVertexDeclaration);
