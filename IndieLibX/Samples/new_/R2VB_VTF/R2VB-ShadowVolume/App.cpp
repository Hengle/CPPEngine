...
public:
	VertexFormatID VertexFormat, ambientModelVF;
	VertexBufferID VertexBuffer;
	IndexBufferID IndexBuffer;
	ShaderID Shader, ambientModelSH;
...
{
	// Load shaders
	model.animationSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/animation.shd", "main", "main");
	model.skinningSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/skinning.shd", "main", "main");
	model.modelSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/model.shd", "main", "main");
	model.ambientModelSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/model_ambient.shd", "main", "main");
	model.calculateNormalSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/normal.shd", "main", "main");
	model.shadowVolumeSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/ShadowVolume.shd", "main", "main");
	...
	// ??? if ((model.VF = IRenderer::GetRendererInstance()->addVertexFormat(modelVertexAttribs, elementsOf(modelVertexAttribs), model.Shader)) == VF_NONE) return false;
	...
	__WORD__ *iDest = &vi[0];
		...
		memcpy(iDest, model.pTriangleList, sizeof(__WORD__)*model.numTriangles*3);
	...
	*/
	...
	model.QuadVB = IRenderer::GetRendererInstance()->addVertexBuffer(4 * 6 * sizeof(float), STATIC, &vv[0]);
...
bool load()
{
	...
	// Load shaders
	if ((skybox = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/skybox.shd")) == SHADER_NONE) return false;
	...
	// ??? if (!floor.load(MODEL_PATH "floor/floor.am", IRenderer::GetRendererInstance())) return false;
...
void drawFloor(Light &lit, bool ambient)
{
	...
	{
		IRenderer::GetRendererInstance()->setShader(floor.Shader);
	IRenderer::GetRendererInstance()->setVertexFormat(floor.VertexFormat);
...
bool Floor::load(char *name)
{
	...
	if ((Shader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/floor.shd")) == SHADER_NONE) return false;
	if ((ambientModelSH = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-ShadowVolume/floor_ambient.shd")) == SHADER_NONE) return false;
	ATIModel::load(name);
	FormatDesc modelVertexAttribs[] = {
