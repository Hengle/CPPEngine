...
bool SnowSystem::load(__DWORD__ maxtexwidth,__DWORD__ maxtexheight, bool debugruntime, bool supportsHalf){
	...
	*/
	...
	// Load shaders
	if ((particleShader = IRenderer::GetRendererInstance()->addShaderFromFile("particle.shd", "main", "main"   )) == SHADER_NONE) return false;
	if ((physics        = IRenderer::GetRendererInstance()->addShaderFromFile("physics.shd", "main", "main"    )) == SHADER_NONE) return false;
	if ((initPhysics    = IRenderer::GetRendererInstance()->addShaderFromFile("initPhysics.shd", "main", "main")) == SHADER_NONE) return false;
	if ((makeQuads      = IRenderer::GetRendererInstance()->addShaderFromFile("makeQuads.shd", "main", "main"  )) == SHADER_NONE) return false;

	// Load textures
	particle = IRenderer::GetRendererInstance()->addImageLibTexture("article.dds", false, IRenderer::GetRendererInstance()->GetbilinearClamp());

	FormatDesc particleAttribs[] = {
		0, TYPE_VERTEX,   supportsHalf? FORMAT_HALF : FORMAT_FLOAT, 4,
		1, TYPE_TEXCOORD, FORMAT_USHORT, 2,
	};
	if ((particleVF = IRenderer::GetRendererInstance()->addVertexFormat(particleAttribs, elementsOf(particleAttribs))) == TEXTURE_NONE) return false;

	FormatDesc physicsAttribs[] = { 0, TYPE_VERTEX, FORMAT_FLOAT, 2 };
	if ((physicsVF = IRenderer::GetRendererInstance()->addVertexFormat(physicsAttribs, elementsOf(physicsAttribs))) == TEXTURE_NONE) return false;
...
void SnowSystem::draw(const D3DXFROMWINEMATRIX &mvp, D3DXFROMWINEMATRIX &mv, float frametime){
	...
	IRenderer::GetRendererInstance()->setVertexBuffer(quadCoordVB, 0); // Dummy vertex buffer
	IRenderer::GetRendererInstance()->setVertexBuffer(quadCoordVB, 1);
