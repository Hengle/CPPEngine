...
GPU_Physics::GPU_Physics() : Physics()
{
    ...
    // Load shaders
    if ((m_HorizontalButterflyShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/HorizontalButterfly.shd")) == SHADER_NONE)
        return;
    if ((m_VerticalButterflyShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/VerticalButterfly.shd")) == SHADER_NONE)
        return;
    if ((m_HorizontalScrambleShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/HorizontalScramble.shd")) == SHADER_NONE)
        return;
    if ((m_VerticalScrambleShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/VerticalScramble.shd")) == SHADER_NONE)
        return;
    if ((m_ExpandShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/Expand.shd")) == SHADER_NONE)
        return;
    if ((m_CopyShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/Copy.shd")) == SHADER_NONE)
        return;
    if ((m_WaterSpectrumShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/WaterSpectrum.shd")) == SHADER_NONE)
        return;
    if ((m_ComputeNormalsShader = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-OceanWater/ComputeNormals.shd")) == SHADER_NONE)
        return;
                                            ...
                                            TextureID* ppIFFTButterflyMap)
{
	...
	*ppScrambleMap=IRenderer::GetRendererInstance()->addTexture(&img, false, IRenderer::GetRendererInstance()->Getlinear());
    ...
    float* fImage = (float*)fimg.create(FORMAT_RGBA32f, iResolution, 16, 1, 0);
    float* wImage = (float*)wimg.create(FORMAT_RGBA32f, iResolution, 16, 1, 0);
    ...
    //
    //fImage = new float[iResolution*16*4];
    memset( fImage, 0, iResolution*16*4*sizeof(float));
                      ...
                      int iResolution, int iLog2Resolution)
{
    ...
    ( IRenderer::GetRendererInstance()->changeRenderTarget(0, pDstSurf));
    ...
    if (iResolution == WATER_GRID_RESOLUTION)
    {
        ( IRenderer::GetRendererInstance()->changeRenderTarget(0, pOutputSurf));
