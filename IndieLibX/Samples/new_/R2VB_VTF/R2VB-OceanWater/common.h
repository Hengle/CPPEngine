...
// Macros for error checking
    ...
    #define (x)           { hr = x; if( FAILED(hr) ) { DXTrace( __FILE__, (__DWORD__)__LINE__, hr, NULL, true ); } }
    #define V_RETURN(x)    { hr = x; if( FAILED(hr) ) { return DXTrace( __FILE__, (__DWORD__)__LINE__, hr, NULL, true ); } }
#else
    #define (x)           { hr = x; }
