...
void changeNPatchLevel(const int level){
	...
	*/
	rtVertex = IRenderer::GetRendererInstance()->addRenderTarget(vertexFactor * texWidth, texHeight, FORMAT_RGBA16f, IRenderer::GetRendererInstance()->GetnearestClamp());

	rtNormal = IRenderer::GetRendererInstance()->addRenderTarget(vertexFactor * texWidth, texHeight, FORMAT_RGBA16f, IRenderer::GetRendererInstance()->GetnearestClamp());
...
bool load(){
	...
	// Load shaders
	lighting = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-NPatches/lighting.shd", "main", "main");
	nPatchLighting = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-NPatches/lighting.shd", "main", "main", "#define N_PATCHES\n");
	wireframe = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-NPatches/wireframe.shd", "main", "main");
	nPatch = IRenderer::GetRendererInstance()->addShaderFromFile("/new/x/x/R2VB_VTF/R2VB-NPatches/NPatch.shd", "main", "main");

#if 0
	// Load the source model
	MeshRenderer2* mesh;
	if (FAILED(D3DXLoadMeshFromX(MODEL_PATH "tiger.x", D3DXMESH_MANAGED, IRenderer::GetRendererInstance(), 0, 0, 0, 0, &mesh))) return false;
	...
	mesh->CloneMesh(D3DXMESH_SYSTEMMEM, modelVertexAttribs, IRenderer::GetRendererInstance(), &tiger);
	mesh->Release();

	D3DXComputeNormals(tiger, 0);
...
#endif
	base=IRenderer::GetRendererInstance()->addImageLibTexture("tiger.bmp", false, IRenderer::GetRendererInstance()->GettrilinearAniso());
