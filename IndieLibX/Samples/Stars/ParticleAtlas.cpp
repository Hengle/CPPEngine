...
CVertexCache* cParticleEmitter::m_vertexCache=0;
#endif

//int cParticleEmitter::atlasTextureCount=0;
...
//}
 ...
 AddTextureToAtlas(atlas0,grass1, 1.0f );
 AddTextureToAtlas(atlas0,grass4, 1.0f );
...
*/
 AddTextureToAtlas(atlas0,tree4, tree1gamma );
  AddTextureToAtlas(atlas0,tree1, tree2gamma );
  AddTextureToAtlas(atlas0,tree2, tree3gamma );
  AddTextureToAtlas(atlas0,tree3, tree4gamma );
  AddTextureToAtlas(atlas0,grass2, grass2gamma );
  AddTextureToAtlas(atlas0,grass3, grass3gamma );
  AddTextureToAtlas(atlas0,grass5, grass5gamma );
...
for( __WORD__ i=0; i<NUMTREETEXTURES; i++ )
{
	...
	for( __WORD__ j=0; j<16; j++ )
	{
	     ...
	     AddTextureToAtlas(atlas0,img, 1.0f );
	}
...
sun.loadImageLibImage("/Stars/SolarSystemDisks/sun.jpg");
 ...
 AddTextureToAtlas(atlas0,sun, 1.0f );
	
 AddTextureToAtlas(atlas0,moon, 1.0f );
	
 AddTextureToAtlas(atlas0,star, 1.0f );
	
 AddTextureToAtlas(atlas0,mercury, 1.0f );
	
 AddTextureToAtlas(atlas0,venus, 1.0f );
	
 AddTextureToAtlas(atlas0,mars, 1.0f );
	
 AddTextureToAtlas(atlas0,jupiter, 1.0f );
	
 AddTextureToAtlas(atlas0,saturn, 1.0f );
	
 AddTextureToAtlas(atlas0,uranus, 1.0f );
	
 AddTextureToAtlas(atlas0,neptune, 1.0f );
	
 AddTextureToAtlas(atlas0,pluto, 1.0f );
...
#if 0
   atlas0.saveDDS("/Stars/SkyAtlas.dds");
...
*/
	...
	//LOG_PRINT("%s -> illuminated disk %f\n",name,au);
		switch(body)
		{
			case MERCURY:
				au = ln_get_mercury_magnitude (JD);
...
cParticleEmitter::CatalogElement cParticleEmitter::catalog[9111];
...
//	float fLeft, fRight, fBottom, fTop, fBack, fFront;
 ...
 vLine[1]=vPickRayOrig+vPickRayDir*1000000.0f;
	...
	D3DXFROMWINEVec3TransformCoord(&pCenter,pCenter0,&matWorld);//m2);
/*
