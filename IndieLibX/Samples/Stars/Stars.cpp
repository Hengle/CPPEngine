...
#endif
#undef __figure_border__
#define __figure__border__
#define _DrawFigures_ 1
#define __ATLAS__ 1
#ifdef __ATLAS__
...
//LOG_PRINT("constellationID=%d\n", constellationID);
            ...
            //LOG_FNLN;
			  ...
			  constellationpositions[constellationID]+=position/constellationBorderNum[constellationID];
#if 0
               LOG_FNLN;
...
//LOG_FNLN;
	...
	if(((p+constellationBorderNum[c]-1)<0) || ((p+constellationBorderNum[c]-1)>constellationnum))
		continue;
...
void DrawFigures()
{
	...
	line[0].colour=D3DXFROMWINEVECTOR4(1.0f,1.0f,1.0f,1.0f);
	line[1].colour=D3DXFROMWINEVECTOR4(1.0f,1.0f,1.0f,1.0f);
	   D3DXFROMWINEMATRIX mvp=g_CameraProjMatrix*g_CameraViewMatrix;
...
*/
	...
	//id0++;
	line[0].pos=GetPosition3(cParticleEmitter::catalog[id0].ra,cParticleEmitter::catalog[id0].dec);
/*
	...
	for(unsigned int i=1;i<constellationFigureNum2[figureFileID];i++)
	{
		...
		if(cmd==0)
		{
			//cmd0=cmd;
			//id0=id;
			line[0].pos=GetPosition3(cParticleEmitter::catalog[id].ra,cParticleEmitter::catalog[id].dec);
/*
...
*/
		...
		{
			line[1].pos=GetPosition3(cParticleEmitter::catalog[id].ra,cParticleEmitter::catalog[id].dec);
/*
			//LOG_PRINT("name1   =%s\n",cParticleEmitter::catalog[id].name);
...
*/
	...
	LOG_PRINT("line[1].pos.z=%f\n", line[1].pos.z);
	#endif
	IRenderer::GetRendererInstance()->setShader(shd3);IRenderer::GetRendererInstance()->setVertexFormat(vf3); D3DXFROMWINEMATRIX mvp=GetMVP();
...
#endif
void DrawConstellationNames(const __DWORD__ stride, const D3DFROMWINEVIEWPORT9& viewport, const D3DXFROMWINEMATRIX& matProj, const D3DXFROMWINEMATRIX& matView, const D3DXFROMWINEMATRIX& matWorld)
{
...
*/
	...
	DrawConstellationNames(stride, viewport, matProj, matView, matWorld);
