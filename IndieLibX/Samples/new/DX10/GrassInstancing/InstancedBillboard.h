...
int InstancedBillboard<T_VERTEX, T_INSTANCE>::CreateDevice( stx_Effect*  pTechique)
{
	...
	( ->CreateBuffer( &bufferDesc, 0, &pInstanceBuffer ) );
...
template <class T_VERTEX, class T_INSTANCE>
int InstancedBillboard<T_VERTEX, T_INSTANCE>::LoadInstData( exBufferID* ppInstanceData, void* pInData, int dataSize )
{
	...
	( ->CreateBuffer( &bufferDesc, 0, ppInstanceData ) );
