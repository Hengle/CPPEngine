...
int  OnCreateDevice()
{
    ...
    // Initialize the font
    ( D3DXFROMWINECreateFont( 12, 0, FW_NORMAL, 1, FALSE, DEFAULT_CHARSET,
                              OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
                              "Arial", &g_pFont ) );

    char str[ MAX_PATH ];
    ( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, "misc/Arrows.bmp" ) );
    if( ( hr ) )
    {
        // Couldn't find the sprites
        LOG_PRINT( "OnCreateDevice() - Could not locate 'Arrows.bmp'.\n" );
        return hr;
    }

    ( D3DXFROMWINECreateTextureFromFile( str, &g_pArrowTex ) );
    if( ( hr ) )
    {

        // Couldn't load the sprites!
        LOG_PRINT( "OnCreateDevice() - Could not load 'Arrows.bmp'.\n" );
        return hr;
...
int  OnResetDevice(  const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    char str[MAX_PATH];
    ( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, "Shader Code/FinalPass.psh" ) );
    ( D3DXFROMWINECompileShaderFromFile( str,
                                         0, 0,
                                         "main",
                                         "ps_2_0",
                                         0,
