...
int CreateResources(, const D3DSURFACE_DESC* pDisplayDesc )
{
    ...
    //------------------------------------
    char str[MAX_PATH];
    ( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, "Shader Code/HDRSource.psh" ) );
    ...
    if( ( hr ) )
    {
        ...
        LOG_PRINT( "HDRScene::CreateResources() - Compiling of 'HDRSource.psh' failed!\n" );
        return hr;

    }


    ( IRenderer::GetRendererInstance()->CreatePixelShader( reinterpret_cast< __DWORD__* >( pCode->GetBufferPointer() ), &HDRScene::g_pCubePS ) );
    if( ( hr ) )
    {

        // Couldn't turn the compiled shader into an actual, usable, pixel shader!
        LOG_PRINT(
            "HDRScene::CreateResources() : Couldn't create a pixel shader object from 'HDRSource.psh'.\n" );
        pCode->Release();
        return hr;
    ...
    //--------------------------------------
    ( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, "Shader Code/HDRSource.vsh" ) );
    ( D3DXFROMWINECompileShaderFromFile(
       str,
       0, 0,
       "main",
    ...
    if( ( hr ) )
    {
        ...
        // Couldn't compile the shader, use the 'compile_shaders.bat' script
        // in the 'Shader Code' folder to get a proper compile breakdown.
        LOG_PRINT( "HDRScene::CreateResources() - Compilation of 'HDRSource.vsh' Failed!\n" );
        return hr;

    }

    ( IRenderer::GetRendererInstance()->CreateVertexShader( reinterpret_cast< __DWORD__* >( pCode->GetBufferPointer() ), &HDRScene::g_pCubeVS ) );
    if( ( hr ) )
    {

        // Couldn't turn the compiled shader into an actual, usable, vertex shader!
        LOG_PRINT(
            "HDRScene::CreateResources() - Could not create a VS object from the compiled 'HDRSource.vsh' code.\n" );
        pCode->Release();
        return hr;
    ...
    if( ( hr ) )
    {
        ...
        // Couldn't load the mesh, could be a file system error...
        LOG_PRINT( "HDRScene::CreateResources() - Could not load 'OcclusionBox.x'.\n" );
        return hr;
    ...
    //----------------------------------------
    FormatDesc vertElems[MAX_FVF_DECL_SIZE];
    HDRScene::g_pOcclusionMesh->GetDeclaration( vertElems );

    ( IRenderer::GetRendererInstance()->addVertexFormat( vertElems, &HDRScene::g_pOcclusionVSDecl ) );
    if( ( hr ) )
    {

        // Couldn't create the declaration for the loaded mesh..
        LOG_PRINT(
            "HDRScene::CreateResources() - Couldn't create a vertex declaration for the occlusion mesh.\n" );
        return hr;
    ...
    //----------------------------------------
    ( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, "Shader Code/OcclusionMesh.vsh" ) );
    ( D3DXFROMWINECompileShaderFromFile(
       str,
       0, 0,
       "main",
    ...
    if( ( hr ) )
    {
        ...
        // Couldn't compile the shader, use the 'compile_shaders.bat' script
        // in the 'Shader Code' folder to get a proper compile breakdown.
        LOG_PRINT( "HDRScene::CreateResources() - Compilation of 'OcclusionMesh.vsh' Failed!\n" );
        return hr;

    }

    ( IRenderer::GetRendererInstance()->CreateVertexShader( reinterpret_cast< __DWORD__* >( pCode->GetBufferPointer() ),
    ...
    if( ( hr ) )
    {
        ...
        // Couldn't turn the compiled shader into an actual, usable, vertex shader!
        LOG_PRINT(
            "HDRScene::CreateResources() - Could not create a VS object from the compiled 'OcclusionMesh.vsh' code.\n"
            );
        pCode->Release();
        return hr;
...
int LoadMesh( char* strFileName, MeshRenderer2 ppMesh )
{
    ...
    D3DXFROMWINELoadMeshFromX( str, D3DXFROMWINEMESH_MANAGED,
                            DXUTGetD3D9Device(), 0, 0, 0, 0, &pMesh );
    if( ( hr ) || ( pMesh == 0 ) )
        return hr;
    ...
    if( !( pMesh->GetFVF() & D3DFVF_NORMAL ) )
    {
        ...
        D3DXFROMWINEComputeNormals( pTempMesh, 0 );
    ...
    rgdwAdjacency = new __DWORD__[pMesh->GetNumFaces() * 3];
    if( rgdwAdjacency == 0 )
        return 1;
    ( pMesh->GenerateAdjacency( 1e-6f, rgdwAdjacency ) );
    pMesh->OptimizeInplace( D3DXFROMWINEMESHOPT_VERTEXCACHE, rgdwAdjacency, 0, 0, 0 );
