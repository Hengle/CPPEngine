...
int  OnResetDevice()
{
    ...
    char Path[MAX_PATH];
    DXUTFindDXSDKMediaFileCch( Path, MAX_PATH, $1 );

    D3DXFROMWINECreateTextureFromFile(Path, &g_pTexWall );


    // Create a texture to paint the floor
    DXUTFindDXSDKMediaFileCch( Path, MAX_PATH, $1 );

    D3DXFROMWINECreateTextureFromFile(Path, &g_pTexFloor );


    // Create a texture to paint the ceiling
    DXUTFindDXSDKMediaFileCch( Path, MAX_PATH, $1 );

    D3DXFROMWINECreateTextureFromFile(Path, &g_pTexCeiling );


    // Create a texture for the paintings
    DXUTFindDXSDKMediaFileCch( Path, MAX_PATH, $1 );

    D3DXFROMWINECreateTextureFromFile(Path, &g_pTexPainting );
...
int BuildWorldMesh()
{
    ...
    D3DXFROMWINECreateMeshFVF( 48, 96, 0, WorldVertex::FVF, IRenderer::GetRendererInstance(), &pWorldMeshTemp );
    if( ( hr ) )
        goto LCleanReturn;
    ...
    pWorldMeshTemp->CloneMeshFVF( D3DXFROMWINEMESH_VB_WRITEONLY | D3DXFROMWINEMESH_IB_WRITEONLY,
                                       WorldVertex::FVF, IRenderer::GetRendererInstance(), &g_pWorldMesh );
    if( ( hr ) )
        goto LCleanReturn;
