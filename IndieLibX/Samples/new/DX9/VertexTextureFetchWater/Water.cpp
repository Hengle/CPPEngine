...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
  ...
  DXUTSetCallbackDeviceCreated(Water::OnCreateDeviceCallback);
  DXUTSetCallbackDeviceReset(Water::OnResetDeviceCallback);
  DXUTSetCallbackDeviceLost(Water::OnLostDeviceCallback);
  DXUTSetCallbackDeviceDestroyed(Water::OnDestroyDeviceCallback);
  DXUTSetCallbackMsgProc(Water::MsgProcCallback);
  DXUTSetCallbackKeyboard(Water::KeyboardProcCallback);
  DXUTSetCallbackFrameRender(Water::OnFrameRenderCallback);
  DXUTSetCallbackFrameMove(Water::OnFrameMoveCallback);
  ...
  DXUTCreateDevice(D3DADAPTER_DEFAULT, true, 800, 600, Water::IsDeviceAcceptableCallback, Water::ModifyDeviceSettingsCallback);
  ...
  DXUTSetCallbackDeviceLost(NULL);
  DXUTSetCallbackDeviceDestroyed(NULL);
                               ...
                               const char* compileTarget, LPDIRECT3DVERTEXSHADER9& vertexShader)
{
  ...
  // Compile the HLSL function
  hr = D3DXCompileShaderFromFile(toUnicode(fullpath.c_str()), NULL, NULL, function, compileTarget, 0, &compiledShader, NULL, NULL);
  if(hr != D3D_OK)
    {
...
HRESULT Water::OnCreateDevice(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc)
{
  ...
  V_RETURN( m_DialogResourceManager.OnCreateDevice( pd3dDevice ) );
  V_RETURN( m_SettingsDlg.OnCreateDevice( pd3dDevice ) );
...
HRESULT Water::OnResetDevice(IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc)
{
  ...
  V_RETURN( m_DialogResourceManager.OnResetDevice() );
  V_RETURN( m_SettingsDlg.OnResetDevice() );
