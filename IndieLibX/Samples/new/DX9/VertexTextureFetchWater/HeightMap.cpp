...
extern LPDIRECT3DDEVICE9 gDirect3DDevice9;

HeightMap::HeightMap(const char* heightFilename, const D3DXVECTOR3& scale, const D3DXVECTOR3& offset)
{
  assert(heightFilename != NULL);
...
void HeightMap::createIndexBuffer(LPDIRECT3DDEVICE9 device)
{
  assert(device != NULL);
  ...
  hr = device->CreateIndexBuffer((_width - 1) * (_depth - 1) * 2 * 3 * sizeof(unsigned int), D3DUSAGE_WRITEONLY, D3DFMT_INDEX32, 
                                 D3DPOOL_MANAGED, &_indexBuffer, NULL);
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"HeightMap::createIndexBuffer()  CreateIndexBuffer() failed\n", hr);
      return;
    }
  ...
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"HeightMap::createIndexBuffer()  _indexBuffer->Lock() failed\n", hr);
      return;
    }
...
void HeightMap::createVertexBuffer(LPDIRECT3DDEVICE9 device)
{
  assert(device != NULL);
  ...
  hr = device->CreateVertexBuffer(_width * _depth * sizeof(Vertex), D3DUSAGE_WRITEONLY, Vertex::FVF, D3DPOOL_MANAGED, &_vertexBuffer, NULL);
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"HeightMap::createVertexBuffer()  CreateVertexBuffer() failed\n", hr);
      return;
    }
  ...
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"HeightMap::createVertexBuffer()  Lock() failed\n", hr);
      return;
    }
...
void HeightMap::createHeightTexture(LPDIRECT3DDEVICE9 device, LPDIRECT3DTEXTURE9& texture)
{
  assert(device != NULL);
  ...
  hr = D3DXCreateTexture(device, _width, _depth, 1, 0, D3DFMT_G16R16, D3DPOOL_MANAGED, &texture);
  assert(hr == D3D_OK);
  assert(texture != NULL);
...
void HeightMap::render(LPDIRECT3DDEVICE9 device, bool disableStateSetup)
{
  assert(device != NULL);
...
void HeightMap::setTexture(LPDIRECT3DTEXTURE9 texture)
{
  assert(texture != NULL);
...
void HeightMap::computePosition(int x, int z, D3DXVECTOR3& position)
{
...
bool HeightMap::load(const char* filename)
{
  ...
  hr = D3DXCreateTextureFromFileEx(gDirect3DDevice9, toUnicode(filename), 0, 0, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM,
  ...
  hr = texture->LockRect(0, &lockedRect, NULL, D3DLOCK_READONLY);
  assert(hr == D3D_OK);
