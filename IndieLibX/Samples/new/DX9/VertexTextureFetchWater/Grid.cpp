                                       ...
                                       const D3DXVECTOR3& scale, const D3DXVECTOR3& offset, HeightMap* heightMap)
{
  ...
  hr = device->CreateVertexBuffer(_width * _height * sizeof(Vertex), D3DUSAGE_WRITEONLY, Vertex::FVF, D3DPOOL_MANAGED, &vertexBuffer, NULL);
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"Grid::createVertexAndIndexBuffers()  CreateVertexBuffer() failed\n", hr);
      return;
    }
  ...
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"Grid::createVertexAndIndexBuffers()  vertexBuffer->Lock() failed\n", hr);
      return;
    }
  ...
  for(y=0; y<_height; y++)
    {
      for(x=0; x<_width; x++)
        {
          position = D3DXVECTOR3((float)x / widthOverTwo - 1.0f, (*this)(x, y), (float)y / heightOverTwo - 1.0f);
          position.x *= scale.x;
          position.y *= scale.y;
          position.z *= scale.z;
          position += offset;
  ...
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"Grid::createVertexAndIndexBuffers()  CreateIndexBuffer() failed\n", hr);
      return;
    }
  ...
  if(hr != D3D_OK)
    {
      DXTRACE_ERR(L"Grid::createVertexAndIndexBuffers()  indexBuffer->Lock() failed\n", hr);
      return;
    }
