...
HairGrid::HairGrid( ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext *IRenderer::GetRendererInstance() ):renderQuadBuffer(0),renderQuadBufferForGrid(0),m_pEffect(0), m_cols(0), m_rows(0),
...
int HairGrid::ReloadShader(EffectID* pEffect,char pEffectPath[MAX_PATH])
{
...
int HairGrid::Initialize( int width, int height, int depth, EffectID* pEffect,char pEffectPath[MAX_PATH] )
{
    int hr;
	...
	( IRenderer::GetRendererInstance()->CreateTexture2D(&desc,0,&pDensityDemux2DTextureArray)); 

	desc.ArraySize = m_totalDepth;
	desc.Format = RenderTargetFormats[RENDER_TARGET_VOXELIZED_OBSTACLES];
	( IRenderer::GetRendererInstance()->CreateTexture2D(&desc,0,&pVoxelizedObstacle2DTextureArray)); 

	desc.ArraySize = m_totalDepth;
	desc.Format = RenderTargetFormats[RENDER_TARGET_DENSITY_BLUR_TEMP];
	( IRenderer::GetRendererInstance()->CreateTexture2D(&desc,0,&pDensityBlurTemp2DTextureArray)); 
	
	desc.ArraySize = m_totalDepth;
	desc.Format = RenderTargetFormats[RENDER_TARGET_DENSITY_BLUR];
	( IRenderer::GetRendererInstance()->CreateTexture2D(&desc,0,&pDensityBlur2DTextureArray)); 

	desc.Format = RenderTargetFormats[RENDER_TARGET_DENSITY];
    desc.ArraySize = m_Depth;
	( IRenderer::GetRendererInstance()->CreateTexture2D(&desc,0,&pDensity2DTextureArray));
    ...
    (IRenderer::GetRendererInstance()->CreateInputLayout( layoutDesc, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &quad_layout ));
    ...
    (CreateVertexBuffer(sizeof(GRID_TEXTURE_DISPLAY_STRUCT)*m_numVerticesRenderQuad,D3D11_BIND_VERTEX_BUFFER, &renderQuadBuffer, renderQuad, m_numVerticesRenderQuad));
    delete [] renderQuad;
    renderQuad = 0;
    ...
    (CreateVertexBuffer(sizeof(GRID_TEXTURE_DISPLAY_STRUCT)*m_numVerticesRenderQuadsForGrid,D3D11_BIND_VERTEX_BUFFER, &renderQuadBufferForGrid, renderQuad, m_numVerticesRenderQuadsForGrid));
    delete [] renderQuad;
    renderQuad = 0;
...
int HairGrid::LoadShaders(EffectID* pEffect,char pEffectPath[MAX_PATH])
{
    int hr = 0;
...
int HairGrid::CreateRTTextureAsShaderResource(RENDER_TARGET rtIndex, char shaderTextureName,EffectID* pEffect,D3D11_SHADER_RESOURCE_VIEW_DESC *SRVDesc )
{
    int hr;
...
int HairGrid::AddHairDensity(D3DXFROMWINEMATRIX& objToVolumeXForm, int (*DrawHair)(D3DXFROMWINEMATRIX&,ID3D11Device*,ID3D11DeviceContext*,float,float,int))
{
    int hr = 0;
	...
	for(int z=0;z<m_Depth;z+=8) 
	{
		...
		DrawHair(worldViewProj, IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), 0.0f, 1.0/32.0, numSlices); //32 = 8 RTs * 4 slices per RT 
...
int HairGrid::demultiplexTexture()
{
	...
	{   
        ...
        (TechniqueDemux->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance()));

		//render the quad
		IRenderer::GetRendererInstance()->IASetPrimitiveTopology( PRIM_TRIANGLES );
        IRenderer::GetRendererInstance()->IASetInputLayout( quad_layout );
...
int HairGrid::voxelizeObstacles()
{
	...
	{   
    	...
    	m_GridZShaderVariable->SetInt(i);
        (TechniqueVoxelizeObstacles->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance()));
...
int HairGrid::demultiplexTextureTo3DFluidObstacles()
{
    ...
    pShaderResourceVariables[RENDER_TARGET_DENSITY]->SetResource( pRenderTargetShaderViews[RENDER_TARGET_DENSITY] );
    (TechniqueDemuxTo3DFluidObstacles->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance()));
...
int HairGrid::DisplayTexture(RENDER_TARGET rtIndex)
{
	...
	if(rtIndex==RENDER_TARGET_DENSITY)
	{     (TechniqueDrawTexture->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance()));}
	else
	{	 (TechniqueDrawTextureDemuxed->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance()));}
...
int HairGrid::CreateVertexBuffer( int ByteWidth, unsigned int bindFlags, exBufferID** vertexBuffer,
...
int HairGrid::setFluidObstacleTexture( TextureID* texture, DXGI_FORMAT format, int texWidth, int texHeight, int texDepth )
{
    int hr = 0;
