...
{
public:
    Grid( ID3D11Device* IRenderer::GetRendererInstance(),ID3D11DeviceContext* IRenderer::GetRendererInstance() );
    virtual ~Grid( void );

    int Initialize( int gridWidth, int gridHeight, int gridDepth,ID3DX11EffectTechnique* technique );
...
protected:
    ...
    int CreateVertexBuffers  (ID3DX11EffectTechnique* technique);
    ...
    int CreateLayout( D3D11_INPUT_ELEMENT_DESC* layoutDesc, unsigned int numElements, ID3DX11EffectTechnique* technique, VertexFormatID** layout);
