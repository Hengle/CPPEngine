...
struct hairShadingParameters
{
	...
	void setShaderVariables(EffectID* pEffect)
	{
...
struct collisionObject
{
	...
	D3DXFROMWINEMATRIX InitialTransform;   // the total initial transform; this is the transform that takes a unit implicit to the hair base pose
    D3DXFROMWINEMATRIX currentTransform;   // the transform that takes the implicit from base pose to the current hair space (different from world space)
    D3DXFROMWINEMATRIX objToMesh;          // the transform that goes from the hair base pose to the mesh bind pose, such that after multiplying with the meshWorldXForm we end up at the correct real world position 
};
...
extern void vectorMatrixMultiply(D3DXFROMWINEVECTOR3* vecOut,const D3DXFROMWINEMATRIX matrix,const D3DXFROMWINEVECTOR3 vecIn);
extern void vectorMatrixMultiply(D3DXFROMWINEVECTOR4* vecOut,const D3DXFROMWINEMATRIX matrix,const D3DXFROMWINEVECTOR4 vecIn);
