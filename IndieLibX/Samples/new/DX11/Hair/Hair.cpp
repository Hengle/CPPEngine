...
ID3DX11EffectTechnique*  g_pTechniqueRenderPlane = 0;
...
//techniques
ID3DX11EffectTechnique*  g_pTechniqueRenderArrow           = 0;
...
// effect variables
stx_Variable* g_pShadowsShaderVariable = 0;
stx_Variable* g_pDOMShadowsShaderVariable = 0;
stx_Variable* g_pApplyHForceShaderVariable = 0;
stx_Variable* g_pCurlyHairShaderVariable = 0;
stx_Variable* g_pAddGravityShaderVariable = 0;
stx_Variable* g_pSimulateShaderVariable = 0;
stx_Variable* g_InterHairForcesShaderVariable = 0;
stx_Variable* g_AngularStiffnessMultiplierShaderVariable = 0;
stx_Variable* g_BlurSigmaShaderVariable = 0;
stx_Variable* g_BlurRadiusShaderVariable = 0;
stx_Variable* g_UseBlurTextureForForcesShaderVariable = 0;
stx_Variable* g_UseGradientBasedForceShaderVariable = 0;
stx_Variable* g_GravityStrengthShaderVariable = 0;
stx_Variable* g_UseScalpTextureShaderVariable = 0;
stx_Variable* g_ArrowColorShaderVariable = 0;
stx_Variable* g_BaseColorShaderVariable = 0;
stx_Variable* g_SpecColorShaderVariable = 0;
stx_Variable* g_hairTextureArrayShaderVariable = 0;
stx_Variable* g_SceneDepthShaderResourceVariable = 0;
stx_Variable* g_SceneColor2XShaderResourceVariable = 0;
stx_Variable* g_collisionSphereTransformationsEV = 0;
stx_Variable* g_collisionSphereInverseTransformationsEV = 0;
stx_Variable* g_collisionCylinderTransformationsEV = 0;
stx_Variable* g_collisionCylinderInverseTransformationsEV = 0;
stx_Variable* g_SphereNoMoveImplicitInverseTransformEV = 0;
stx_Variable* g_SphereNoMoveImplicitTransformEV = 0;
...
void RenderInterpolatedHair(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), RENDERTYPE renderType, INTERPOLATE_MODEL interpolateModel, int numMStrands, int numSStrands)
{
	...
	IRenderer::GetRendererInstance()->IASetInputLayout(0);
	...
	pEffectPass->Apply(0, IRenderer::GetRendererInstance());    


	//do the actual primitive rendering
	if((interpolateModel == MULTISTRAND || interpolateModel == MULTI_HYBRID) && renderType == SOATTRIBUTES) renderPointList(IRenderer::GetRendererInstance(), g_TessellatedMasterStrandLengthMax*numMStrands);
	else if(interpolateModel == SINGLESTRAND && g_useDX11) renderHairWithTessellation(IRenderer::GetRendererInstance(), interpolateModel, pEffectPass);
	else if(interpolateModel == SINGLESTRAND && renderType == SOATTRIBUTES && !g_useDX11) renderPointList(IRenderer::GetRendererInstance(), g_TessellatedMasterStrandLengthMax*numSStrands);
	else if(interpolateModel == MULTI_HYBRID && g_useDX11) renderHairWithTessellation(IRenderer::GetRendererInstance(), interpolateModel, pEffectPass);
	else if(interpolateModel == MULTI_HYBRID && !g_useDX11) renderLineStrip(IRenderer::GetRendererInstance(), g_TessellatedMasterStrandLengthMax * numMStrands);
	else if (interpolateModel == SINGLESTRAND) renderLineStrip(IRenderer::GetRendererInstance(), g_TessellatedMasterStrandLengthMax * numSStrands);
	else if(interpolateModel == MULTISTRAND && renderType == INSTANCED_INTERPOLATED_COLLISION && g_useDX11)	renderHairWithTessellation(IRenderer::GetRendererInstance(), interpolateModel, pEffectPass);
	else if(interpolateModel == MULTISTRAND && renderType == INSTANCED_INTERPOLATED_COLLISION && !g_useDX11)renderPointListInstanced(IRenderer::GetRendererInstance(),g_TessellatedMasterStrandLengthMax, numMStrands);
...
void renderHairWithTessellation(INTERPOLATE_MODEL interpolateModel, ID3DX11EffectPass* pEffectPass)
{
	if(interpolateModel == MULTISTRAND || interpolateModel == MULTI_HYBRID)
	{		
		IRenderer::GetRendererInstance()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_1_CONTROL_POINT_PATCHLIST);
...
int StepHairSimulation(ID3D11Device* IRenderer::GetRendererInstance())
{
    ...
    if(g_useCS)
		return stepHairSimulationDX11(IRenderer::GetRendererInstance(), frame);
	
    //hair simulation using Dx10 style GPGPU with Streamout
	return stepHairSimulationDX10(IRenderer::GetRendererInstance(), frame);
}
...
int stepHairSimulationDX10(int frame)
{
	...
	if(g_RestoreHairStrandsToDefault)
	{
		IRenderer::GetRendererInstance()->IASetInputLayout(g_MasterStrandIL);

		IRenderer::GetRendererInstance()->setVertexBuffer(0, 1, &g_MasterStrandVB[g_currentVB], &stride, &offset);
		IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_POINTS);
		g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("restoreToDefaultPositions")->Apply(0, IRenderer::GetRendererInstance());
    ...
    RepropagateCoordinateFrames(IRenderer::GetRendererInstance(), g_numCFIterations); 
	...
	if(g_addAngularForces)
	{
		...
		g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("addAngularForces")->Apply(0, IRenderer::GetRendererInstance());
		...
		IRenderer::GetRendererInstance()->setIndexBuffer(g_MasterStrandSimulation1IB, DXGI_FORMAT_R16_UINT, 0);
		...
		IRenderer::GetRendererInstance()->setVertexBuffer(0, 1, &g_ForceVB[g_currentForceVB], &stride, &offset);
		IRenderer::GetRendererInstance()->SOSetTargets(1, &g_ForceVB[(g_currentForceVB+1)%2], &offset);
		IRenderer::GetRendererInstance()->setIndexBuffer(g_MasterStrandSimulation2IB, DXGI_FORMAT_R16_UINT, 0);
		IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_LINES);
	...
	//add per vertex forces and integrate using verlet integration: this is not iterated; we only add forces once
	{
		...
		g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("addForcesAndIntegrate")->Apply(0, IRenderer::GetRendererInstance());

		IRenderer::GetRendererInstance()->IASetInputLayout(g_MasterStrandILAddForces);
    ...
    //iterate over all the constraints multiple times
    {
        for(int totalSimIterations=0;totalSimIterations<g_NumTotalConstraintIterations ;totalSimIterations++)
        {
			...
			if(g_useLinearStiffness)
			{
				...
				IRenderer::GetRendererInstance()->IASetInputLayout(g_MasterStrandSpringConstraintsIL);
				IRenderer::GetRendererInstance()->setVertexBuffer(0, 2, buffers, strides, offsets);
				IRenderer::GetRendererInstance()->SOSetTargets(1, &g_MasterStrandVB[(g_currentVB+1)%2], &offset);
				IRenderer::GetRendererInstance()->setIndexBuffer(g_MasterStrandSimulation1IB, DXGI_FORMAT_R16_UINT, 0);
				IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_LINES);
				g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfySpringConstraints")->Apply(0, IRenderer::GetRendererInstance());
				IRenderer::GetRendererInstance()->DrawIndexed(g_numIndicesFirst*2.0, 0, 0);
				IRenderer::GetRendererInstance()->SOSetTargets(0, 0, 0);
				g_currentVB = (g_currentVB+1)%2;
			...
			if(g_useAngularStiffness)
			{
				...
				IRenderer::GetRendererInstance()->IASetInputLayout(g_MasterStrandAngularSpringConstraintsIL);
				IRenderer::GetRendererInstance()->setVertexBuffer(0, 2, buffers, strides, offsets);
				IRenderer::GetRendererInstance()->setIndexBuffer(g_MasterStrandSimulationAngular1IB, DXGI_FORMAT_R16_UINT, 0);
				IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_LINES /* _ADJ */);
				IRenderer::GetRendererInstance()->SOSetTargets(1, &g_MasterStrandVB[(g_currentVB+1)%2], &offset);
				g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfyAngularSpringConstraints")->Apply(0, IRenderer::GetRendererInstance());
				...
				IRenderer::GetRendererInstance()->setVertexBuffer(0, 2, buffers, strides, offsets);
				IRenderer::GetRendererInstance()->SOSetTargets(1, &g_MasterStrandVB[(g_currentVB+1)%2], &offset);
				IRenderer::GetRendererInstance()->setIndexBuffer(g_MasterStrandSimulationAngular2IB, DXGI_FORMAT_R16_UINT, 0);
				IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_LINES /* _ADJ */);
				IRenderer::GetRendererInstance()->SOSetTargets(1, &g_MasterStrandVB[(g_currentVB+1)%2], &offset);
				g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfyAngularSpringConstraints")->Apply(0, IRenderer::GetRendererInstance());
			...
			if( g_collisions)
			{
				...
				IRenderer::GetRendererInstance()->SOSetTargets(1, &g_MasterStrandVB[(g_currentVB+1)%2], &offset);
				IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_POINTS);
				g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfyCollisionConstraints")->Apply(0, IRenderer::GetRendererInstance());
				IRenderer::GetRendererInstance()->Draw(g_NumMasterStrandControlVertices, 0);
				IRenderer::GetRendererInstance()->SOSetTargets(0, 0, 0);
				g_currentVB = (g_currentVB+1)%2;
...
void OnD3D11FrameRender(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), double fTime, float fElapsedTime, void* pUserContext)
{
	int hr;
	...
	if (g_Reset)
		resetScene(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
	...
	if (g_reloadEffect)
		reloadEffect(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
	...
	RenderArrow(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), vWindForce);

	//render arrow for the light
	float whiteColor[4] = {1,1,1,1};
	g_ArrowColorShaderVariable->SetFloatVector(whiteColor);
	RenderArrow(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), g_HairShadows.GetLightWorldDir());
	...
	{ 
		...
		IRenderer::GetRendererInstance()->IASetInputLayout(g_RenderMeshIL);
		g_UseScalpTextureShaderVariable->SetBool(false);
		g_BaseColorShaderVariable->SetFloatVector(skinColor);
		g_Effect->GetTechniqueByName("RenderMesh")->GetPassByName("RenderMeshDepthPass")->Apply(0, IRenderer::GetRendererInstance());
		g_FaceMesh->Render(IRenderer::GetRendererInstance());
		g_Scalp->Render(IRenderer::GetRendererInstance());
	}
	...
	if(g_RenderCollsionImplicits)
		for(int i=0;i<g_numSpheres;i++)
		{
			...
			g_Effect->GetTechniqueByName("RenderCollisionObjects")->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
			g_Sphere.Render(IRenderer::GetRendererInstance());
		}
    ...
    if(g_simulate)
	    StepHairSimulation(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
	...
	if( (g_simulate || g_firstMovedFrame) && g_InterpolateModel != NO_HAIR)
	{
		...
		if(g_useCS)
		{
		    ...
		    g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("TessellateSB")->Apply(0, IRenderer::GetRendererInstance());
		}
		else
		{
			g_Effect->GetVariableByName("g_MasterStrand")->AsShaderResource()->SetResource(g_MasterStrandRV[g_currentVB]);                   
 			g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("Tessellate")->Apply(0, IRenderer::GetRendererInstance());
		}

		IRenderer::GetRendererInstance()->IASetInputLayout(g_MasterStrandTesselatedInputIL);
	...
	if( (g_simulate || g_firstMovedFrame) && g_InterpolateModel != NO_HAIR)
	{
		...
		if(g_useCS)
		{
		    ...
		    g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("TessellateCoordinateFramesSB")->Apply(0, IRenderer::GetRendererInstance());
		}
		else
		{
		    g_Effect->GetVariableByName("g_coordinateFrames")->AsShaderResource()->SetResource(g_CoordinateFrameRV[g_currentVBCoordinateFrame]);
		    g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("TessellateCoordinateFrames")->Apply(0, IRenderer::GetRendererInstance());
		}
	...
	if( (g_simulate || g_firstMovedFrame) && g_InterpolateModel != NO_HAIR)
	{
		...
		g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("TessellateLengths")->Apply(0, IRenderer::GetRendererInstance());
		IRenderer::GetRendererInstance()->Draw(g_NumTessellatedMasterStrandVertices, 0);
		IRenderer::GetRendererInstance()->SOSetTargets(0, 0, 0);		
	}
	...
	if(g_firstMovedFrame || (g_simulate && (g_InterpolateModel==MULTI_HYBRID || g_InterpolateModel==HYBRID)))
	{
		...
		RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_INTERPOLATED_COLLISION, MULTISTRAND, g_totalMStrands, g_totalSStrands);
	...
	if(g_useSOInterpolatedAttributes && (g_firstMovedFrame || g_simulate))
	{
		...
		{
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), SOATTRIBUTES, MULTI_HYBRID, g_totalMStrands, g_totalSStrands); 
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), SOATTRIBUTES, SINGLESTRAND, g_totalMStrands, g_totalSStrands); 
		}
		else
		{
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), SOATTRIBUTES,g_InterpolateModel,g_totalMStrands, g_totalSStrands); 
		}
	}
	...
	if( g_renderShadows )
	{
		...
		IRenderer::GetRendererInstance()->IASetInputLayout(g_MasterStrandIL);
		g_HairShadows.BeginShadowMapRendering(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
		{
			if(g_InterpolateModel==HYBRID)
			{
				RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_DEPTH, MULTI_HYBRID, g_totalMStrands, g_totalSStrands); 
				RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_DEPTH, SINGLESTRAND, g_totalMStrands, g_totalSStrands); 
			}
			else
				RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_DEPTH, g_InterpolateModel,g_totalMStrands, g_totalSStrands); 
		}
		if(g_RenderScalp  && g_RenderScene) 
		{
			// unset hull and domain shaders
			g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("AllNulls")->Apply(0, IRenderer::GetRendererInstance());
			IRenderer::GetRendererInstance()->IASetInputLayout(g_RenderMeshIL);
			g_Effect->GetTechniqueByName("RenderMeshDepth")->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
			g_FaceMesh->Render(IRenderer::GetRendererInstance());
		}
		g_HairShadows.EndShadowMapRendering(IRenderer::GetRendererInstance());
	...
	if (g_VisShadowMap)
	{
		g_HairShadows.VisualizeShadowMap(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
		return;
	}
	...
	if(g_RenderFloor && g_RenderScene)
	{
		...
		DrawPlane(IRenderer::GetRendererInstance());
		(g_Effect->GetVariableByName("ViewProjection")->AsMatrix()->SetMatrix(transformViewProjection));
	}
	...
	if(g_RenderScalp && g_RenderScene)
	{
		...
		g_BaseColorShaderVariable->SetFloatVector(skinColor);
		g_HairShadows.SetHairShadowTexture();
		g_Effect->GetTechniqueByName("RenderMesh")->GetPassByName("RenderWithShadows")->Apply(0, IRenderer::GetRendererInstance());
		g_FaceMesh->Render(IRenderer::GetRendererInstance());
		g_HairShadows.UnSetHairShadowTexture();
		g_Effect->GetTechniqueByName("RenderMesh")->GetPassByName("RenderWithShadows")->Apply(0, IRenderer::GetRendererInstance());
		...
		g_HairShadows.SetHairShadowTexture();
		g_Effect->GetTechniqueByName("RenderMesh")->GetPassByName("RenderWithoutShadows")->Apply(0, IRenderer::GetRendererInstance());
		g_Scalp->Render(IRenderer::GetRendererInstance());
		g_HairShadows.UnSetHairShadowTexture();
		g_Effect->GetTechniqueByName("RenderMesh")->GetPassByName("RenderWithoutShadows")->Apply(0, IRenderer::GetRendererInstance());
	}
	...
	if(g_renderDensityOnHair)
	{
		if(g_InterpolateModel==HYBRID)
		{
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_DENSITY, MULTI_HYBRID, g_totalMStrands, g_totalSStrands); 
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_DENSITY, SINGLESTRAND, g_totalMStrands, g_totalSStrands); 
		}
		else
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_DENSITY,g_InterpolateModel,g_totalMStrands, g_totalSStrands); 
	}
	else
	{	
		if(g_InterpolateModel==HYBRID)
		{
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_NORMAL_HAIR, MULTI_HYBRID,g_totalMStrands, g_totalSStrands); 
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_NORMAL_HAIR,SINGLESTRAND,g_totalMStrands, g_totalSStrands);
		}
		else
		{
			RenderInterpolatedHair(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), INSTANCED_NORMAL_HAIR,g_InterpolateModel,g_totalMStrands, g_totalSStrands); 
		}
	...
	// apply all changes
	g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("InterpolateAndRenderM_HardwareTess")->Apply(0, IRenderer::GetRendererInstance());
	// unset hull and domain shaders
	g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("AllNulls")->Apply(0, IRenderer::GetRendererInstance());
	...
	IRenderer::GetRendererInstance()->ResolveSubresource(g_pSceneColorBuffer2X, 0, g_pSceneColorBuffer2XMS, 0, DXGI_FORMAT_R8G8B8A8_UNORM);
	...
	g_Effect->GetTechniqueByName("DrawTexture")->GetPassByName( "ResolveSuperSample" )->Apply(0, IRenderer::GetRendererInstance());       
	RenderScreenQuad(IRenderer::GetRendererInstance(),IRenderer::GetRendererInstance(),g_ScreenQuadBuffer);	   
	...
	if(g_bVisualizeCFs)
		VisualizeCoordinateFrames(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), g_bVisualizeTessellatedCFs);
...
void reloadEffect(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext *IRenderer::GetRendererInstance())
{
	g_reloadEffect = false;
	//SAFE_RELEASE(g_Effect);
	InitEffect(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
	...
	InitializeFluidState(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
}

int resetScene(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext *IRenderer::GetRendererInstance())
{
	int hr = 0;
...
#ifdef SUPERSAMPLE
    ReinitSuperSampleBuffers(IRenderer::GetRendererInstance());
#endif

	LoadMayaHair(g_loadDirectory, g_useShortHair );
	LoadCollisionObstacles(IRenderer::GetRendererInstance());
	CreateBuffers(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
...
void PlayTransforms()
{
    if(g_loopTransforms && g_transformsFileIn.eof())
	{
		...
		char RecordedTransformsFile[MAX_PATH];
...
int InitEffect(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance())
{
	int hr;
    ...
    if (FAILED(hr))
    {
		(NVUTFindDXSDKMediaFileCchT(g_pEffectPath, MAX_PATH, "Hair.fx"));
		(MyCreateEffectFromFile(g_pEffectPath, D3DXFROMWINESHADER_SKIPVALIDATION, IRenderer::GetRendererInstance(), &g_Effect));
    }
    else
    {
	    MyCreateEffectFromCompiledFile(g_pEffectPath, D3DXFROMWINESHADER_SKIPVALIDATION, IRenderer::GetRendererInstance(), &g_Effect);
    }

	g_pTechniqueRenderArrow  = g_Effect->GetTechniqueByName("RenderArrow");

	( LoadComputeShader(IRenderer::GetRendererInstance(), "HairSimulateCS.hlsl", "UpdateParticlesSimulate", "cs_4_0", &g_pCSSimulateParticles) );
	...
	//textures
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH, ("HairTextures\\specNoise.dds") ));
	(loadTextureFromFile(fullPath,"specularNoise",IRenderer::GetRendererInstance()));

	TextureID* hairTexture = 0;
	(LoadTextureArray(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), "HairTextures/HairSpecMap", 4 , &hairTexture, &hairTextureRV,1));
	g_hairTextureArrayShaderVariable->SetResource( hairTextureRV );
	...
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH,wfilename));
	(loadTextureFromFile(fullPath,&g_HairBaseRV[0],IRenderer::GetRendererInstance()));

	sprintf_s(filename,100,"%s\\%s",g_loadDirectory,"hairbaseBrown.dds");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH,wfilename));
	(loadTextureFromFile(fullPath,&g_HairBaseRV[1],IRenderer::GetRendererInstance()));

	sprintf_s(filename,100,"%s\\%s",g_loadDirectory,"hairbaseRed.dds");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH,wfilename));
	(loadTextureFromFile(fullPath,&g_HairBaseRV[2],IRenderer::GetRendererInstance()));

	sprintf_s(filename,100,"%s\\%s",g_loadDirectory,"meshAOMap.dds");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH,wfilename));
	(loadTextureFromFile(fullPath,"meshAOMap",IRenderer::GetRendererInstance()));

	sprintf_s(filename,100,"%s\\%s",g_loadDirectory,"densityThicknessMapBarycentric.dds");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH,wfilename));
	(loadTextureFromFile(fullPath,"densityThicknessMapBarycentric",IRenderer::GetRendererInstance()));

	sprintf_s(filename,100,"%s\\%s",g_loadDirectory,"densityThicknessMapClump.dds");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH, wfilename));
	(loadTextureFromFile(fullPath,"densityThicknessMapClump",IRenderer::GetRendererInstance()));
...
//--------------------------------------------------------------------------------------
int ReinitSuperSampleBuffers(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr(0);
	TextureID *pRTV = DXUTGetD3D11RenderTargetView();
	...
	CreateScreenQuadVB(IRenderer::GetRendererInstance());
	CreateSuperSampleScreenQuadVB(IRenderer::GetRendererInstance());
	...
	(IRenderer::GetRendererInstance()->CreateTexture2D(&descTex,0,&g_pSceneColorBuffer2XMS));

	//also create a non-MS texture to resolve to
	descTex.SampleDesc.Count = 1;
	descTex.SampleDesc.Quality = 0;
	descTex.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	(IRenderer::GetRendererInstance()->CreateTexture2D(&descTex,0,&g_pSceneColorBuffer2X));
	...
	( IRenderer::GetRendererInstance()->CreateRenderTargetView( g_pSceneColorBuffer2XMS, &descRTV, &g_pSceneRTV2X ) );
	...
	( IRenderer::GetRendererInstance()->CreateShaderResourceView( g_pSceneColorBuffer2X, &viewDesc, &g_pSceneColorBufferShaderResourceView) );
	...
	(IRenderer::GetRendererInstance()->CreateTexture2D(&descTex,0,&g_pSceneDepthBuffer2X));
...
//--------------------------------------------------------------------------------------
int InitializeFluidState(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance())
{
	int hr(0);
	...
	g_fluid = new Fluid(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
	if( !g_fluid ) 
...
//--------------------------------------------------------------------------------------
int OnD3D11CreateDevice(ID3D11Device* IRenderer::GetRendererInstance(), const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	int hr;
	...
	ID3D11DeviceContext* IRenderer::GetRendererInstance() = DXUTGetD3D11DeviceContext();
	(g_DialogResourceManager.OnD3D11CreateDevice(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance()));
	(g_D3DSettingsDlg.OnD3D11CreateDevice(IRenderer::GetRendererInstance()));
#if 0
	(D3DXFROMWINE11CreateFont(IRenderer::GetRendererInstance(), 15, 0, FW_BOLD, 1, FALSE, DEFAULT_CHARSET, 
		OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, 
		"Arial", &g_pFont10));
	(D3DXFROMWINE11CreateSprite(IRenderer::GetRendererInstance(), 512, &g_pSprite11));
#endif
	g_pTxtHelper = new CDXUTTextHelper( IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance(), &g_DialogResourceManager, 15 );
	if(!g_pTxtHelper)
		return E_OUTOFMEMORY;
	...
	(LoadScalp(IRenderer::GetRendererInstance()));
	LoadMayaHair(g_loadDirectory, g_useShortHair);
	LoadCollisionObstacles(IRenderer::GetRendererInstance());
	...
	(CreateResources(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance()));

	(CDXUTDirectionWidget::StaticOnD3D11CreateDevice(IRenderer::GetRendererInstance(), DXUTGetD3D11DeviceContext()));
	...
	g_HairGrid = new HairGrid(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance());
	if( !g_HairGrid ) 
		return E_OUTOFMEMORY;
	...
	unsigned int numElements = sizeof(layoutArrow)/sizeof(layoutArrow[0]);
	...
	char str[MAX_PATH];
	( NVUTFindDXSDKMediaFileCch( str, MAX_PATH, "arrow.sdkmesh" ) ); 
	( g_MeshArrow.Create( IRenderer::GetRendererInstance(), str) );

	//-------------------------------------------------------------------------------------------------
	(CreatePlane(IRenderer::GetRendererInstance(),GROUND_PLANE_RADIUS,-10.0f));

	g_HairShadows.OnD3D11CreateDevice(IRenderer::GetRendererInstance(), DXUTGetD3D11DeviceContext());
...
//--------------------------------------------------------------------------------------
int OnD3D11SwapChainResized(ID3D11Device* IRenderer::GetRendererInstance(), IDXGISwapChain *pSwapChain, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext)
{
	int hr = 0;

	( g_DialogResourceManager.OnD3D11ResizedSwapChain( IRenderer::GetRendererInstance(), pBackBufferSurfaceDesc ) );
	( g_D3DSettingsDlg.OnD3D11ResizedSwapChain( IRenderer::GetRendererInstance(), pBackBufferSurfaceDesc ) );
...
void RenderArrow(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), D3DXFROMWINEVECTOR3 pos)
{
	int hr;
	...
	g_pTechniqueRenderArrow->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
	g_MeshArrow.Render(IRenderer::GetRendererInstance());
}
...
{ 
	...
	RecreateCoordinateFrames(IRenderer::GetRendererInstance());

	unsigned int stride = 3*4*sizeof(float);     
	unsigned int offset = 0;
	IRenderer::GetRendererInstance()->IASetInputLayout(g_CoordinateFrameIL);
	(g_Effect->GetVariableByName("g_MasterStrand")->AsShaderResource()->SetResource(g_MasterStrandRV[g_currentVB]));
	(g_Effect->GetVariableByName("g_VertexNumber")->AsShaderResource()->SetResource(g_MasterStrandLocalVertexNumberRV));
	...
	{
		g_Effect->GetVariableByName("g_coordinateFrames")->AsShaderResource()->SetResource(g_CoordinateFrameRV[g_currentVBCoordinateFrame]);
		g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("PropagateCoordinateFrame")->Apply(0, IRenderer::GetRendererInstance());
...
void RecreateCoordinateFrames(ID3D11DeviceContext* IRenderer::GetRendererInstance())
{
	unsigned int stride = 3*4*sizeof(float);     
	unsigned int offset = 0;
	IRenderer::GetRendererInstance()->IASetInputLayout(g_CoordinateFrameIL);
	...
	g_Effect->GetVariableByName("g_MasterStrand")->AsShaderResource()->SetResource(g_MasterStrandRV[g_currentVB]);
	//g_Effect->GetVariableByName("g_coordinateFrames")->AsShaderResource()->SetResource(0);
	g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("UpdateCoordinateFrame")->Apply(0, IRenderer::GetRendererInstance());
...
void VisualizeCoordinateFrames(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), bool drawTessellatedCFs)
{
	unsigned int stride =  3*4*sizeof(float);        
	unsigned int offset = 0;
	IRenderer::GetRendererInstance()->IASetPrimitiveTopology(PRIM_POINTS);
	...
	{   
		//set variables
		g_Effect->GetVariableByName("g_TessellatedMasterStrand")->AsShaderResource()->SetResource(g_TessellatedMasterStrandRV);
		g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("RenderCoordinateFrameTessellated")->Apply(0, IRenderer::GetRendererInstance());
	...
	else    
	{
        if(g_useCS)
		{
		    g_Effect->GetVariableByName("g_MasterStrandSB")->AsShaderResource()->SetResource(g_MasterStrandUAB_SRV);
			g_Effect->GetVariableByName("g_coordinateFramesSB")->AsShaderResource()->SetResource(g_CoordinateFrameUAB_SRV);
		    g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("RenderCoordinateFrameUnTessellatedSB")->Apply(0, IRenderer::GetRendererInstance());
		}
		else
		{
		    g_Effect->GetVariableByName("g_MasterStrand")->AsShaderResource()->SetResource(g_MasterStrandRV[g_currentVB]);
		    g_Effect->GetVariableByName("g_coordinateFrames")->AsShaderResource()->SetResource(g_CoordinateFrameRV[g_currentVBCoordinateFrame]);
		    g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("RenderCoordinateFrameUnTessellated")->Apply(0, IRenderer::GetRendererInstance());
    	}
...
int renderInstancedInterpolatedHairForGrid(D3DXFROMWINEMATRIX& worldViewProjection,ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), float ZMin, float ZStep, int numRTs )
{
	int hr = 0;
	...
	g_Effect->GetVariableByName("g_MasterStrandLengthsUntessellated")->AsShaderResource()->SetResource(g_MasterLengthsRVUntessellated);
	g_Effect->GetVariableByName("g_MasterStrandRootIndicesUntessellated")->AsShaderResource()->SetResource(g_RootsIndicesRVUntessellated); 
	g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("InterpolateInstancedVSForGrid3DTextureMRT")->Apply(0, IRenderer::GetRendererInstance());
...
// Load scalp 
//--------------------------------------------------------------------------------------
int LoadScalp(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	...
	D3DXFROMWINE11_PASS_DESC passDesc;    
	(g_Effect->GetTechniqueByName("RenderMesh")->GetPassByIndex(0)->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements, sizeof(elements) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_RenderMeshIL));       
	...
	(g_Scalp->Create(IRenderer::GetRendererInstance(), xModelFile));

    //load the unit sphere to display the collision objects with
	(g_Sphere.Create(IRenderer::GetRendererInstance(), "unitSphere.sdkmesh"));

	sprintf_s(filename,100,"%s\\%s",g_loadDirectory,"Face.sdkmesh");
	mbstowcs_s(0, xModelFile, MAX_PATH, filename, MAX_PATH);
	g_FaceMesh = new MeshRenderer2();
	(g_FaceMesh->Create(IRenderer::GetRendererInstance(), xModelFile));
...
int LoadCollisionObstacles(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr = 0;


	char CollisionImplicitsFile[MAX_PATH];
	char wfilename[MAX_PATH];
	char collisionFile[MAX_PATH];
	sprintf_s(collisionFile,100,"%s\\%s",g_loadDirectory,"collision_objects.txt");
	mbstowcs_s(0, wfilename, MAX_PATH, collisionFile, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( CollisionImplicitsFile, MAX_PATH,wfilename));
...
// Create resources 
//--------------------------------------------------------------------------------------
int CreateResources(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext *IRenderer::GetRendererInstance())
{
	int hr;
	(CreateBuffers(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance()));
	(CreateInputLayouts(IRenderer::GetRendererInstance()));
	return 0;
}
...
// Create buffers 
//--------------------------------------------------------------------------------------
int CreateBuffers(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext *IRenderer::GetRendererInstance())
{

	int hr = 0;
	(CreateMasterStrandSimulationIBs(IRenderer::GetRendererInstance()));
	(CreateMasterStrandVB(IRenderer::GetRendererInstance()));
	(CreateForceVB(IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance()));
	(CreateTessellatedMasterStrandVB(IRenderer::GetRendererInstance()));
	(CreateTessellatedWispVB(IRenderer::GetRendererInstance()));
	(CreateMasterStrandTesselatedInputVB(IRenderer::GetRendererInstance()));
	(CreateStrandColors(IRenderer::GetRendererInstance()));
	(CreateStrandLengths(IRenderer::GetRendererInstance()));
	(CreateDeviantStrandsAndStrandSizes(IRenderer::GetRendererInstance()));
	(CreateCurlDeviations(IRenderer::GetRendererInstance()));
	(CreateStrandBarycentricCoordinates(IRenderer::GetRendererInstance()));
	(CreateRandomCircularCoordinates(IRenderer::GetRendererInstance()));
	(CreateStrandAttributes(IRenderer::GetRendererInstance()));
	(CreateMasterStrandSpringLengthVBs(IRenderer::GetRendererInstance()));
	(CreateCoordinateFrameVB(IRenderer::GetRendererInstance()));
	(CreateTessellatedCoordinateFrameVB(IRenderer::GetRendererInstance()));
	(CreateCollisionDetectionResources(IRenderer::GetRendererInstance()));
	(CreateCollisionScreenQuadVB(IRenderer::GetRendererInstance()));
	(CreateTangentJitter(IRenderer::GetRendererInstance()));
	(CreateStrandToBarycentricIndexMap(IRenderer::GetRendererInstance()));
	(CreateStrandToClumpIndexMap(IRenderer::GetRendererInstance()));
	(CreateInterpolatedValuesVBs(IRenderer::GetRendererInstance()));
    (CreateComputeShaderConstantBuffers(IRenderer::GetRendererInstance()));

	return 0;
}

int CreateMasterStrandSimulationIBs(ID3D11Device* IRenderer::GetRendererInstance())
{
...
int CreateMasterStrandSpringLengthVBs(ID3D11Device* IRenderer::GetRendererInstance())
{
...
int CreateMasterStrandTesselatedInputVB(ID3D11Device* IRenderer::GetRendererInstance())
{   
	D3D11_SUBRESOURCE_DATA initialData;
...
int CreateInterpolatedValuesVBs(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
...
int CreateMasterStrandVB(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	D3D11_BUFFER_DESC bufferDesc;
	...
	bufferDescUA.StructureByteStride  = sizeof(D3DXFROMWINEVECTOR4);
	initialData.pSysMem = g_MasterStrandControlVertices;
	//SAFE_RELEASE(g_MasterStrandUAB);
	//SAFE_RELEASE(g_MasterStrandPrevUAB);
	(IRenderer::GetRendererInstance()->CreateBuffer(&bufferDescUA, &initialData, &g_MasterStrandUAB));
	(IRenderer::GetRendererInstance()->CreateBuffer(&bufferDescUA, &initialData, &g_MasterStrandPrevUAB));	
	...
	sbSRVDesc.Buffer.ElementWidth           = sizeof(D3DXFROMWINEVECTOR4); //g_NumMasterStrandControlVertices
	...
	( IRenderer::GetRendererInstance()->CreateShaderResourceView(g_MasterStrandUAB, &sbSRVDesc, &g_MasterStrandUAB_SRV) );
	...
	( IRenderer::GetRendererInstance()->CreateUnorderedAccessView(g_MasterStrandUAB, &sbUAVDesc, &g_MasterStrandUAB_UAV) );
	( IRenderer::GetRendererInstance()->CreateUnorderedAccessView(g_MasterStrandPrevUAB, &sbUAVDesc, &g_MasterStrandPrevUAB_UAV) );
...
int CreateForceVB(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance())
{
	...
	char str[MAX_PATH];
	(NVUTFindDXSDKMediaFileCch(str, MAX_PATH, "LongHairResources/stiffnessMap.dds"));
	ID3D11Resource *pRes = 0;
	D3DXFROMWINE11_IMAGE_LOAD_INFO loadInfo;
	ZeroMemory( &loadInfo, sizeof( D3DXFROMWINE11_IMAGE_LOAD_INFO ) );
	...
	(D3DXFROMWINE11CreateTextureFromFile( IRenderer::GetRendererInstance(), str, &loadInfo, 0, &pRes, &hr ));
	if(!pRes )
		return 1;
...
int CreateCoordinateFrameVB(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	D3D11_BUFFER_DESC bufferDesc;
	...
	( IRenderer::GetRendererInstance()->CreateShaderResourceView(g_CoordinateFrameUAB, &desc, &g_CoordinateFrameUAB_SRV) );
	...
	( IRenderer::GetRendererInstance()->CreateUnorderedAccessView(g_CoordinateFrameUAB, &sbUAVDesc, &g_CoordinateFrameUAB_UAV) );

	return 0; 
}

int CreateTessellatedCoordinateFrameVB(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	D3D11_BUFFER_DESC bufferDesc;
...
int CreateCollisionDetectionResources(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr = 0;
	...
	TextureID* texture; 
	( IRenderer::GetRendererInstance()->CreateTexture2D(&TexDesc,0,&texture));
	...
	( IRenderer::GetRendererInstance()->CreateRenderTargetView( texture, &DescRT, &g_CollisionsRTV) );
	...
	(IRenderer::GetRendererInstance()->CreateShaderResourceView( texture, &SRVDesc, &g_CollisionsRV));

	//SAFE_RELEASE( texture );
	return hr;
}

int CreateTessellatedMasterStrandVB(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
...
int CreateTessellatedWispVB(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
...
int CreateComputeShaderConstantBuffers(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr = 0;
...
int CreateStrandColors(ID3D11Device* IRenderer::GetRendererInstance())
{
	D3D11_SUBRESOURCE_DATA initialData;
...
int CreateDeviantStrandsAndStrandSizes(ID3D11Device* IRenderer::GetRendererInstance())
{
	D3D11_SUBRESOURCE_DATA initialData;
...
int CreateCurlDeviations(ID3D11Device* IRenderer::GetRendererInstance())
{
	//curls---------------------------------------------------------------------------------------
	int hr;
...
int CreateStrandLengths(ID3D11Device* IRenderer::GetRendererInstance())
{
	D3D11_SUBRESOURCE_DATA initialData;
...
int CreateStrandBarycentricCoordinates(ID3D11Device* IRenderer::GetRendererInstance())
{
	D3D11_SUBRESOURCE_DATA initialData;
...
int CreateTangentJitter(ID3D11Device* IRenderer::GetRendererInstance())
{
	D3D11_SUBRESOURCE_DATA initialData;
...
int CreateStrandToClumpIndexMap(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr = 0;
	...
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCch( fullPath, MAX_PATH, wfilename));
	wcstombs_s(0,filename,MAX_PATH,fullPath,MAX_PATH);

	FILE * inTexture;
	errno_t error;
	if((error = fopen_s(&inTexture,filename,"r")) != 0)
	{
        MessageBoxA(0,"Could not find file clumpMapBarycentric.raw","Error",MB_OK);
		return 1;
	}
...
int CreateStrandToBarycentricIndexMap(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr = 0;
	...
	if((error = fopen_s(&inTexture,filename,"r")) != 0)
	{
        MessageBoxA(0,"Could not find file densityMapBarycentric.raw","Error",MB_OK);
		return 1;
	}
...
int CreateRandomCircularCoordinates(ID3D11Device* IRenderer::GetRendererInstance())
{
...
int CreateStrandAttributes(ID3D11Device* IRenderer::GetRendererInstance())
{
	D3D11_SUBRESOURCE_DATA initialData;
...
//--------------------------------------------------------------------------------------
int CreateInputLayouts(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	(CreateMasterStrandIL(IRenderer::GetRendererInstance()));
	(CreateMasterStrandTesselatedInputIL(IRenderer::GetRendererInstance()));
	return 0;
}

int CreateMasterStrandIL(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	...
	D3DXFROMWINE11_PASS_DESC passDesc2;    
	(g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("addForcesAndIntegrate")->GetDesc(&passDesc2));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements2, sizeof(elements2) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc2.pIAInputSignature, passDesc2.IAInputSignatureSize, &g_MasterStrandILAddForces));
	...
	D3DXFROMWINE11_PASS_DESC passDesc;    
	(g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfyCollisionConstraints")->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements, sizeof(elements) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_MasterStrandIL));
	...
	(g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfySpringConstraints")->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements3, sizeof(elements3) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_MasterStrandSpringConstraintsIL));
	...
	(g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("SatisfyAngularSpringConstraints")->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements4, sizeof(elements4) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_MasterStrandAngularSpringConstraintsIL));
	...
	(g_Effect->GetTechniqueByName("SimulateHair")->GetPassByName("PropagateCoordinateFrame")->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements6, sizeof(elements6) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_CoordinateFrameIL));
	...
	(g_Effect->GetTechniqueByName("DrawTexture")->GetPassByName("drawCollisionsTexture")->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements7, sizeof(elements7) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_screenQuadIL));


	return 0;
}

int CreateMasterStrandTesselatedInputIL(ID3D11Device* IRenderer::GetRendererInstance())
{
	int hr;
	...
	D3DXFROMWINE11_PASS_DESC passDesc;    
	(g_Effect->GetTechniqueByName("RenderHair")->GetPassByName("Tessellate")->GetDesc(&passDesc));
	(IRenderer::GetRendererInstance()->CreateInputLayout(elements, sizeof(elements) / sizeof(D3D11_INPUT_ELEMENT_DESC), passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &g_MasterStrandTesselatedInputIL));
...
int loadTextureFromFile(char file,char shaderTextureName, ID3D11Device* IRenderer::GetRendererInstance())
{
	...
	( D3DXFROMWINE11CreateTextureFromFile(IRenderer::GetRendererInstance(), str, &texLoadInfo, 0, &pRes, &hr ) );
	if( pRes )
	{
		TextureID* texture;
		TextureID* textureRview;
		stx_Variable* textureRVar;
...
//load a single texture and associated view
int loadTextureFromFile(char file,TextureID** textureRview, ID3D11Device* IRenderer::GetRendererInstance())
{
...
int CreateCollisionScreenQuadVB(ID3D11Device* IRenderer::GetRendererInstance())
{	
	int hr;
...
int CreateScreenQuadVB(ID3D11Device* IRenderer::GetRendererInstance())
{	
	int hr;
...
int CreateSuperSampleScreenQuadVB(ID3D11Device* IRenderer::GetRendererInstance())
{	
	int hr;
...
void RenderScreenQuad(ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), exBufferID* buffer)
{
	...
	IRenderer::GetRendererInstance()->IASetInputLayout( g_screenQuadIL );
	IRenderer::GetRendererInstance()->setVertexBuffer( 0, 1,  &buffer, stride, offset );
...
// of textures on disk.
//--------------------------------------------------------------------------------------
int LoadTextureArray( ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance(), char* sTexturePrefix, int iNumTextures, TextureID** ppTex2D, TextureID** ppSRV,int offset)
{
	int hr = 0;
	SURFACE_DESC desc;
	...
	{
		wsprintf(szTextureName, "%S%.2d.dds", sTexturePrefix, i+offset); 
		( NVUTFindDXSDKMediaFileCch( str, MAX_PATH, szTextureName ) );
		...
		loadInfo.Filter = D3DXFROMWINE10_FILTER_TRIANGLE;
		loadInfo.MipFilter = D3DXFROMWINE10_FILTER_TRIANGLE;
		...
		if( pRes )
		{
			...
			{
				IRenderer::GetRendererInstance()->Map(pTemp, iMip, D3D11_MAP_READ, 0, &mappedTex2D );
				...
				IRenderer::GetRendererInstance()->Unmap(pTemp, iMip);
			}
...
int CreatePlane( ID3D11Device* IRenderer::GetRendererInstance(), float radius, float height )
{
    int hr;
    ...
    D3DXFROMWINE11_PASS_DESC PassDesc;
    unsigned int numElements = sizeof(layoutPlane)/sizeof(layoutPlane[0]);
    g_pTechniqueRenderPlane->GetPassByIndex( 0 )->GetDesc( &PassDesc );
    ( IRenderer::GetRendererInstance()->CreateInputLayout( layoutPlane, numElements, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &g_pPlaneVertexLayout ) );

    return 0;
}

void DrawPlane(ID3D11DeviceContext *IRenderer::GetRendererInstance())
{
	g_HairShadows.SetHairShadowTexture();

    if(g_MeshWithShadows && g_renderShadows)
        g_pTechniqueRenderPlane->GetPassByName("RenderPlaneWithShadows")->Apply(0, IRenderer::GetRendererInstance());
    else
        g_pTechniqueRenderPlane->GetPassByName("RenderPlaneWithoutShadows")->Apply(0, IRenderer::GetRendererInstance());
    ...
    IRenderer::GetRendererInstance()->setIndexBuffer( g_pPlaneIndexBuffer, DXGI_FORMAT_R32_UINT, 0 );
    IRenderer::GetRendererInstance()->IASetPrimitiveTopology( PRIM_TRIANGLES );
    IRenderer::GetRendererInstance()->IASetInputLayout( g_pPlaneVertexLayout );
