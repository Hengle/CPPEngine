...
Fluid::Fluid( ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance() ) : m_pGrid(0), IRenderer::GetRendererInstance()(0), m_pEffect(0),
...
int Fluid::Initialize( int width, int height, int depth, FLUID_TYPE fluidType )
{
    ...
    try
    {
        m_pGrid = new Grid( IRenderer::GetRendererInstance(), IRenderer::GetRendererInstance() );
    }
    catch(...)
    {
        SAFE_DELETE(m_pGrid);
...
int Fluid::Draw( int field )
{
    ...
    (m_etDrawTexture->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance()));
...
void Fluid::Reset( void )
{
    ...
    if( m_eFluidType == FT_LIQUID )
    {
        ...
        m_etInitLevelSetToLiquidHeight->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
        m_pGrid->DrawSlices();
...
void Fluid::DrawObstacleTestBox( RENDER_TARGET dstObst, RENDER_TARGET dstObstVel )
{
    ...
    m_etDrawBox->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    m_pGrid->DrawSlices();
...
void Fluid::DrawObstacleBoundaryBox( RENDER_TARGET dstObst, RENDER_TARGET dstObstVel )
{
    ...
    m_etStaticObstacleTriangles->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    m_pGrid->DrawBoundaryQuads();
    
    m_etStaticObstacleLines->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::Advect(  float timestep, float decay, bool bAsLiquidVelocity, bool bAsTemperature, RENDER_TARGET dstPhi, RENDER_TARGET srcVel, RENDER_TARGET srcPhi, RENDER_TARGET srcObst, RENDER_TARGET srcLevelSet)
{
    ...
    m_etAdvect->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::AdvectMACCORMACK( float timestep, float decay, bool bAsLiquidVelocity, bool bAsTemperature, RENDER_TARGET dstPhi, RENDER_TARGET srcVel, RENDER_TARGET srcPhi, RENDER_TARGET srcPhi_hat, RENDER_TARGET srcObst, RENDER_TARGET srcLevelSet )
{
    ...
    m_evTreatAsLiquidVelocity->SetBool( bAsLiquidVelocity );
    m_evTexture_levelset->SetResource( m_pShaderResourceViews[srcLevelSet] );
    m_etAdvectMACCORMACK->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    SetRenderTarget( dstPhi );
...
void Fluid::Diffuse( float timestep, float viscosity, RENDER_TARGET dstPhi, RENDER_TARGET srcPhi, RENDER_TARGET tmpPhi, RENDER_TARGET srcObst )
{
    ...
    m_etDiffuse->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    SetRenderTarget( dst );
    m_pGrid->DrawSlices();
    IRenderer::GetRendererInstance()->OMSetRenderTargets(0, 0, 0);
...
void Fluid::RedistanceLevelSet( RENDER_TARGET dstLevelSet, RENDER_TARGET srcLevelSet, RENDER_TARGET tmpLevelSet )
{
    ...
    for( int iteration = 0; iteration < m_nRedistancingIterations; iteration++ )
    {
        ...
        m_etRedistance->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
        SetRenderTarget( dst );
...
void Fluid::ExtrapolateVelocity( RENDER_TARGET dstAndSrcVel, RENDER_TARGET tmpVel, RENDER_TARGET srcObst, RENDER_TARGET srcLeveSet )
{
    ...
    for( int iteration = 0; iteration < m_nVelExtrapolationIterations; iteration++ )
    {
        ...
        m_etExtrapolateVelocity->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
        SetRenderTarget( dst );
    ...
    m_evTexture_velocity->SetResource(0);
    m_evTreatAsLiquidVelocity->SetBool(false);
    m_etScalarJacobi->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    IRenderer::GetRendererInstance()->OMSetRenderTargets(0, 0, 0);
}
...
{
    m_evTexture_velocity->SetResource( m_pShaderResourceViews[srcVel] );
    m_etVorticity->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::ApplyVorticityConfinement( float timestep, RENDER_TARGET dstVel, RENDER_TARGET srcVorticity, RENDER_TARGET srcObst, RENDER_TARGET srcLevelSet )
{
    ...
    m_etConfinement->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::AddNewMatter( float timestep, RENDER_TARGET dstPhi, RENDER_TARGET srcObst )
{
    ...
    switch(m_eFluidType)
    {
        ...
        case FT_FIRE:
        {
            ...
            m_evTexture_obstacles->SetResource( m_pShaderResourceViews[srcObst] );
            m_etCopyTextureDensity->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());            
        ...
        case FT_SMOKE:
        {
            ...
            m_evTexture_obstacles->SetResource( m_pShaderResourceViews[srcObst] );
            m_etGaussian->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
        ...
        case FT_LIQUID:
        {
...
#if 0
            m_etInjectLiquid->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
            m_pGrid->DrawSlices();
#endif
            ...
            if(m_bLiquidStream) {
                ...
                m_evTexture_obstacles->SetResource( m_pShaderResourceViews[srcObst] );
                m_etLiquidStream_LevelSet->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::AddExternalForces( float timestep, RENDER_TARGET dstVel, RENDER_TARGET srcObst, RENDER_TARGET srcLevelSet, float impulseSize, float randCenterScale)
{
    ...
    {    
        case FT_FIRE:
        {
            ...
            m_evTexture_obstacles->SetResource( m_pShaderResourceViews[srcObst] );
            m_etAddDensityDerivativeVelocity->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
        ...
        case FT_LIQUID:
        {
            ...
            if(m_bLiquidStream) {
                ...
                m_evTexture_obstacles->SetResource( m_pShaderResourceViews[srcObst] );
                m_etLiquidStream_Velocity->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    ...
    if( m_bGravity )
    {
        ...
        m_evTexture_levelset->SetResource( m_pShaderResourceViews[srcLevelSet] );
        m_evTexture_obstacles->SetResource( m_pShaderResourceViews[srcObst] );
        m_etGravity->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::ComputeVelocityDivergence( RENDER_TARGET dstDivergence, RENDER_TARGET srcVel, RENDER_TARGET srcObst, RENDER_TARGET srcObstVel )
{
    ...
    m_etDivergence->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::ComputePressure( RENDER_TARGET dstAndSrcPressure, RENDER_TARGET tmpPressure, RENDER_TARGET srcDivergence, RENDER_TARGET srcObst, RENDER_TARGET srcLevelSet)
{
    ...
    for( int iteration = 0; iteration < m_nJacobiIterations/2.0; iteration++ )
    {
        ...
        if( (m_eFluidType == FT_LIQUID) && m_bTreatAsLiquidVelocity )
        {
            ...
            m_etAirPressure->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
...
void Fluid::ProjectVelocity( RENDER_TARGET dstVel, RENDER_TARGET srcPressure, RENDER_TARGET srcVel, RENDER_TARGET srcObst, RENDER_TARGET srcObstVel, RENDER_TARGET srcLevelSet )
{
    ...
    if( (m_eFluidType == FT_LIQUID) && m_bTreatAsLiquidVelocity )
        m_evTreatAsLiquidVelocity->SetBool( true );
    m_evTexture_levelset->SetResource( m_pShaderResourceViews[srcLevelSet] );
    m_etProject->GetPassByIndex(0)->Apply(0, IRenderer::GetRendererInstance());
    SetRenderTarget( dstVel );
...
int Fluid::LoadShaders( )
{
    ...
    (NVUTFindDXSDKMediaFileCchT( m_effectPath, MAX_PATH, "FluidSim.fx" ));
	(MyCreateEffectFromFile(m_effectPath, dwShaderFlags, IRenderer::GetRendererInstance(), &m_pEffect));
...
int Fluid::CreateTextureAndViews(int rtIndex, D3D11_TEXTURE3D_DESC TexDesc)
{
    ...
    // Create the texture
    ( IRenderer::GetRendererInstance()->CreateTexture3D(&TexDesc,0,&m_p3DTextures[rtIndex]));
    ...
    ( IRenderer::GetRendererInstance()->CreateRenderTargetView( m_p3DTextures[rtIndex], &DescRT, &m_pRenderTargetViews[rtIndex]) );
    ...
    (IRenderer::GetRendererInstance()->CreateShaderResourceView( m_p3DTextures[rtIndex], &SRVDesc, &m_pShaderResourceViews[rtIndex]));
