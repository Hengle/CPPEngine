...
bool LoadMayaHair( char* directory, bool b_shortHair )
{
	...
	char HairStrandsTextFile[MAX_PATH];
	sprintf_s(filename,100,"%s\\%s",directory,"hair_vertices.txt");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( HairStrandsTextFile, MAX_PATH,wfilename));

	char OBJScalpMeshFile[MAX_PATH];
	sprintf_s(filename,100,"%s\\%s",directory,"scalp_mesh.obj");
	mbstowcs_s(0, wfilename, MAX_PATH, filename, MAX_PATH);
	(NVUTFindDXSDKMediaFileCchT( OBJScalpMeshFile, MAX_PATH,wfilename));
	...
	for(int s=0;s<int(HairStrands.size());s++)
	{
		for(int i=0;i<int(HairStrands.at(s).size())-1;i++)
		{
			...
			{   
				...
				D3DXFROMWINEVec3Cross(&y,&x,&z);
				D3DXFROMWINEVec3Normalize(&y,&y);
			}
			else
			{
				...
				D3DXFROMWINEVec3Cross(&xRotationVector, &x, &prevX );
				float sinTheta = D3DXFROMWINEVec3Length(&xRotationVector);
				float theta = asin(sinTheta);
				D3DXFROMWINEVec3Normalize(&xRotationVector,&xRotationVector);
