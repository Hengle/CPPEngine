...
public:
    ...
    HairGrid (ID3D11Device* IRenderer::GetRendererInstance(), ID3D11DeviceContext* IRenderer::GetRendererInstance());
	virtual ~HairGrid ();
    int Initialize      ( int width, int height, int depth, EffectID* pEffect , char pEffectPath[MAX_PATH]);
	void Reset( void );
    int AddHairDensity(D3DXFROMWINEMATRIX& objToVolumeXForm, int (*DrawHair)(D3DXFROMWINEMATRIX&,ID3D11Device*,ID3D11DeviceContext*,float,float,int));
	void AddDensityBasedForces();
    int DisplayTexture(RENDER_TARGET rtIndex);
	int demultiplexTexture();
	int setFluidObstacleTexture( TextureID* texture, DXGI_FORMAT format, int texWidth, int texHeight, int texDepth );
    int demultiplexTextureTo3DFluidObstacles();
	int ReloadShader( EffectID* pEffect, char pEffectPath[MAX_PATH] );
...
protected:
    HairGrid                   ( void ) {};
	int LoadShaders        ( EffectID* pEffect,char pEffectPath[MAX_PATH]  );
    ...
    int CreateRTTextureAsShaderResource(RENDER_TARGET rtIndex, char shaderTextureName,EffectID* pEffect,D3D11_SHADER_RESOURCE_VIEW_DESC *SRVDesc );
    ...
    stx_Variable*  pShaderResourceVariables     [ NUM_RENDER_TARGET_TYPES ];
    DXGI_FORMAT                          RenderTargetFormats          [ NUM_RENDER_TARGET_TYPES ]; 

	ID3D11Device*              IRenderer::GetRendererInstance();
	ID3D11DeviceContext*       IRenderer::GetRendererInstance();
	EffectID*             m_pEffect;
    ...
    stx_Variable*          m_TextureWidthVar;
    stx_Variable*          m_TextureHeightVar;
    stx_Variable*          m_TextureDepthVar;
	stx_Variable*          m_RowWidth;
    stx_Variable*          m_ColWidth;  
	stx_Variable*          m_textureIndexShaderVariable; //which render target texture to render from
	stx_Variable*          m_GridZShaderVariable; //which slice of texture array to read from
    stx_Variable*          m_BlurDirectionShaderVariable;   
	stx_Variable*  m_TextureToBlurShaderShaderVariable;
};