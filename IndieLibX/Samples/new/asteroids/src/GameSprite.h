...
#include <Framework3/IRenderer.h>
	 ...
	 struct GameSprite
	{
		  ...
		  GameSprite(float x, float y, int width, int height, int angle=0)
		{
			...
			this->angle = angle;
			visible = true;
			collis_points = new std::vector(
			new D3DXFROMWINEVECTOR2(0, 0),
			new D3DXFROMWINEVECTOR2(width, 0),
			new D3DXFROMWINEVECTOR2(width, height),
			new D3DXFROMWINEVECTOR2(0, height)
			);
			rotate_offset = new D3DXFROMWINEVECTOR2(0, 0);
		}
		 void Render()
		{            
		}
		 void Update()
		{
			...
			{
				collis_points = [
					new D3DXFROMWINEVECTOR2(x, y),
					new D3DXFROMWINEVECTOR2(x+width, y),
					new D3DXFROMWINEVECTOR2(x+width, y+height),
					new D3DXFROMWINEVECTOR2(x, y+height)			
				];
			}
			...
			if (angle != 0)
			{
				//have to find the 4 corners of the collision box, and we'll subtract the rotate point so that the points will rotate around that instead of the top left/ origin
				 std points::vector = [
					new D3DXFROMWINEVECTOR2(0-rotate_offset.x, 0-rotate_offset.y),
					new D3DXFROMWINEVECTOR2(width-rotate_offset.x, 0-rotate_offset.y),
					new D3DXFROMWINEVECTOR2(width-rotate_offset.x, height-rotate_offset.y),
					new D3DXFROMWINEVECTOR2(0-rotate_offset.x, height-rotate_offset.y)			
				];
				 std points_trans::vector = new std::vector();
		...
		/*  CheckRectIntersect(GameSprite obj2):bool
		{
			...
			//first step is we get all the edges by using the formula: edge = v(n) - v(n-1)
			 std edges1::vector = new std::vector();
			for ( int i = 0; i < collis_points.length; i++)
			{
			 ...
			 std edges2::vector = new std::vector();
			...
			//rotated.x = -unrotated.y; rotated.y =  unrotated.x
			 std rotated1::vector = new std::vector();
			for ( int k; k < edges1.length; k++)
				rotated1.push( -edges1[k].y, edges1[k].x);
			
			 std rotated2::vector = new std::vector();
