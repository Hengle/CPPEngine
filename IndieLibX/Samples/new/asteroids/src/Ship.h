...
#include <Framework3/IRenderer.h>
	 ...
	 struct Ship
	{
		  ...
		  Ship(int x, int y, int width, int height)
		{
			...
			rotate_amount = 15;
			speed =  new D3DXFROMWINEVECTOR2(0, 0);
			rotate_offset = new D3DXFROMWINEVECTOR2(width / 2, height / 2);
			
            tex_ship=IRenderer::GetRendererInstance()->addImageLibTexture("/new/x/asteroids/ship.png");
		   ...
		   void Render()
		{
			if (!visible)
			 ...
			 D3DXFROMWINEMATRIX matrix = new D3DXFROMWINEMATRIX();
		   ...
		   void Update()
		{
			...
			collis_points = [new D3DXFROMWINEVECTOR2(x_top, y_top), new D3DXFROMWINEVECTOR2(x_b_left, y_b_left), new D3DXFROMWINEVECTOR2(x_b_right, y_b_right)];
		}
