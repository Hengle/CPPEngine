...
#include <Framework3/IRenderer.h>
	 ...
	 */
	 struct Asteroid extends GameSprite 
	{
		  ...
		  Asteroid(int x, int y, float size, float direction, int type)
		{
			...
			this->speed = new D3DXFROMWINEVECTOR2( -Speeds[size] * cosf(direction), -Speeds[size] * sinf(direction));
			...
			if (type == 0)
			{
                tex_asteroid1=IRenderer::GetRendererInstance()->addImageLibTexture("/new/x/asteroids/asteroid1.png");
			...
			{
                tex_asteroid2=IRenderer::GetRendererInstance()->addImageLibTexture("/new/x/asteroids/asteroid2.png");
