...
Voxelizer::Voxelizer(void) : m_pD3DDevice(NULL), m_pDstInOutTexture3D(NULL), m_pDstInOutTexRTView(NULL),
    ...
    m_pSkinnedMesh(NULL), m_timeStep(1.0f), m_velocityMultiplier(1.0f),
    m_pDSTex2D(NULL), m_pDSTex2DDSView(NULL), m_pDSTex2DSRView(NULL), 
    m_pVoxEffect(NULL), m_etNZ(NULL), m_etResolveWithPS(NULL), m_etGenVelocityWireframe(NULL),
    m_evWorldViewProjectionMatrix(NULL), m_evProjSpacePixDim(NULL), m_evGridDim(NULL), m_evRecTimeStep(NULL),
    m_evVelocityMultiplier(NULL), m_evSliceIdx(NULL), m_evSliceZ(NULL), m_evTexture_depthStencil(NULL),
...
HRESULT Voxelizer::InitShaders(void)
{
    ...
    V_RETURN(NVUTFindDXSDKMediaFileCchT( fullPath, MAX_PATH, L"Voxelizer.fx" ));
    V_RETURN(D3DX10CreateEffectFromFile(fullPath, g_pShadersMacros, NULL, "fx_4_0", NULL, 0, m_pD3DDevice, 
        NULL, NULL, &m_pVoxEffect, NULL, &hr ));
    ...
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evWorldViewProjectionMatrix, "WorldViewProjection", AsMatrix);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evProjSpacePixDim, "projSpacePixDim", AsVector);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evGridDim, "gridDim", AsVector);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evRecTimeStep, "recTimeStep", AsScalar);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evVelocityMultiplier, "velocityMultiplier", AsScalar);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evSliceIdx, "sliceIdx", AsScalar);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evSliceZ, "sliceZ", AsScalar);
    V_GET_VARIABLE_RET(m_pVoxEffect, fullPath, m_evTexture_depthStencil, "stencilbufferTex2D", AsShaderResource);
...
HRESULT Voxelizer::InitSlices(void)
{
    ...
    D3D10_PASS_DESC passDesc;
    V_RETURN(m_etResolveWithPS->GetPassByIndex(0)->GetDesc( &passDesc ));
...
HRESULT Voxelizer::StencilClipVolume(void)
{
    ...
    for( UINT z=0; z<m_depth; z++)
    {
        ...
        V_RETURN(RenderClippedMesh((float)z/m_depth - 0.5f, 100000.0f, m_etNZ));
    }
    ...
    V_RETURN(m_evTexture_depthStencil->SetResource(m_pDSTex2DSRView));
    V_RETURN(m_etResolveWithPS->GetPassByIndex(0)->Apply(0));
    ...
    V_RETURN(m_evTexture_depthStencil->SetResource(NULL));
...
HRESULT Voxelizer::VoxelizeVelocity(void)
{
    ...
    for( UINT z=0; z<m_depth; z++)
    {
        ...
        V_RETURN(RenderClippedMesh(-0.5, 0.5, m_etGenVelocityWireframe));
    }
...
HRESULT Voxelizer::RenderClippedMesh(float zNear, float zFar, ID3D10EffectTechnique *pTechnique)
{
    ...
    V_RETURN(m_evWorldViewProjectionMatrix->SetMatrix(reinterpret_cast<float*>(&worldViewProj)));
