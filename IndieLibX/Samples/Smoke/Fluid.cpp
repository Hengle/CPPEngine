...
Fluid::Fluid( ID3D10Device* pd3dDevice ) : m_pGrid(NULL), m_pD3DDevice(NULL), m_pEffect(NULL),
...
ID3D10ShaderResourceView* Fluid::getCurrentShaderResourceView()
{
...
void Fluid::Reset( void )
{
    ...
    m_t=0;
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_VELOCITY0], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_VELOCITY1], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_PRESSURE], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_SCALAR0], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_SCALAR1], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_OBSTACLES], obstacle_exterior );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_OBSTVELOCITY], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_TEMPSCALAR], zero );
    m_pD3DDevice->ClearRenderTargetView( m_pRenderTargetViews[RENDER_TARGET_TEMPVECTOR], zero );
...
HRESULT Fluid::LoadShaders( )
{
    ...
    //source for adding density, temperature and velocity
    V_RETURN(loadTextureFromFile(m_pD3DDevice, L"..\\..\\Media\\fireBase.dds", m_pEffect, m_effectPath, "Texture_inDensity"));
...
{
    m_pD3DDevice->OMSetRenderTargets( 1, &m_pRenderTargetViews[rtIndex] , optionalDSV ); 
}


HRESULT Fluid::CreateTextureAndViews(int rtIndex, D3D10_TEXTURE3D_DESC TexDesc)
{
