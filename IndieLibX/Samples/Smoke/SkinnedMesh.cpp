...
SkinnedMesh *SkinnedMesh::CreateD3D10FromColladaFile(LPCTSTR filename,
	...
	bool useExternalEffect, ID3D10Effect* pEffect, TCHAR pEffectPath[MAX_PATH] )
{
    ...
    if( !newSkinnedMesh->LoadFromColladaFile(filename) ||
		!newSkinnedMesh->InitializeForRendering(pd3dDevice, maxSoBufNum, useExternalEffect,pEffect,pEffectPath) )
    {
...
SkinnedMeshD3D10::SkinnedMeshD3D10() : m_pd3dDevice(NULL), m_pIndexBuffer(NULL),
    ...
    m_etSkinned(NULL), m_etRigid(NULL), m_pRigid_TC_Technique(NULL),
    m_etSkinnedSO(NULL), m_etRigidSO(NULL), m_pRigidSO_TC_Technique(NULL),
    m_etFromSO(NULL), m_evWorldMatrix(NULL),
    m_evViewProjectionMatrix(NULL), m_evBoneMatrices(NULL),
    m_evEyeVector(NULL), m_evTexture_diffuse(NULL), m_evTexture_specular(NULL), m_evTexture_normal(NULL), 
    m_evUseNormalMap(NULL), m_IsRenderingToSO(false), m_IsRenderingFromSO(false),
...
bool SkinnedMeshD3D10::InitializeForRendering(ID3D10Device* pd3dDevice, int maxSoBufNum, bool useExternalEffect, ID3D10Effect* pEffect, TCHAR pEffectPath[MAX_PATH] )
{
    HRESULT hr(S_OK);
    ...
    if(!useExternalEffect)
    {
        ...
        VRETFALSE(NVUTFindDXSDKMediaFileCchT(fullPath, MAX_PATH, L"SkinnedMesh.fx" ));
        VRETFALSE(D3DX10CreateEffectFromFile(fullPath, NULL, NULL, "fx_4_0", dwShaderFlags,
            0, pd3dDevice, NULL, NULL, &m_pEffect, NULL, &hr ));
        ...
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evWorldMatrix , "WorldMatrix", AsMatrix);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evViewProjectionMatrix, "ViewProjectionMatrix", AsMatrix);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evBoneMatrices, "Bones", AsMatrix);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evEyeVector, "EyePos", AsVector);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evTexture_diffuse, "sceneTextureDiffuse", AsShaderResource);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evTexture_specular, "sceneTextureSpecular", AsShaderResource);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evTexture_normal, "sceneTextureNormal", AsShaderResource);
        V_GET_VARIABLE_RETBOOL(m_pEffect, fullPath, m_evUseNormalMap, "useNormalMap", AsScalar);
    }
    else
    {
...
bool SkinnedMeshD3D10::reloadExternalEffect( ID3D10Effect* pEffect, TCHAR pEffectPath[MAX_PATH] )
{
    HRESULT hr(S_OK);
...
HRESULT SkinnedMeshD3D10::LoadTextureFromFile(ID3D10Device* pd3dDevice, LPCTSTR file, ID3D10ShaderResourceView** pOutTexSRV)
{
    HRESULT hr(S_OK);
    ...
    TCHAR str[MAX_PATH];
    V_RETURN(NVUTFindDXSDKMediaFileCchT(str, MAX_PATH, file));
...
bool SkinnedMeshD3D10::Render(float *pWorldMatrix, float *pViewMatrix, float *pProjectionMatrix,
							  ...
							  bool useAlternativeTechnique, ID3D10EffectTechnique* SkinnedAlternativeTechnique, ID3D10EffectTechnique* RigidAlternativeTechnique)
{
...
bool SkinnedMeshD3D10::RenderRootNodes(float *pWorldMatrix, bool useAlternativeTechnique, ID3D10EffectTechnique* SkinnedAlternativeTechnique, ID3D10EffectTechnique* RigidAlternativeTechnique)
{
...
bool SkinnedMeshD3D10::RenderNode(Node *pNode, float *pWorldMatrix, bool useAlternativeTechnique, ID3D10EffectTechnique* SkinnedAlternativeTechnique, ID3D10EffectTechnique* RigidAlternativeTechnique)
{
    HRESULT hr(S_OK);
    assert(pNode);
    ...
    if(!skipNodeRender)
    {
        ...
        for(; instIt != instEnd; ++instIt)
        {
            if(instIt->pMesh->hasSkin)
            {
                ...
                VRETFALSE(m_evBoneMatrices->SetMatrixArray((float*)&boneMatrices[0], 0, static_cast<UINT>(boneMatrices.size())));
