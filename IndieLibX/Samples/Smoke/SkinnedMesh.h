...
public:
    ...
    static SkinnedMesh *CreateD3D10FromColladaFile(LPCTSTR filename,
		...
		bool useExternalEffect = false, ID3D10Effect* pEffect = NULL, TCHAR pEffectPath[MAX_PATH]=NULL);
    ...
    virtual bool Render(float *pWorldMatrix, float *pViewMatrix, float *pProjectionMatrix,
		...
		bool useAlternativeTechnique = false, ID3D10EffectTechnique* SkinnedAlternativeTechnique = NULL, 
		ID3D10EffectTechnique* RigidAlternativeTechnique = NULL
		) = 0;
...
public:
    ...
    virtual bool Render(float *pWorldMatrix, float *pViewMatrix, float *pProjectionMatrix,
		...
		bool useAlternativeTechnique = false, ID3D10EffectTechnique* SkinnedAlternativeTechnique = NULL, ID3D10EffectTechnique* RigidAlternativeTechnique=NULL);
	...
	bool reloadExternalEffect( ID3D10Effect* pEffect, TCHAR pEffectPath[MAX_PATH] );
...
protected:
    ...
    ID3D10EffectTechnique       *m_etSkinned;
    ID3D10EffectTechnique       *m_etRigid;
    ID3D10EffectTechnique       *m_pRigid_TC_Technique;    
    ID3D10EffectTechnique       *m_etSkinnedSO;
    ID3D10EffectTechnique       *m_etRigidSO;
    ID3D10EffectTechnique       *m_pRigidSO_TC_Technique;
    
    ID3D10EffectTechnique       *m_etFromSO;

    ID3D10EffectMatrixVariable  *m_evWorldMatrix;
    ID3D10EffectMatrixVariable  *m_evViewProjectionMatrix;
    ID3D10EffectMatrixVariable  *m_evBoneMatrices;
    ID3D10EffectVectorVariable  *m_evEyeVector;

    ID3D10EffectShaderResourceVariable *m_evTexture_diffuse;
    ID3D10EffectShaderResourceVariable *m_evTexture_specular;
    ID3D10EffectShaderResourceVariable *m_evTexture_normal;
    
    ID3D10EffectScalarVariable  *m_evUseNormalMap;
	...
	virtual bool InitializeForRendering(ID3D10Device* pd3dDevice, int maxSoBufNum, bool useExternalEffect = false, ID3D10Effect* pEffect = NULL, TCHAR pEffectPath[MAX_PATH]=NULL );
    ...
    virtual bool RenderRootNodes(float *pWorldMatrix, bool useAlternativeTechnique =false, ID3D10EffectTechnique* SkinnedAlternativeTechnique=NULL, ID3D10EffectTechnique* RigidAlternativeTechnique=NULL);
    virtual bool RenderNode(Node *pNode, float *pWorldMatrix, bool useAlternativeTechnique, ID3D10EffectTechnique* SkinnedAlternativeTechnique, ID3D10EffectTechnique* RigidAlternativeTechnique);

public:
    static HRESULT LoadTextureFromFile(ID3D10Device* pd3dDevice, LPCTSTR file, ID3D10ShaderResourceView** pOutTexSRV);
