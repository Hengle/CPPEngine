...
parse(GrammarT const& g, char const* filename)
{
    ...
    if (!in)
    {
        printf("Could not open input file: " << filename << endl;
        return;
    }
    ...
    if (result.full)
        cerr << filename << " Parses OK\n";
    else {
        cerr << filename << " Fails Parsing\n";
        for (int i = 0; i < 50; i++)
        {
            if (result.stop == end)
                break;
    ...
    struct definition
    {
        ...
        // C keywords
            keywords =
                ...
                "goto", "if", "int", "int", "register", "return", "short",
...
// Start grammar definition
    printf("C Grammar checker implemented with Spirit ...\n");

std::string fn=std::string("../../IndieLib_resources")+std::string(filename[0]);
