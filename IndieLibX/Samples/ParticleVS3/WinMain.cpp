...
{
	IRenderer* r=IRenderer::GetRendererInstance("ParticleVS3");
...
bool DoInit()
{
  ...
  for(__DWORD__ i=0;i<50;i++) {
    ...
    {D3DXFROMWINEVECTOR3 v1((float)cos(rot)*dist,25.0f,(float)sin(rot)*dist);
    g_TreeEmitter.Add(Type++,
                      &v1,
                      50.0f, 0xFFFFFFFF, 0, &vO);}
    if(Type > PARTICLE_TREE6)
      Type = PARTICLE_TREE1;
...
void DoFrame()
{
  ...
  IRenderer::GetRendererInstance()->BeginScene();
	...
	D3DXFROMWINEMATRIX W=matView*g_matProj;
    ...
    g_ChopperEmitter.Render(matView, g_matProj);
    ...
    g_PeopleEmitter.Render(matView, g_matProj);
    g_TreeEmitter.Render(matView, g_matProj);
