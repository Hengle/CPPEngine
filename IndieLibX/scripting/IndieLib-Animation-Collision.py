...
class AppPy:
	...
	def render(self):
		...
		if IndieLib.CIndieLib.Instance().Input.OnKeyPress(IndieLib.KEY_p):
			bShowBoundsToggle = not bShowBoundsToggle

		if IndieLib.CIndieLib.Instance().Input.IsKeyPressed(IndieLib.KEY_LEFT):
			self.VelocityX -= speed * self.fElapsedTime
			if not self.playerJumping:
				Player.SetSequence(0)
			self.Player.SetMirrorX(1)

		else:
			if IndieLib.CIndieLib.Instance().Input.IsKeyPressed(IndieLib.KEY_RIGHT):
				self.VelocityX += speed * self.fElapsedTime
			...
			if IndieLib.CIndieLib.Instance().Input.OnKeyPress(IndieLib.KEY_SPACE) and not playerJumping:
			...
			Length = cmath.sqrt(self.VelocityX * self.VelocityX)
			...
			Dec = -Dir * slideFriction * self.fElapsedTime

			if IndieLib.CIndieLib.Instance().Entity2dManager.IsCollision (Player, "/IndieLib-Animation-Collision/entire", Beetle, "/IndieLib-Animation-Collision/beetle_boy_head"):
				VelocityY = 80
				VelocityX = 0

			if (x > (IndieLib.CIndieLib.Instance().Window.GetWidth() / 2)):
				Cameras2d.SetPosition(x, MiddleScreenY)
			...
			IndieLib.CIndieLib.Instance().Render.SetCamera2d (Cameras2d)
