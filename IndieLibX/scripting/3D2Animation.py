...
class AppPy:
	...
	def init(self):
		IndieLib.InitNSGUI("/GUILayout.xml")
		self.v=[]
		self.v.append("/trex dx/dino videogame.x")
		self.v.append("/trex dx/dino videogame2.x")
		...
		c=0
		for i in self.v:
			self.m_Mesh.append(IndieLib.FW3.GetInstance().GetMesh(self.v[c]))
	...
	def actionPerformed_(self):
		if(IndieLib.STX_Service.GetInputInstance().OnKeyPress(IndieLib.KEY_PAGEUP)):
			self.i=self.i+1
			if(self.i>(len(self.v)-1)):
				self.i=0
		elif(IndieLib.STX_Service.GetInputInstance().OnKeyPress(IndieLib.KEY_PAGEDOWN)):
			if(self.i==0):
				self.i=len(self.v)-1
			else:
				self.i=self.i-1
	...
	def render(self):
		...
		if (IndieLib.STX_Service.GetInputInstance().IsMouseButtonPressed (IndieLib.MBUTTON_LEFT)):
			self.mAngleX += self.mSpeedRotation * self.mDelta
		if (IndieLib.STX_Service.GetInputInstance().IsMouseButtonPressed (IndieLib.MBUTTON_RIGHT)):
...
if __name__=="__main__":
	app=AppPy()
	IndieLib.IRenderer.GetRendererInstance(640, 512, "3D2Animation")
	app.init()
