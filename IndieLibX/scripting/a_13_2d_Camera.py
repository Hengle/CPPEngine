...
class AppPy:
	...
	def init(self, width, height):
		...
		if not IndieLib.CIndieLib.Instance().SurfaceManager.Add (self.mSurfaceBug, "/Enemy Bug.png", IndieLib.IND_ALPHA, IndieLib.IND_32): 
			return 0
	...
	def render(self):
		...
		if IndieLib.CIndieLib.Instance().Input.IsKeyPressed (IndieLib.KEY_w):
			self.mCameraY -= self.mSpeedMoveCamera * self.mDelta
		if IndieLib.CIndieLib.Instance().Input.IsKeyPressed (IndieLib.KEY_s):
			self.mCameraY += self.mSpeedMoveCamera * self.mDelta
		if IndieLib.CIndieLib.Instance().Input.IsKeyPressed (IndieLib.KEY_a):
			self.mCameraX -= self.mSpeedMoveCamera * self.mDelta
		if IndieLib.CIndieLib.Instance().Input.IsKeyPressed (IndieLib.KEY_d):
			self.mCameraX += self.mSpeedMoveCamera * self.mDelta

		if IndieLib.CIndieLib.Instance().Input.OnMouseButtonPress	(IndieLib.MBUTTON_WHEELUP):
			self.mZoom += 0.1
		if IndieLib.CIndieLib.Instance().Input.OnMouseButtonPress	(IndieLib.MBUTTON_WHEELDOWN):
			self.mZoom -= 0.1

		if IndieLib.CIndieLib.Instance().Input.IsMouseButtonPressed (IndieLib.MBUTTON_LEFT):
			self.mCameraAngle += self.mSpeedRotation * self.mDelta
		if IndieLib.CIndieLib.Instance().Input.IsMouseButtonPressed (IndieLib.MBUTTON_RIGHT):
			self.mCameraAngle -= self.mSpeedRotation * self.mDelta
