...

{
  ...
  if(!xmlFile.is_open())
    ...
    {Logger::writeErrorLog(NSString("Cannot export AlphaAttributes to XML: file not ready."));return false;}



  if(function == -1.0f || reference == -1.0f)

    {Logger::writeErrorLog(NSString("Cannot export AlphaAttributes to XML: invalid attributes {-1.0}."));return false;}
...
*/
   ...
   Logger::writeErrorLog(NSString("Invalid Alpha function, AlphaAttributes::setFunction(int function)"));
...

{
  ...
  if(reference != referenceArg)
    ...
    Logger::writeErrorLog(NSString("Alpha reference clamped to fit in [0.0, 1.0]"));
