...
bool NVTexture::checkForRepeat(const char* string)
...
bool NVTexture::finalizeLoading(const char* string)
	    ...
	    (IRenderer::GetRendererInstance()->CreateTexture( desc.Width, desc.Height,1, D3DUSAGE_DYNAMIC, desc.Format, D3DPOOL_DEFAULT, &newTarget, 0));
...

			{
...

				{
					...
					memcpy(((BYTE *) pLockedRect2.pBits) + y * pLockedRect2.Pitch,((BYTE *) pLockedRect1.pBits) + y * pLockedRect1.Pitch, sizeof(pLockedRect2.Pitch));//???
...

{
...

  {
    ...
    LOG_PRINT("invalid texID, NVTexture::setID(unsigned int texID)\n");
...

{
  ...
  LOG_PRINT("Unsupported texture type\n");
...

{
  ...
  if(!xmlFile.is_open())
    ...
    {LOG_PRINT("Cannot export NVTexture to XML: file not ready.\n");return false;}



  if(!id)

    {LOG_PRINT("Cannot export NVTexture to XML: invalid texture handle.\n");return false;}
...

{
  ...
  aniso = clampNS(getClosestPowerOfTwo(level), 0, Renderer::getMaxAnisotropicFilter());
