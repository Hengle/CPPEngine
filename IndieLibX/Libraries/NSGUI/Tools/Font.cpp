...
{
  const char *fontlocation = //MediaPathManager::lookUpMediaPath(
	  fontPath;
  Image3 image;
  int   width  = 0,
        height = 0;
	...
	LOG_PRINT("fontPath:%s\n",fontPath);

  if(!image.loadImageLibImage(fontlocation,false))
    return false;//Logger::writeErrorLog("Can't load font file");

  if(getChannelCount(image.format) != 4)
    return false;//Logger::writeErrorLog("RGBA image required for font");
  ...
  //if(!
   ...
   //return false;//Logger::writeErrorLog("Can't create font texture");
...
/*
	...
	memcpy(stringdata,string.data,string.getLength());
