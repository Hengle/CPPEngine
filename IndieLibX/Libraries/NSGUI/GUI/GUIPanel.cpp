...
{
  const char *xmlFile = //MediaPathManager::lookUpMediaPath(
	  stackPath;
  XMLStack    cfgStack;
  bool        result = false;
  if(!xmlFile)
...
int GUIPanel::getWidgetIndexByCallbackString(const char *callbackString) const
{
  if(!callbackString)
    return 0;
...
/*
		...
		if(memcmp("Visibility",elements[t]->getCallbackString().data,10)==0)DBG_HALT;
*/
...
//	if(memcmp("Visibility",getCallbackString().data,10)==0)DBG_HALT;
	//if(memcmp("Visibility",getCallbackString().data,10)==0)
	//	visible=true;
  ...
  if(element)
  {
    for(size_t t = 0; t < elements.size(); t++)
      if(elements[t]->getCallbackString() == element->getCallbackString())
        return false;//Logger::writeErrorLog(NSString("Panel already has a child with CBS -> ")
