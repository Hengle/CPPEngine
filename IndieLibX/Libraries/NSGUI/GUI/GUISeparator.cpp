...
bool GUISeparator::loadXMLSettings(XMLElement *element)
{
  if(!element || (element->getName() != "Separator"))
    {LOG_PRINT("Need a <Separator> tag");return false;}
  ...
  if(child = element->getChildByName("ratio"))
   ...
   {LOG_PRINT("Need a <ratio> float attribute between 0 and 1");return false;}
...
void GUISeparator::render(float tick)
{
  ...
  IRenderer::GetRendererInstance()->Color3fv(((GUIPanel*)parent)->getBordersColor());
#if 1//???
