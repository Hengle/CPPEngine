...
bool GUIRectangle::loadXMLSettings(XMLElement *element)
{
  ...
  if((child = element->getChildByName("callbackString")) ||
    ...
    return false;//Logger::writeErrorLog(NSString("Need a <name> or <callbackString> attribute, check ")
                   //              + element->getName());
  ...
  if(mouseOver2)
{
  switch(getWidgetType())
  {
	...
	case CHECK_BOX:
		...
		break;
	case CHECK_BOX_MARK:
		LOG_PRINT("CHECK_BOX_MARK\n");
		break;
	case RADIO_BUTTON:
		LOG_PRINT("RADIO_BUTTON\n");
		break;
	case CHECK_RB_MARK:
		LOG_PRINT("CHECK_RB_MARK\n");
		break;
	case LABEL:
		LOG_PRINT("LABEL\n");
		break;
...
#if 0
  LOG_PRINT("windowBounds.x=%d\n", windowBounds.x);
  LOG_PRINT("windowBounds.y=%d\n", windowBounds.y);
  LOG_PRINT("windowBounds.z=%d\n", windowBounds.z);
  LOG_PRINT("windowBounds.w=%d\n", windowBounds.w);
  LOG_PRINT("mx=%d\n", mx);
  LOG_PRINT("my=%d\n", my);
#endif
}
