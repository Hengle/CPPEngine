...
struct PCTVertex {
  ...
  if(drawBackground)
  {
	...
	//IRenderer::GetRendererInstance()->setRasterizerState(IRenderer::GetRendererInstance()->GetcullFront());
		...
		if(PCTvertices)
		IRenderer::GetRendererInstance()->DrawPrimitiveUP(PRIM_TRIANGLE_FAN,
...
/*
  ...
  if(drawBounds)
  {
    IRenderer::GetRendererInstance()->setShaderConstant4f("colorRGBA", D3DXFROMWINEVECTOR4(bordersColor.x,bordersColor.y,bordersColor.z,1));
	...
	//IRenderer::GetRendererInstance()->setRasterizerState(IRenderer::GetRendererInstance()->GetcullFront());
		...
		if(PCTvertices)
		{
////LOG_PRINT("PRIM_LINE_STRIP=%d\n",IRenderer::GetRendererInstance()->getPrimitiveCountR(PRIM_LINE_STRIP,nvertices));
