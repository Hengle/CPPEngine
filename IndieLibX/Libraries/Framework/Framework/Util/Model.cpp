...
void Model::appendBatch(const void *verts, const unsigned int nVerts, const unsigned short *inds, const unsigned int nInds){
	...
	memcpy(vertices + nVertices * vertexSize, verts, nVerts * vertexSize);
...
void Model::appendFormat(const AttributeType attType, const unsigned int size){
	...
	for (unsigned int i = 0; i < nVertices; i++){
		memcpy(newVertices + i * newVertexSize, vertices + i * vertexSize, vertexSize);
		memset(newVertices + i * newVertexSize + vertexSize, 0, size * sizeof(float));
	}
...
void Model::addNormals(){
	...
	memset(newVertices, 0, nVertices * newVertexSize);
