...
struct stx_CModelViewerCamera
{
	...
	stx_CModelViewerCamera(const stx_CModelViewerCamera& rhs)
	{
        ...
        void SetViewParams( D3DXFROMWINEVECTOR4* aEye, D3DXFROMWINEVECTOR4* aAt );
    	void SetRadius(float x, float y, float z);
	void SetProjParams( float aFOV, float aAspectRatio, float aNear, float aFar );
	void FrameMove( float aTime );
	D3DXFROMWINEMATRIX GetWorldMatrix();
	D3DXFROMWINEMATRIX GetProjMatrix();
	D3DXFROMWINEMATRIX GetViewMatrix();
	D3DXFROMWINEVECTOR4 GetEyePt();
protected:
