...
#define SDLKtoKEY_MACRO(X) case SDLK_##X: return KEY_##X;
...
/*
    case MOUSE_MOTION:
    case MBUTTON_WHEELUP:
case MBUTTON_WHEELDOWN:
*/
};
	return false;
}
...
bool SDLInput::IsKeyPressed(eKey e,unsigned long)
{
...
bool SDLInput::IsMouseButtonPressed(eMouseButton e,unsigned long){return MouseButtonPressed(e);}
...
if((!(event.type==SDL_MOUSEBUTTONUP))&& 
 //(!event.type==SDL_BUTTON_WHEELUP)&&
 (!(event.type==SDL_MOUSEBUTTONDOWN))//&& 
 //(!event.type==SDL_BUTTON_WHEELDOWN)
 ) return false;

	Uint8 b=event.button.button;
	if((b==SDL_BUTTON_LEFT)&&(e==MBUTTON_LEFT)) return true;
	if((b==SDL_BUTTON_RIGHT)&&(e==MBUTTON_RIGHT)) return true;
	if((b==SDL_BUTTON_MIDDLE)&&(e==MBUTTON_MIDDLE)) return true;
	#if defined(SDL_1_2_15_BUILD)
	if((b==SDL_BUTTON_WHEELUP)&&(e==MBUTTON_WHEELUP)) return true;
	if((b==SDL_BUTTON_WHEELDOWN)&&(e==MBUTTON_WHEELDOWN)) return true;
	#endif
	return false;
}
