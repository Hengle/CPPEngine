...
//stx_Effect::
void stx_CModelViewerCamera::SetViewParams( D3DXFROMWINEVECTOR4* aEye, D3DXFROMWINEVECTOR4* aAt )
{
	D3DXFROMWINEVECTOR3 up(0.0f, 1.0f, 0.0f);
	D3DXFROMWINEVECTOR3 Eye3(aEye->x, aEye->y, aEye->z);
	D3DXFROMWINEVECTOR3 At3(aAt->x, aAt->y, aAt->z);
	D3DXFROMWINEMatrixLookAtLH(&m_mView, &Eye3, &At3, &up);
}
void stx_CModelViewerCamera::SetRadius(float x, float y, float z)
{
}
void stx_CModelViewerCamera::SetProjParams( float aFOV, float aAspectRatio, float aNear, float aFar )
{
	D3DXFROMWINEMatrixPerspectiveFovLH(&m_mProj, aFOV, aAspectRatio, aNear, aFar);
}
void stx_CModelViewerCamera::FrameMove( float aTime )
{
}
D3DXFROMWINEMATRIX stx_CModelViewerCamera::GetWorldMatrix()
{
	return m_mWorld;
}
D3DXFROMWINEMATRIX stx_CModelViewerCamera::GetProjMatrix()
{
	return m_mProj;
}
D3DXFROMWINEMATRIX stx_CModelViewerCamera::GetViewMatrix()
{
	return m_mView;
}
D3DXFROMWINEVECTOR4 stx_CModelViewerCamera::GetEyePt()
{
	return m_vEye;
}
