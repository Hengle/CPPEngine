...
public:
  ...
  {
    TextureOTL* texture = renderer->loadTexture(file);
  ...
  {
    TextureOTL* texture = renderer->createTexture(x, y);
    texture->setName(name);
    forest.addTexture(texture);
  }
                    ...
                    float from, float to, const char* texture)
  {
    ...
    TextureOTL* tex = forest.findTexture(texture);
                         ...
                         const char** source, int sources)
  {
    ...
    VertexList* vlist = forest.findVertexList(vertexlist);
    TextureOTL* texture = forest.findTexture(tex);
    TextureOTL* normal_texture = 0;
    TextureOTL* depth_texture = 0;
    ...
    {
      ShaderCG shader;
      shader.init("/otldata/shaders/normals.shd", "normals");
      ...
      nmap_shader->init("/otldata/shaders/parallax.shd", "parallax");
	nmap_shader->preRender(renderer);
