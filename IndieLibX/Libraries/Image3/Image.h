...
#define FORMAT_R16f     FORMAT_R16F
#define FORMAT_RG16f    FORMAT_RG16F
#define FORMAT_RGB16f   FORMAT_RGB16F
#define FORMAT_RGBA16f  FORMAT_RGBA16F
#define FORMAT_R32f     FORMAT_R32F
#define FORMAT_RG32f    FORMAT_RG32F
#define FORMAT_RGB32f   FORMAT_RGB32F
#define FORMAT_RGBA32f  FORMAT_RGBA32F
...
#define FORMAT_I32f    FORMAT_I32F
#define FORMAT_I16f    FORMAT_I16F
...
struct Image3 {
protected:
    	static std::map<std::string, unsigned int> m_Image3_filecache;
    	static std::vector<Image3*> m_Image3s;
	int m_id;
public:
	FORMAT format;
	static unsigned int find_from_Image3_cache(const char* fileName);
	static void insert_to_Image3_cache(const char* fileName, unsigned int id);
	Image3();
	Image3(const Image3 &img);
	~Image3();
	int IsImageLoaded ();	
	unsigned int getPitch(int ml) const ;
	void Dump();
	unsigned int getBpp(){return getBytesPerPixel(format);}
	void setPixels(ubyte * src);
	bool loadImageLibImage(const char *filename, const bool swapColors=false, const bool useMipMaps=false);
	bool saveImageLibImage(const char *fileName, const bool useMipMaps);

	ubyte *create(const FORMAT fmt, const int w, const int h, const int d, const int mipMapCount, const int arraysize = 1);
	void free();
	void clear();
