...
Image3::Image3(const Image3 &img){
	...
	extraData = new ubyte[nExtraData];
...
ubyte *Image3::create(const FORMAT fmt, const int w, const int h, const int d, const int mipMapCount, const int arraysize){
	...
	pixels = new ubyte[getMipMappedSize(0, nMipMaps) * arraySize];
...
bool Image3::loadHTEX(const char *fileName){
	...
	pixels = new ubyte[getMipMappedSize(0, nMipMaps)];
...
bool Image3::loadJPEG(const char *fileName){
	...
	pixels = new ubyte[width * height * cinfo.num_components];
	ubyte *curr_scanline = pixels;
...
bool Image3::loadTGA(const char *fileName){
	...
	case 8:
		if (palLength > 0){
			format = FORMAT_RGB8;
			dest = pixels = new ubyte[width * height * 3];
			for (y = 0; y < height; y++){
				for (x = 0; x < width; x++){
					tempPixel = 3 * (*src++);
			...
			format = FORMAT_I8;
			dest = pixels = new ubyte[width * height];
			for (y = 0; y < height; y++){
	...
	case 16:
		format = FORMAT_RGBA8;
		dest = pixels = new ubyte[width * height * 4];
		for (y = 0; y < height; y++){
			for (x = 0; x < width; x++){
...
bool Image3::loadBMP(const char *fileName){
	...
	//LOG_FNLN;
		format = FORMAT_RGB8;
		pixels = new ubyte[width * height * 3];
	//LOG_FNLN;
...
bool Image3::loadImage(const char *fileName, uint flags){
	//LOG_FNLN;
...
bool Image3::saveImage(const char *fileName){
	const char *extension = strrchr(fileName, '.');
...
bool Image3::removeMipMaps(const int firstMipMap, int mipMapsToSave){
...
bool Image3::unpackImage(){
	...
	if (format == FORMAT_RGBE8){
		...
		newPixels = new ubyte[getMipMappedSize(0, nMipMaps)];
...
bool Image3::swap(const int ch0, const int ch1){
	...
	if (format <= FORMAT_RGBA8){
		swapChannels((ubyte *) pixels, nPixels, nChannels, ch0, ch1);
