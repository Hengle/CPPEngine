...
AssimpView::MeshRenderer* FW3::GetMesh(const char* filename, const char* aShader="SimpleShader")
{
	MeshRenderer* mesh=new MeshRenderer(filename, aShader);
	return mesh;
}
...
void FW3_Animation::Sequence::render(D3DXFROMWINEVECTOR2 aposition, D3DXFROMWINEVECTOR2 asize)
{
	...
	DWORD dt=now-frames[m_index].m_then;
...
FW3_Animation::Sequence::FrameInSequence::FrameInSequence(const char* aname, DWORD atime)
{
	m_name=std::string(aname);
	m_time=atime;
	m_then=0;		
