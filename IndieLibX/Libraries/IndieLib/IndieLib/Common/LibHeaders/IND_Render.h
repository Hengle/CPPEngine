...
public:
	...
	bool	Init				(IND_Window *pWindow, tRenderer rtb=eGLSLGL_1_1, int argc=0, char *argv[]=0);
	bool	Init				(
	...
	*/
	...
	INLINE void BlitGrid				(/*int ab,*/IND_Surface *pSu, byte pR, byte pG, byte pB, byte pA);
...
private:
	...
	bool Direct3DInit				(int pWidth, int pHeight, int pBpp, bool pVsync, bool pFullscreen, tRenderer rtb=eGLSLGL_1_1, const char* path=".", int argc=0, char *argv[]=0);
	bool Direct3dReset							(int pWidth, int pHeight, int pBpp, bool pVsync, bool pFullscreen);

	bool CreateRender							(IND_Window *pWindow, tRenderer rtb=eGLSLGL_1_1
							   ,const char* path="."
...
//	void UpdateFrameMatrices					(const aiScene*frameBase, const D3DXFROMWINEMATRIX *parentMatrix);
	void DrawFrame								(IND_3dMesh *p3dMesh,aiScene* frame);
	void DrawMeshContainer						(IND_3dMesh *p3dMesh,aiScene* meshContainerBase,
		aiScene* frameBase);
#endif
#endif
	// ----- Primitives -----
	...
	INLINE void BlitGridLine					(/*int ab,*/int pPosX1, int pPosY1, int pPosX2, int pPosY2,  byte pR, byte pG, byte pB, byte pA, D3DXFROMWINEMATRIX pWorldMatrix);
	...
	INLINE void BlitCollisionCircle				(/*int ab,*/int pPosX, int pPosY, int pRadius, float pScale, byte pR, byte pG, byte pB, byte pA, IND_Matrix pWorldMatrix);
	INLINE void BlitCollisionLine				(/*int ab,*/int pPosX1, int pPosY1, int pPosX2, int pPosY2,  byte pR, byte pG, byte pB, byte pA, IND_Matrix pIndWorldMatrix);
	...
	INLINE WORD CullFrustumBox					(const D3DXFROMWINEVECTOR3 &pAABBMin, const D3DXFROMWINEVECTOR3 &pAABBMax);
