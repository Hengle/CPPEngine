...
{
	if(!p3dMesh->m3dMesh->mAnimator)
		return;

	if (pIndex == p3dMesh->m3dMesh->mAnimator->CurrentAnimIndex())
		return;

	if (pIndex >= (unsigned int) p3dMesh->m3dMesh->mAnimator->GetNumAnimations() )
		pIndex = 0;

	// Remember current animation
//	p3dMesh->m3dMesh.m_currentAnimationSet = pIndex;
	p3dMesh->m3dMesh->mAnimator->SetAnimIndex(pIndex);
...
void IND_Render::DrawFrame(IND_3dMesh *p3dMesh)
{
	...
	p3dMesh->m3dMesh->render();//(mInfo.mDevice,p3dMesh->m3dMesh->mAnimator->mRootNode)
#endif
}
...
void IND_Render::DrawMeshContainer(IND_3dMesh *p3dMesh)
{
	...
	p3dMesh->m3dMesh->render();//p3dMesh->DrawScene(mInfo.mDevice,p3dMesh->m3dMesh->mAnimator->mRootNode)
#endif
