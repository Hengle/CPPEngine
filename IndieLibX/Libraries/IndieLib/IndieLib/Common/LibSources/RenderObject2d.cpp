...
INLINE void IND_Render::BlitSurface (/*int ab,*/IND_Surface *pSu)
{
	// ----- Blitting -----
...
*/
INLINE void IND_Render::BlitGrid (/*int ab,*/IND_Surface *pSu, byte pR, byte pG, byte pB, byte pA)
{
	...
	for (int i = 0; i < pSu->GetNumBlocks() * 4; i += 4)
	{
		...
		if (CullFrustumBox (mP1_f3, mP2_f3))
		{
			BlitGridQuad	(/*ab,*/(int) pSu->GetVertexArray()[i].mX, (int) pSu->GetVertexArray()[i].mY,
...
*/
INLINE bool IND_Render::BlitRegionSurface (/*int ab,*/IND_Surface *pSu,
										  int pX,
										  int pY,
										  int pWidth,
										  int pHeight)
...
*/
INLINE bool IND_Render::BlitWrapSurface		(/*int ab,*/IND_Surface *pSu,
											int pWidth,
											int pHeight,
											float pUDisplace,
											float pVDisplace)
...
- IND_Entity2d::SetWrapDisplacement()
*/
INLINE int IND_Render::BlitAnimation (/*int ab,*/IND_Animation *pAn, int pSequence,
									  int pX, int pY,
...
*/
	...
	{
		BlitSurface (/*ab,*/pAn->GetActualSurface (pSequence));
	}
	else
	...
	if (!pToggleWrap)
	{
		if (pAn->GetActualSurface (pSequence)->GetNumTextures() > 1)
			return 0;
		BlitRegionSurface (/*ab,*/pAn->GetActualSurface (pSequence), pX, pY, pWidth, pHeight);
	}
	...
	else
	{
		if (pAn->GetActualSurface (pSequence)->GetNumTextures() > 1)
			return 0;
		BlitWrapSurface (/*ab,*/pAn->GetActualSurface (pSequence), pWidth, pHeight, pUDisplace, pVDisplace);
	}
