...
{
	Debug->Header ("Initializing Renderer", 5);
#if 0
	// Fill Info
...
void IND_Render::End()
{
	if (mOk)
	{
		Debug->Header ("Finalizing Renderer", 5);
		FreeVars ();
		Debug->Header ("Renderer finalized ", 6);
		mOk = false;
	}
}
...
{
	mInfo.mDevice=IRenderer::GetRendererInstance(pWidth, pHeight);
	return true;
}
							   ...
							   )
{
	...
	*/
	...
	Debug->Header ("Create Renderer!", 5);	

	if(!Init(pWidth,pHeight,argc,argv)) return 0;
#endif
	Debug->Header ("Create Renderer finished!", 5);	
...
bool IND_Render::Direct3dReset	(int pWidth,
								...
								bool pFullscreen)
{
	Debug->Header ("Reseting Renderer", 5);
#if 0
	// Windowed
	if (!pFullscreen)
	{
	...
	Debug->Header ("Renderer reseted", 6);
...
void IND_Render::WriteInfo ()
{
	...
	Debug->Header ("Version:" , 3);
	Debug->DataChar ((char*)mInfo.mVersion.c_str(), 1);

	// ----- Vendor -----

	Debug->Header ("Vendor:" , 3);
	Debug->DataChar ((char*)mInfo.mVendor.c_str(), 1);

	// ----- Renderer -----

	Debug->Header ("Renderer:" , 3);
	Debug->DataChar ((char*)mInfo.mRenderer.c_str(), 1);

	// ----- Antialiasing -----

	Debug->Header ("Primitive antialising:", 3);
