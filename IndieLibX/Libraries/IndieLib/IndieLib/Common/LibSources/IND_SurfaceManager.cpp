...
bool IND_SurfaceManager::Delete (IND_Surface *pSu)
{
	...
	Debug->Header	("Name:", 3);
	Debug->DataInt (long(&pSu->mSurface), 1);
...
*/
	...
	Debug->DataInt (long(&pNewSurface->mSurface), 1);
...
//LOG_PRINT("mPtrBlock=%d\n",mPtrBlock);
	...
	Debug->DataInt (mI0.mWidthImage, 0);
	Debug->DataChar ("x", 0);
	Debug->DataInt (mI0.mHeightImage, 1);

	Debug->Header ("Block size:", 3);
	Debug->DataInt (mI0.mWidthBlock, 0);
	Debug->DataChar ("x", 0);
	Debug->DataInt (mI0.mHeightBlock, 1);

	Debug->Header ("Number of blocks:", 3);
	Debug->DataInt (mI0.mBlocksX, 0);
	Debug->DataChar ("x", 0);
	Debug->DataInt (mI0.mBlocksY, 1);

	Debug->Header ("Spare (Right | Down):", 3);
	Debug->DataInt (mI0.mSpareX, 0);
	Debug->DataChar ("x", 0);
	Debug->DataInt (mI0.mSpareY, 1);

	Debug->Header ("Not used percentage:", 3);
	Debug->DataFloat (mI0.mNotUsedProportion, 0);
	Debug->DataChar ("%", 1);
...
*************************
*/
TextureID IND_SurfaceManager::CreateTexture	(byte *pImage,
														int pBlockWidth,
														int pBlockHeight,
														int pSrcBpp,
														FORMAT pSrcFormat,
...
void IND_SurfaceManager::FreeVars ()
{
		 ...
		 mSurfaceListIter++)
	{
		...
		Debug->DataInt (long(&(*mSurfaceListIter)->mSurface), 1);
#ifndef OMITDELETE
		// Free textures
