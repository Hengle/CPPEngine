...
CMudFontDlg::CMudFontDlg(CWnd* pParent /*=0*/): 
...
void CMudFontDlg::OnPaint() 
{
	...
	int width = destRect.Width() * zoom;
	int height = destRect.Height() * zoom;
...
void CMudFontDlg::UpdateTextureSize()
{
	for (int y=0; y<512; y++)
	{
		for (int x=0; x<512; x++)
		{
			...
			{
				m_Dib.SetPixel(x,y, RGB(255,0,255, 0));
			} else
			{
...
void CMudFontDlg::AddCharacter(char a_Char)
{
	int len = m_Chars.GetSize();
	for (int i=0; i<len; i++)
	{
...
int CALLBACK CMudFontDlg::EnumFontsProc(CONST LOGFONT* lplf, CONST TEXTMETRIC* lptm, __DWORD__ dwType, LPARAM lpData)
{
...
void CMudFontDlg::OnFileOpen() 
{
	...
	ofn.lpstrFile = szFile;
	ofn.nMaxFile = sizeof(szFile);
	ofn.lpstrFilter = "Font (*.fnt)\0*.fnt\0All (*.*)\0*.*\0";
	ofn.nFilterIndex = 1;
	ofn.lpstrFileTitle = 0;
	ofn.nMaxFileTitle = 0;
	ofn.lpstrDefExt = "fnt";
...
void CMudFontDlg::ExportTGA(const char* a_Filename)
{
	char path[_MAX_PATH];
	...
	char	AttrBits		= 0x20 | 8;		// first pixel at top-left, and 8 bits for alpha.
...
void CMudFontDlg::ExportXML(const char* a_Filename)
{
	char path[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
...
//	fprintf(file, "// Fichero de fuente\n\n");
//	fprintf(file, "{\n");
	...
	m_MemDC.SelectObject(m_DrawFont);
	int numChars = m_Chars.GetSize();
	//fprintf (file, ".NUMCHARS %d\n", numChars);
	//fprintf(file, "\n");

	fprintf(file, "<?xml version=\"1.0\" encoding=\"utf-8\" ?>\n");
	fprintf(file, "<font num_characters=\"%i\">\n", numChars);
	...
	for (int i=0; i<numChars; i++)
	{
		...
		/*
		fprintf(file, ".CHAR_ID %d\t.OFFSET_X %d\t.OFFSET_Y %d\t.WIDTH %d\t.HEIGHT %d\n",
			(unsigned char) ch.m_Char, ch.m_Pos.x, ch.m_Pos.y, ch.m_Size.cx, ch.m_Size.cy);
		*/
