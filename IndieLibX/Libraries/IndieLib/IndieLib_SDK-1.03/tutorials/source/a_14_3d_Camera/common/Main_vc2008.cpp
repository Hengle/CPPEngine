...
, "/trex dx/dino videogame.x", "/trex dx"
)) return 0;
#endif
	// Font
	//////////IND_Light mLight;
...
#ifdef BUILD_MESHRENDERER
		if (mI->Input->IsKeyPressed (KEY_LEFT))	mYaw -= mSpeedRotation * mDelta;
		if (mI->Input->IsKeyPressed (KEY_RIGHT))	mYaw += mSpeedRotation * mDelta;

		if (mI->Input->IsKeyPressed (KEY_UP))	mCamera3d.MoveWalk (0.01f);
		if (mI->Input->IsKeyPressed (KEY_DOWN))	mCamera3d.MoveWalk (-0.01f);
		if (mI->Input->IsKeyPressed (KEY_z))		mCamera3d.MoveStrafe (-0.01f);
		if (mI->Input->IsKeyPressed (KEY_x))		mCamera3d.MoveStrafe (0.01f);
