...
virtual int render()
{
	STX_TRY;
		...
		if (mI->Input->IsKeyPressed (KEY_RIGHT))	mPos += mSpeed * mDelta;

		// Move entities when pressing left
		if (mI->Input->IsKeyPressed (KEY_LEFT))	mPos -= mSpeed * mDelta;

		// If CTRL + X pressed then stx_exit
		if (mI->Input->IsKeyPressed (KEY_LCTRL) && mI->Input->IsKeyPressed (KEY_x))
		{
			mI->Render->EndScene ();
			//mI->End0???();
			stx_exit (0);
