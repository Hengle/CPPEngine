...
virtual int render()
{
	...
	while (!mI->Input->OnKeyPress (KEY_ESCAPE) && !mI->Input->Quit())
	{
		...
		if (mI->Input->OnKeyPress (KEY_1))
		{

			if (!mI->Render->Reset ("Alien BenchMark", 640, 480, 32, 0, 1)) return 0;
		...
		if (mI->Input->OnKeyPress (KEY_2))
		{

			if (!mI->Render->Reset ("Alien BenchMark", 800, 600, 32, 0, 1)) return 0;
		...
		if (mI->Input->OnKeyPress (KEY_3))
		{

			if (!mI->Render->Reset ("Alien BenchMark", 1024, 768, 32, 0, 1)) return 0;
		...
		// Camera angle
		if (mI->Input->IsMouseButtonPressed (MBUTTON_LEFT))		mAngle += mSpeedRotation * mDelta;

		if (mI->Input->IsMouseButtonPressed (MBUTTON_RIGHT))		mAngle -= mSpeedRotation * mDelta;
