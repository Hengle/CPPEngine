...
//CIndieLib * mI = CIndieLib::Instance();
	...
	// Loading bug
	////////////////////IND_Entity2d mBug;
	if (!mI->SurfaceManager->Add (&mSurfaceBug, "/Enemy Bug.png", IND_ALPHA, IND_32)) return 0;
...
virtual int render()
{
	STX_TRY;
		...
		// Camera translation
		if (mI->Input->IsKeyPressed (KEY_w))							mCameraY -= mSpeedMoveCamera * mDelta;
		if (mI->Input->IsKeyPressed (KEY_s))							mCameraY += mSpeedMoveCamera * mDelta;
		if (mI->Input->IsKeyPressed (KEY_a))							mCameraX -= mSpeedMoveCamera * mDelta;
		if (mI->Input->IsKeyPressed (KEY_d))							mCameraX += mSpeedMoveCamera * mDelta;

		// Camera Zoom in / out
		if (mI->Input->OnMouseButtonPress	(MBUTTON_WHEELUP))		mZoom += 0.1f;
		if (mI->Input->OnMouseButtonPress	(MBUTTON_WHEELDOWN))	mZoom -= 0.1f;

		// Camera angle
		if (mI->Input->IsMouseButtonPressed (MBUTTON_LEFT))			mCameraAngle += mSpeedRotation * mDelta;
		if (mI->Input->IsMouseButtonPressed (MBUTTON_RIGHT))		mCameraAngle -= mSpeedRotation * mDelta;
