...
virtual int render()
{
		...
		// Fire weapon!!
		//---------------------------
		if (mI->Input->IsMouseButtonPressed(MBUTTON_LEFT) && g_FireDelayTimer == 0.0f)
		{
		...
		// Toggle show bounds
		if (mI->Input->OnKeyPress(KEY_b))
			bToggleShowBounds = !bToggleShowBounds;

		// Toggle fullscreen
		if (mI->Input->OnKeyPress(KEY_F1))
		{
