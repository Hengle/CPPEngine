...
public:
	...
	virtual void RenderTexture(TextureID id, D3DXFROMWINEVECTOR2 aposition, D3DXFROMWINEVECTOR2 asize, D3DXFROMWINEVECTOR2 texposition=D3DXFROMWINEVECTOR2(0.0f, 0.0f), D3DXFROMWINEVECTOR2 texsize=D3DXFROMWINEVECTOR2(1.0f, 1.0f));
...
RendererHLSLCg::ib=-1;
   ...
   virtual void TexCoordPointer(const int w, /*GL_FLOAT,*/ const int off, const void* p)
   {
	...
	virtual void EndVrtl(TextureID id=-1);
  ...
  virtual void getExtents(FontID font,char *string, float x, float y, const float charWidth, const float charHeight, const float skewX = 0.0f, const float skewY = 0.0f,
	...
	virtual void drawLineVrtl(const float x0, const float y0, const float x1, const float y1, const D3DXFROMWINEVECTOR4 &color, const float lineWidth);
	virtual void drawRectVrtl(const float x0, const float y0, const float x1, const float y1, const D3DXFROMWINEVECTOR4 &color, const float lineWidth);
	virtual void drawRoundRectVrtl(const float x0, const float y0, const float x1, const float y1, const float r, const D3DXFROMWINEVECTOR4 &color, const float lineWidth);
	virtual void drawCircleVrtl(const float x, const float y, const float r, const D3DXFROMWINEVECTOR4 &color, const float lineWidth);
	virtual void drawTriangleVrtl(const float x0, const float y0, const float x1, const float y1, const float x2, const float y2, const D3DXFROMWINEVECTOR4 &color, const float lineWidth);
	virtual void drawQuadVrtl(const float x0, const float y0, const float x1, const float y1, const float s0, const float t0, const float s1, const float t1, const D3DXFROMWINEVECTOR4 &color);
...
/*
	...
	static VArray <AssimpView::MeshRenderer*> meshs;
...
public:
	...
	void DrawText(unsigned int x, unsigned int y, const char *pszFormat, ...);
	void DrawSmallText(unsigned int x, unsigned int y, const char *pszFormat, ...);
...
void drawPlain(const Primitives primitives, D3DXFROMWINEVECTOR2 *vertices, const uint nVertices, const BlendStateID blendState, const DepthStateID depthState, const D3DXFROMWINEVECTOR4 *color);
void drawTextureVrtl(TextureID texture, int x=0, int y=0, int sx=600, int sy=600, bool InScene=true, float tx=0.0f, float ty=0.0f, float stx=1.0f, float sty=1.0f);
...
unsigned int getVertexCount(const Primitives primitives, const int  NumPrim);
	...
	int getShaderParameterIndex(const ShaderID shader, const char *samplerName)
	{
		return getSamplerUnit(shader, samplerName);
	}
	int WorkDirectory(int awidth, int aheight, bool InitLogFile,
		int argc=0, char *argv[]=0);
	RendererHLSLCg(int awidth=800, int aheight=600,
...
inline IndexBufferID addIndexBuffer(const uint nIndices, const uint indexSize, const BufferAccess bufferAccess, const void *data = 0, IndexBufferID old=-1){return addIndexBufferVrtl(nIndices, indexSize, bufferAccess, data, old);}//, const IDataProcessor* p=0)  = 0;
		TextureID addTextureVrtl(int Width, int Height, bool MipLevels, FORMAT format, const SamplerStateID samplerState = SS_NONE, float3 alpha=float3(0.0f,0.0f,0.0f),const uint flags = 0);/*
		{
...
inline RasterizerStateID addRasterizerState(const int cullMode, const int fillMode = IRenderer::SOLID, const bool multiSample = true, const bool scissor = false){return addRasterizerStateVrtl(cullMode, fillMode, multiSample, scissor);}
    ...
    void DrawPrimitiveUP(Primitives PrimitiveType,
		UINT PrimitiveCount,
...
inline void Present(bool aB=false){PresentVrtl(aB);}
	...
	virtual void reset0(const uint flags = RESET_ALL);




    void drawTexturedVrtl(const Primitives primitives, TexVertex *vertices, const uint nVertices, const TextureID texture, const BlendStateID blendState, const DepthStateID depthState, const D3DXFROMWINEVECTOR4 *color = 0);
...
/*	void setSamplerState(const uint samplerUnit, const SamplerStateID samplerState){
	...
	void setBlendStateVrtl(const BlendStateID blendState, const uint sampleMask = ~0){
		//assert(blendState>=0);
	...
	void setDepthStateVrtl(const DepthStateID depthState, const uint stencilRef = 0){
		//assert(depthState>=0);
	...
	void setShaderConstantArray2f(const char *name, const D3DXFROMWINEVECTOR2  *constant, const uint count);
	void setShaderConstantArray3f(const char *name, const D3DXFROMWINEVECTOR3  *constant, const uint count);
	virtual void setShaderConstantArray4f(const char *name, const D3DXFROMWINEVECTOR4  *constant, const uint count);
	virtual void setShaderConstantArray4x4f(const char *name, const D3DXFROMWINEMATRIX *constant, const uint count);
...
inline void changeRenderTargets(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT = TEXTURE_NONE, const int depthSlice = NO_SLICE, const int *slices = 0){changeRenderTargetsVrtl(colorRTs, nRenderTargets, depthRT, depthSlice, slices);}
	...
	virtual TextureID addTexture(Image3* img, const bool useMipMaps=false, 
		const SamplerStateID samplerState = SS_NONE, uint flags = 0
	...
	//virtual TextureID addCubemapVrtl(const char **fileNames, const bool useMipMaps, const SamplerStateID samplerState, const int nArraySlices, uint flags);
	...
	TextureID addRenderTargetVrtl(const int width, const int height, const FORMAT format, const SamplerStateID samplerState = SS_NONE, uint flags = 0){
...
inline void changeDepthState(const DepthStateID depthState, const uint stencilRef = 0){changeDepthStateVrtl(depthState, stencilRef);}
    ...
    virtual void DrawFPSVrtl(int x=10, int y=10);
...
union {
	...
	ShaderID plainShader3;
	VertexFormatID plainVF3;
...
struct MESH {
	...
	DWORD GetNumFaces(){return m_dNumFaces;}
	DWORD GetNumVertices(){return m_dNumVertices;}
	//void GetVertexBuffer(VertexBufferID* id){*id=vid;}
	//void GetIndexBuffer(IndexBufferID* id){*id=iid;}
	void CreateMesh( DWORD nf, DWORD nv)
	{
		m_dNumFaces=nf;
		m_dNumVertices=nv;
...
int GetRegKey(CHAR* key, CHAR *fn, DWORD count);
...
void D3DXFROMWINECreateTextureFromFile(
	...
	TextureID* id,
	bool cm=true,
	SamplerStateID si=-1, float3 alpha=float3(0.0f,0.0f,0.0f));/*IRenderer::GetRendererInstance()->linear())
{
	*id=addTexture(
		n,
		cm,
