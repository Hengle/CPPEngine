...
struct IndexedMesh
{
	...
	std::vector<D3DXFROMWINEVECTOR3> m_colors3;
	...
	std::vector<stx_VertexPositionNormalTexture> g_vertices_;
	std::vector<stx_VertexPositionNormalTexture> g_verticesC_;
	...
	void init(std::string afn="", std::string afiletosave="", bool abUseLua=true)
	{
		...
		if((m_IndexedMesh.m_vertices3.size()==0)&&(g_verticesC_.size()==0))
			stx_exit(0);
		{
		shd = IRenderer::GetRendererInstance()->addShaderFromFile("/MeshRenderer2/ColorShader.HLSL.shd", "main", "main");
		FormatDesc format[] =
		{
...
void DrawVertex(IndexedMesh* aIndexedMesh, const D3DXFROMWINEMATRIX& aW, const D3DXFROMWINEVECTOR3& aC, const D3DXFROMWINEVECTOR2& aTC, const D3DXFROMWINEVECTOR3& aV_)
{
	...
	aIndexedMesh->m_colors3.push_back(aC);
	...
	if(m_bUseColor){
	stx_VertexPositionNormalTexture vx;
	//(aIndexedMesh->m_vertices3[aIndexedMesh->m_vertices3.size()-1], color, aIndexedMesh->m_texcoords2[aIndexedMesh->m_texcoords2.size()-1]);
	vx.Position=aV_;
	vx.Normal=aC;
	vx.Color=aC;
	vx.Tex=aTC;
	...
	stx_VertexPositionNormalTexture vx;
	//(aIndexedMesh->m_vertices3[aIndexedMesh->m_vertices3.size()-1], aIndexedMesh->m_texcoords2[aIndexedMesh->m_texcoords2.size()-1]);
	vx.Position=aV_;
	vx.Normal=aC;
	vx.Color=aC;
	vx.Tex=aTC;
		...
		const D3DXFROMWINEVECTOR2 aY=D3DXFROMWINEVECTOR2(-1.0f, 1.0f))
{
	...
	for(unsigned int i=iStart;i<iStop;i++)
	{
		...
		m.m_colors3.push_back(m_IndexedMesh.m_colors3[i]);
	...
	if(rp<0)
		return;
			...
			for(unsigned int j=i1;j<i2;j++)
			{
				...
				IndexedMesh_.m_colors3.push_back(m_IndexedMesh.m_colors3[j]);
	...
	while(i<v.size())
	{
		...
		else if(v[i]=="Vertex")
		{
			if(g_Terminal==eVertex_Terminal)
			{
				...
				w=stx_GetPivotMatrix3(vR, vS, vT);
				DrawVertex(&m_IndexedMesh, w, vC, vTC, vP_);
			}
			i+=6;
		}
		else
...
/*
	...
	LOG_PRINT("m_IndexedMesh.m_colors3.size()  =%d\n", m_IndexedMesh.m_colors3.size());
	...
	for (unsigned int i=0;i<m_Meshes.size();i++)
	{
		...
		LOG_PRINT("2:obj[%d].m_vertices3.size() =%d\n", i, m_Meshes[i].m_vertices3.size());
		LOG_PRINT("2:obj[%d].m_colors3.size()  =%d\n", i, m_Meshes[i].m_colors3.size());
		LOG_PRINT("2:obj[%d].m_texcoords2.size()=%d\n", i, m_Meshes[i].m_texcoords2.size());
		LOG_PRINT("2:obj[%d].m_indices1.size()  =%d\n", i, m_Meshes[i].m_indices1.size());
		LOG_PRINT("2:obj[%d].m_indices2.size()  =%d\n", i, m_Meshes[i].m_indices2.size());
    	}
	...
	for(unsigned int i=0;i<m_IndexedMesh.m_vertices3.size();i++)
	{
		...
		v.Normal=m_IndexedMesh.m_colors3[i];
	...
	stx_writeXFile(afiletosave.c_str(), mesh_);//, m_IndexedMesh.m_vertices3, m_IndexedMesh.m_colors3, m_IndexedMesh.m_texcoords2, m_IndexedMesh.m_indices1, texture);
	}
	#endif
	...
	#else
	mesh.Load(afiletosave.c_str(), eColorShader, true, true);
	#endif
	}
	return 0;
}
...
if(m_bUseLua){
	...
	D3DXFROMWINEVECTOR3 vC(1.0f, 0.0f, 0.0f);
	for(unsigned int i=0;i<vtex.size();i++)
	{
		...
		D3DXFROMWINEVECTOR3 vC(0.0f, 0.0f, 1.0f);
...
void render()
{
...
#ifndef _MSC_VER
	{
		...
		IRenderer::GetRendererInstance()->setTexture("Sampler", textures[texID]);
        	IRenderer::GetRendererInstance()->DrawIndexedPrimitiveUP(PRIM_TRIANGLES, 0, g_vertices_.size(), g_indices.size()/3, &g_indices[0], &g_indices[0], CONSTANT_INDEX2, &g_vertices_[0], &g_vertices_[0], sizeof(stx_VertexPositionNormalTexture));
	}
#endif
	#if 1
...
#else
void proceduralBuilding5_init(std::string afn="", std::string afiletosave="", bool abUseLua=true);
