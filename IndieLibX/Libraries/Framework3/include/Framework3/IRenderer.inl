...
inline ShaderID addHLSLShader(const char *shaderTextp, const char *vsName, const char *psName,  const char **attributeNames, const int nAttributes, const char *extra, const uint flags)
{
	return IRenderer::GetRendererInstance()->addHLSLShader(shaderTextp, vsName, psName, attributeNames, nAttributes, extra, flags);
}
inline IndexBufferID addIndexBuffer(const uint nIndices,  const uint indexSize,  const BufferAccess bufferAccess,  const void *data ,  IndexBufferID old)
{
...
inline TextureID addRenderDepth(const int width,  const int height,  const int arraySize,  const FORMAT format,  const int msaaSamples ,  const SamplerStateID samplerState ,  uint flags )
{
	return IRenderer::GetRendererInstance()->addRenderDepth(width, height, arraySize, format, msaaSamples, samplerState, flags);
}
inline TextureID addRenderTarget(const int width,  const int height,  const FORMAT format,  const SamplerStateID samplerState ,  uint flags )
{
	return IRenderer::GetRendererInstance()->addRenderTarget(width, height, format, samplerState, flags);
}
inline TextureID addRenderTarget(const int width,  const int height,  const int depth,  const int mipMapCount,  const int arraySize,  const FORMAT format,  const int msaaSamples ,  const SamplerStateID samplerState ,  uint flags )
{
...
inline SamplerStateID addSamplerState(const Filter filter,  const AddressMode s,  const AddressMode t,  const AddressMode r,  const float lod ,  const uint maxAniso ,  const int compareFunc ,  const float *border_color )
{
	return IRenderer::GetRendererInstance()->addSamplerState(filter, s, t, r, lod, maxAniso, compareFunc, border_color);
}
inline ShaderID addShaderFromFile(const char *fileName, const char *vsName, const char *psName,  const char **attributeNames, const int nAttributes, const char *extra, const uint flags)
{
...
inline TextureID addTexture(int Width,  int Height,  bool MipLevels,  FORMAT format,  const SamplerStateID samplerState ,  float3 alpha, const uint flags )
{
	return IRenderer::GetRendererInstance()->addTexture(Width, Height, MipLevels, format, samplerState, alpha, flags);
}
inline VertexBufferID addVertexBuffer(const long size,  const BufferAccess bufferAccess,  const void *data ,  VertexBufferID old)
{
	return IRenderer::GetRendererInstance()->addVertexBuffer(size, bufferAccess, data, old);
}
inline VertexFormatID addVertexFormat(const FormatDesc *formatDesc,  const uint nAttribs,  const ShaderID shader )
{
...
inline void changeBlendState(const BlendStateID blendState,  const uint sampleMask )
{
	IRenderer::GetRendererInstance()->changeBlendState(blendState, sampleMask);
}
inline void changeDepthState(const DepthStateID depthState,  const uint stencilRef )
{
...
inline void changeRenderTargets(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT, const int *faces)
{
	IRenderer::GetRendererInstance()->changeRenderTargets(colorRTs, nRenderTargets, depthRT, faces);
}
inline void changeRenderTargets(const TextureID *colorRTs,  const uint nRenderTargets,  const TextureID depthRT ,  const int depthSlice ,  const int *slices )
{
...
inline void drawPlain(const Primitives primitives, D3DXFROMWINEVECTOR2 *vertices, const uint nVertices, const BlendStateID blendState, const DepthStateID depthState, const D3DXFROMWINEVECTOR4 *color)
{
...
inline void drawTextured(const Primitives primitives,  TexVertex *vertices,  const uint nVertices,  const TextureID texture,  const SamplerStateID samplerState,  const BlendStateID blendState,  const DepthStateID depthState,  const D3DXFROMWINEVECTOR4 *color )
{
...
{
	return IRenderer::GetRendererInstance()->getSamplerUnit(shader, samplerName);
}
inline int getShaderParameterIndex(const ShaderID shader, const char *samplerName)
{
	return IRenderer::GetRendererInstance()->getShaderParameterIndex(shader, samplerName);
}
...
inline uint getTextQuads(const char *str)
{
...
inline void reset(const uint flags)
{
...
inline void setBlendState(const BlendStateID blendState,  const uint sampleMask )
{
	IRenderer::GetRendererInstance()->setBlendState(blendState, sampleMask);
}
inline void setDepthState(const DepthStateID depthState,  const uint stencilRef )
{
...
inline void setShaderConstantArray1f(const char *name, const float *constant, const uint count)
{
	IRenderer::GetRendererInstance()->setShaderConstantArray1f(name, constant, count);
}
inline void setShaderConstantArray2f(const char *name, const D3DXFROMWINEVECTOR2  *constant, const uint count)
{
	IRenderer::GetRendererInstance()->setShaderConstantArray2f(name, constant, count);
}
inline void setShaderConstantArray3f(const char *name, const D3DXFROMWINEVECTOR3  *constant, const uint count)
{
	IRenderer::GetRendererInstance()->setShaderConstantArray3f(name, constant, count);
}
inline void setShaderConstantArray4f(const char *name, const D3DXFROMWINEVECTOR4  *constant, const uint count)
{
	IRenderer::GetRendererInstance()->setShaderConstantArray4f(name, constant, count);
}
inline void setShaderConstantArray4x4f(const char *name, const D3DXFROMWINEMATRIX *constant, const uint count)
{
