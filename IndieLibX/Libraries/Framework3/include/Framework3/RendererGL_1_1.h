#if !defined(USE_GLES_SHADERS) && !defined(D3D11) && !defined(D3D10) && !defined(D3D9)
#ifndef __RendererGL_1_1_H__
#define __RendererGL_1_1_H__
...
struct RendererGL_1_1: public RendererHLSLCg
{
public:
#if 0
	union
...
void	EndPointSpriteRendering();
	...
	int getSamplerUnit(const ShaderID shader, const char *samplerName) const{return -1;}
	void applyTextures(){}
void applyConstants(){}
...
ShaderID addHLSLShader(const char *vsText0, const char *gsText, const char *fsText0,
		...
		const char *header, const char *extra, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags){return -1;}

	RendererGL_1_1(int awidth=800, int aheight=600,
		bool InitLogFile=false,
			bool fullScreen=false,
		int argc=0, char *argv[]=0);
	virtual ~RendererGL_1_1(){
	}
...
/*
        ...
        TextureID addRenderTargetVrtl(const int width, const int height, const int depth, const int mipMapCount, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags);
        TextureID addRenderDepthVrtl(const int width, const int height, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags);
...
//      void changeSamplerState(const uint samplerUnit, const SamplerStateID samplerState);
        void changeBlendStateVrtl(const BlendStateID blendState, const uint sampleMask);
        void changeDepthStateVrtl(const DepthStateID depthState, const uint stencilRef);
