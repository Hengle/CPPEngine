...
/////
/////////////////////////////////////// CROSS ///////////////////////////////////////*
float3 Cross(float3 vVector1, float3 vVector2)
{
	float3 vNormal;
...
/////
/////////////////////////////////////// MAGNITUDE ///////////////////////////////////////*
float Magnitude(float3 vNormal)
{
...
/////
/////////////////////////////////////// NORMALIZE ///////////////////////////////////////*
float3 Normalize(float3 vVector)
{
...
CCamera::CCamera()
{
	float3 vZero = float3(0.0, 0.0, 0.0);		// Init a vVector to 0 0 0 for our position
	float3 vView = float3(0.0, 1.0, 0.5);		// Init a starting view vVector (looking up and out the screen)
	float3 vUp   = float3(0.0, 0.0, 1.0);		// Init a standard up vVector (Rarely ever changes)
...
{
	float3 vPosition	= float3(positionX, positionY, positionZ);
	float3 vView		= float3(viewX, viewY, viewZ);
	float3 vUpVector	= float3(upVectorX, upVectorY, upVectorZ);
...
void CCamera::SetViewByMouse()
{
	...
	int middleX = IRenderer::GetRendererInstance()->GetViewportWidth()  >> 1;				// This is a binary shift to get half the width
	int middleY = IRenderer::GetRendererInstance()->GetViewportHeight() >> 1;				// This is a binary shift to get half the height
...
static float lastRotX = 0.0f;
	...
	if(currentRotX > 1.0f)
	{
		...
		if(lastRotX != 1.0f)
		{
			...
			float3 vAxis = Cross(m_vView - m_vPosition, m_vUpVector);
...
void CCamera::RotateView(float angle, float x, float y, float z)
{
	...
	float3 vView = m_vView - m_vPosition;
...
/////
//////////////////////////// CHECK FOR MOVEMENT ///////////////////////////////////////*
void CCamera::CheckForMovement(IInput* Input)
{
#if 1
	// Check if we hit the Up arrow or the 'w' key
	if(Input->OnKeyPress(KEY_PAGEUP)  || Input->OnKeyPress(KEY_W) ) {
		// Move our camera forward by a positive SPEED
		MoveCamera(kSpeed);
	}
	// Check if we hit the Down arrow or the 's' key
	if(Input->OnKeyPress(KEY_PAGEDOWN)  || Input->OnKeyPress(KEY_S) ) {
		// Move our camera backward by a negative SPEED
		MoveCamera(-kSpeed);
	}
	// Check if we hit the Left arrow or the 'a' key
	if(Input->OnKeyPress(KEY_LEFT) || Input->OnKeyPress(KEY_A)) {
		// Strafe the camera left
		StrafeCamera(-kSpeed);
	}
	// Check if we hit the Right arrow or the 'd' key
	if(Input->OnKeyPress(KEY_RIGHT) || Input->OnKeyPress(KEY_D)) {
...
void CCamera::Update()
{
	...
	float3 vCross = Cross(m_vView - m_vPosition, m_vUpVector);
...
void CCamera::Look()
{
  ...
  &D3DXFROMWINEVECTOR3(m_vPosition.x, m_vPosition.y, m_vPosition.z),
			  &D3DXFROMWINEVECTOR3(m_vView.x,	 m_vView.y,     m_vView.z),
			  &D3DXFROMWINEVECTOR3(m_vUpVector.x, m_vUpVector.y, m_vUpVector.z));
}
