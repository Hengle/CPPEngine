...
RendererHLSLD3D10::RendererHLSLD3D10(int awidth, int aheight,
		bool InitLogFile,
			bool fullScreen,
int argc, char *argv[]
) : mOk(false), RendererHLSLCg(awidth, aheight,InitLogFile,argc, argv),m_pd3dDevice10(0)
{
		HRESULT hr = E_FAIL;
#if 0
	...
	createDeviceFlags |= D3D10_CREATE_DEVICE_DEBUG;
#endif
	hr=D3D10CreateDeviceAndSwapChain(0, D3D10_DRIVER_TYPE_HARDWARE, 0, createDeviceFlags, D3D10_SDK_VERSION, &scDesc, &m_pSwapChain, &dev);
#endif
		if( FAILED( hr ) ) return;
		...
		dev->OMSetRenderTargets( 1, &RTV, DSV );
		D3D10_VIEWPORT vp;
		vp.Width = width;
		vp.Height = height;
		vp.MinDepth = 0.0f;
...
void RendererHLSLD3D10::resetVrtl(const uint flags){
	reset0(flags);
	if (flags & RESET_TEX){
		for (uint i = 0; i < MAX_TEXTUREUNIT; i++){
			selectedTexturesVS[i] = TEXTURE_NONE;
...
TextureID RendererHLSLD3D10::addTexture(ID3D10Resource *resource, uint flags){
	TextureD3D10 tex;
...
TextureID RendererHLSLD3D10::addTexture(Image3 &img, const SamplerStateID samplerState, uint flags){
	TextureD3D10 tex;
	memset(&tex, 0, sizeof(tex));
...
TextureID RendererHLSLD3D10::addRenderTargetVrtl(const int width, const int height, const int depth, const int mipMapCount, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags){
	TextureD3D10 tex;
	memset(&tex, 0, sizeof(tex));
	...
	if (depth == 1){
		...
		if (FAILED(device->CreateTexture2D(&desc, 0, (ID3D10Texture2D **) &tex.texture))){
			LOG_PRINT("Couldn't create render target");
			return TEXTURE_NONE;
		}
	} else {
		...
		if (FAILED(device->CreateTexture3D(&desc, 0, (ID3D10Texture3D **) &tex.texture))){
			LOG_PRINT("Couldn't create render target");
			return TEXTURE_NONE;
		}
	}
...
TextureID RendererHLSLD3D10::addRenderDepthVrtl(const int width, const int height, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags){
	TextureD3D10 tex;
	memset(&tex, 0, sizeof(tex));
...
/*		case D3D10_RESOURCE_DIMENSION_TEXTURE1D:
		...
		case D3D10_RESOURCE_DIMENSION_TEXTURE2D:
			...
			if (FAILED(device->CreateTexture2D(&desc2d, 0, (ID3D10Texture2D **) &textures[renderTarget].texture))){
				LOG_PRINT("Couldn't create render target");
				return false;
			}
			break;
...
#endif
ShaderID RendererHLSLD3D10::addHLSLShaderVrtl(const char *vsText, const char *gsText, const char *fsText
	...
	const char *header, const char *extra, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags)
	{
		...
		char buf[1024];
		ShaderD3D10  shader;
		memset(&shader, 0, sizeof(ShaderD3D10));

		ID3D10Blob *shaderBuf = 0;
		ID3D10Blob *errorsBuf = 0;
		ID3D10ShaderReflection *vsRefl = 0;
		ID3D10ShaderReflection *psRefl = 0;
		UINT compileFlags = D3D10_SHADER_PACK_MATRIX_ROW_MAJOR | D3D10_SHADER_ENABLE_BACKWARDS_COMPATIBILITY | D3D10_SHADER_DEBUG | D3D10_SHADER_SKIP_OPTIMIZATION;
		...
		if (fsText != 0)
		{
			...
			#ifdef USE_D3D10_1
			hr=D3DCompile(	shaderStringfs.c_str(), shaderStringfs.length(),
							"psfileName", 0, 0, mainps, pfl, compileFlags,
							0, &shaderBuf, &errorsBuf);
			if (SUCCEEDED(hr))
			{
				#else
				hr=D3D10CompileShader(shaderStringfs.c_str(), shaderStringfs.length(), "psfileName", 0, 0, mainps, pfl, compileFlags, &shaderBuf, &errorsBuf);
				if (SUCCEEDED(hr))
				{
					#endif
					if (SUCCEEDED(device->CreatePixelShader(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), &(shader).pixelShader)))
					{
						...
						{
							ID3D10Blob *disasm = 0;
							if (SUCCEEDED(D3D10DisassembleShader(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), FALSE, 0, &disasm)))
							{
								LOG_PRINT("%s", (const char *) disasm->GetBufferPointer());
							}
							SAFE_RELEASE(disasm);
						}
						#endif
			...
			if (vsText != 0)
			{
					...
					hr=D3D10CompileShader(shaderStringvs.c_str(), shaderStringvs.length(), "vsfileName", 0, 0, mainvs, vfl,
					compileFlags, &shaderBuf, &errorsBuf);
					if (SUCCEEDED(hr))
					{
					...
					D3D10_SHADER_DESC vsDesc, psDesc;
					if (vsRefl)
					{
					...
					for (uint i = 0; i < (shader).nVSCBuffers; i++)
					{
						...
						int limit=std::min<unsigned int>(sbDesc.Variables,20);
						...
						for (uint k = 0; k < limit; k++)
						{
                            ...
                            stx_snprintf(buf,1024,"n:%s",vDesc.Name);
							OutputDebugStringA(buf);
                            stx_snprintf(buf,1024,"s:%d",vDesc.Size);
							OutputDebugStringA(buf);
                            stx_snprintf(buf,1024,"o:%d",vDesc.StartOffset);
							OutputDebugStringA(buf);
	...
	//shader.constants = new ConstantD3D10[shader.nConstants];
	//memcpy(shader.constants, constants.getArray(), shader.nConstants * sizeof(ConstantD3D10));
	...
	std::sort((shader).constants.begin(), (shader).constants.end(), constantCompD3D10std);
					...
					if (maxResources)
					{
						//shader.textures = (SamplerD3D10 *) malloc(maxResources * sizeof(Sampler));
						//shader.samplers = (SamplerD3D10 *) malloc(maxResources * sizeof(Sampler));
						shader.nTextures = 0;
						shader.nSamplers = 0;
	...
	//shader.textures = (SamplerD3D10 *) realloc(shader.textures, shader.nTextures * sizeof(SamplerD3D10));
	//shader.samplers = (SamplerD3D10 *) realloc(shader.samplers, shader.nSamplers * sizeof(SamplerD3D10));
#if 1
...
VertexFormatID RendererHLSLD3D10::addVertexFormatVrtl(const FormatDesc *formatDesc, const uint nAttribs, const ShaderID shader){
...
VertexBufferID RendererHLSLD3D10::addVertexBufferVrtl(const long size, const BufferAccess bufferAccess, const void *data, VertexBufferID old){
...
IndexBufferID RendererHLSLD3D10::addIndexBufferVrtl(const uint nIndices, const uint indexSize, const BufferAccess bufferAccess, const void *data, IndexBufferID old){
	IndexBufferD3D10 ib;
...
SamplerStateID RendererHLSLD3D10::addSamplerStateVrtl(const Filter filter, const AddressMode s, const AddressMode t, const AddressMode r, const float lod, const uint maxAniso, const int compareFunc, const float *border_color){
...
void RendererHLSLD3D10::setTexture(const char *textureName, const TextureID texture){
...
void RendererHLSLD3D10::setTexture(const char *textureName, const TextureID texture, const SamplerStateID samplerState){
	ASSERT(selectedShader != SHADER_NONE);
...
void RendererHLSLD3D10::applyConstants(){
	if (currentShader != SHADER_NONE){
		...
		for (uint i = 0; i < shader->nVSCBuffers; i++){
			if (shader->vsDirty[i]){
				device->UpdateSubresource(shader->vsConstants[i], 0, 0, shader->vsConstMem[i], 0, 0);
				shader->vsDirty[i] = false;
			}
		}
		/*
		for (uint i = 0; i < shader->nGSCBuffers; i++){
			if (shader->gsDirty[i]){
				device->UpdateSubresource(shader->gsConstants[i], 0, 0, shader->gsConstMem[i], 0, 0);
		...
		*/
		for (uint i = 0; i < shader->nPSCBuffers; i++){
			if (shader->psDirty[i]){
				device->UpdateSubresource(shader->psConstants[i], 0, 0, shader->psConstMem[i], 0, 0);
				shader->psDirty[i] = false;
			}
		}
	}
...
void RendererHLSLD3D10::changeRenderTargetsVrtl(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT, const int depthSlice, const int *slices){
	...
	D3D10_VIEWPORT vp0;
	UINT nvp0;
	device->RSGetViewports(&nvp0, &vp0);
...
void RendererHLSLD3D10::changeShader(const ShaderID shaderID){
	if (shaderID != currentShader){
		if (shaderID == SHADER_NONE){
			device->VSSetShader(0);
			//device->GSSetShader(0);
			device->PSSetShader(0);
		} else {
...
void RendererHLSLD3D10::changeIndexBuffer(const IndexBufferID indexBufferID){
	if (indexBufferID != currentIndexBuffer){
		if (indexBufferID == IB_NONE){
			device->IASetIndexBuffer(0, DXGI_FORMAT_UNKNOWN, 0);
		} else {
...
void RendererHLSLD3D10::changeBlendStateVrtl(const BlendStateID blendState, const uint sampleMask){
	if (blendState != currentBlendState || sampleMask != currentSampleMask){
		if (blendState == BS_NONE){
			device->OMSetBlendState(0, D3DXFROMWINEVECTOR4(0, 0, 0, 0), sampleMask);
		} else {
			device->OMSetBlendState(blendStates[blendState].blendState, D3DXFROMWINEVECTOR4(0, 0, 0, 0), sampleMask);
		}
...
void RendererHLSLD3D10::changeDepthStateVrtl(const DepthStateID depthState, const uint stencilRef){
	if (depthState != currentDepthState || stencilRef != currentStencilRef){
		if (depthState == DS_NONE){
			device->OMSetDepthStencilState(0, stencilRef);
		} else {
...
void RendererHLSLD3D10::clear(const bool clearColor, const bool clearDepth, const bool clearStencil, const float *color, const float depth, const uint stencil){
	if (clearColor){
...
void RendererHLSLD3D10::drawArrays(const Primitives primitives, const int firstVertex, const int nVertices){
...
void RendererHLSLD3D10::drawElements(const Primitives primitives, const int firstIndex, const int nIndices, const int firstVertex, const int nVertices){
	if(D3D10_PRIMITIVE_TOPOLOGY_UNDEFINED==d3dPrim[primitives]) return;
	device->IASetPrimitiveTopology(d3dPrim[primitives]);
...
#ifdef USE_D3D10_1
	if (FAILED(device->CreateShaderResourceView/*1*/(resource, &srvDesc, &srv))){
#else
...
ubyte *RendererHLSLD3D10::mapRollingVB(const uint size){
...
void RendererHLSLD3D10::unmapRollingVB(const uint size){
	vertexBuffers[rollingVB].vertexBuffer->Unmap();

	rollingVBOffset += size;
}

uint RendererHLSLD3D10::copyToRollingVB(const void *src, const uint size){
	ASSERT(size <= ROLLING_VB_SIZE);
...
{
        dev->UpdateSubresource( vertexBuffers[id].vertexBuffer, 0, 0, m_pData, 0, 0 );
	return 0;
}
BYTE *RendererHLSLD3D10::fillIndexBufferVrtl(const IndexBufferID id, BYTE*  m_pData, int l)
{
        dev->UpdateSubresource( indexBuffers[id].indexBuffer, 0, 0, m_pData, 0, 0 );
	return 0;
}
...
float RendererHLSLD3D10::getTextWidth(const FontID font, const char *str, int length) const {
	...
	void RendererHLSLD3D10::captureScreenshot(Image3& img)
	{
		...
		if (SUCCEEDED(m_pd3dDevice10->CreateTexture2D(&desc, 0, &texture)))
		{
			if (msaaSamples > 1)
			{
				...
				desc.Usage = D3D10_USAGE_DEFAULT;
				desc.CPUAccessFlags = 0;
				if (SUCCEEDED(m_pd3dDevice10->CreateTexture2D(&desc, 0, &resolved)))
				{
					m_pd3dDevice10->ResolveSubresource(resolved, 0, m_pBackBuffer, 0, desc.Format);
					m_pd3dDevice10->CopyResource(texture, resolved);
					resolved->Release();
				}
			}
			else
			{
				m_pd3dDevice10->CopyResource(texture, m_pBackBuffer);
			}
...
void RendererHLSLD3D10::PresentVrtl(bool aB)
{
	...
	void RendererHLSLD3D10::DrawPrimitive(Primitives PrimitiveType,UINT StartVertex,UINT PrimitiveCount, const uint flags)
	{
		apply();
	...
	void RendererHLSLD3D10::DrawIndexedPrimitive(Primitives PrimitiveType,INT BaseVertexIndex,UINT MinVertexIndex,UINT NumVertices,UINT startIndex,UINT primCount, const uint flags)
	{
		apply();
...
int RendererHLSLD3D10::getSamplerUnit(const ShaderID shader, const char *samplerName) const
{
	if(!shaders[selectedShader].nSamplers) return -1;
	const SamplerD3D10 *samplers=&(shaders[selectedShader].samplers[0]);
...
TextureID RendererHLSLD3D10::addTexture(Image3* img, const bool useMips, const SamplerStateID samplerState, uint flags, float3 alpha)
{
	TextureD3D10 tex;
	memset(&tex, 0, sizeof(TextureD3D10));
	img->BindImage();
	tex.ss=samplerState;
...
//			img->convert(FORMAT_RGB10A2);
	...
	if (FAILED(hr)){
		...
		return TEXTURE_NONE;
			...
			if (flags==STAGING)
			{
			            ...
			            hr = dev->CreateTexture1D( &desc, 0, (ID3D10Texture1D **)&tex.pStaging );
			}
								hr = device->CreateTexture1D(&desc, texData, (ID3D10Texture1D **) &tex.texture);
							}
							else if (img->is2D() || img->isCube())
			...
			if (flags==STAGING)
			{
			            ...
			            desc.BindFlags = D3D10_BIND_SHADER_RESOURCE;//0;
			            desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
			            hr = dev->CreateTexture2D( &desc, 0, (ID3D10Texture2D **)&tex.pStaging );
			}
			...
			if (flags==STAGING)
			{
			            ...
			            desc.BindFlags = D3D10_BIND_SHADER_RESOURCE;//0;
			            desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
			            hr = dev->CreateTexture3D( &desc, 0, (ID3D10Texture3D **)&tex.pStaging );
			}
								hr = device->CreateTexture3D(&desc, texData, (ID3D10Texture3D **) &tex.texture);
							}
							if (FAILED(hr))
	...
	void RendererHLSLD3D10::Clear(bool color, bool depth, D3DXFROMWINEVECTOR4 colors)
	{
		if(color)
		{
			...
			;
			dev->ClearRenderTargetView( RTV, ClearColor );
		}
		if(depth) dev->ClearDepthStencilView( DSV, D3D10_CLEAR_DEPTH, 1.0, 0 );
	}
