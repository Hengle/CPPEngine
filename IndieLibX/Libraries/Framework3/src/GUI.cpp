...
"void main(VsOut In): COLOR{\n"
	...
	"}\n";
		...
		shader = IRenderer::GetRendererInstance()->addHLSLShader(plainSHD, "main", "main");
		FormatDesc format[] =
		{
...
Menu::Menu(const float ix, const float iy, Font3D *fnt, const char *text, MenuListener *listener) : title(text){
...
bool MsgBox::onKeyPress(const unsigned int key, const bool pressed){
	if (key == KEY_RETURN || key == KEY_SPACE || key == KEY_ESCAPE){
		if (pressed){
