...
ID3D10Device* RendererHLSLD3D10::device=0;
...
) : mOk(false), RendererHLSLCg(argc, argv)
{
	...
	createDeviceFlags |= D3D10_CREATE_DEVICE_DEBUG;
#endif
	hr=D3D10CreateDeviceAndSwapChain(0, D3D10_DRIVER_TYPE_HARDWARE, 0, createDeviceFlags, D3D10_SDK_VERSION, &scDesc, &m_pSwapChain, &device);
#endif
		if( FAILED( hr ) ) return;
...
/*bool createRenderTarget(LPDIRECT3DDEVICE9 device, TextureD3D10 &tex){
	if (isDepthFormat(tex.format)){
		if (tex.surfaces == NULL) tex.surfaces = new LPDIRECT3DSURFACE9;
		if (device->CreateDepthStencilSurface(tex.width, tex.height, formatsd3d10[tex.format], D3DMULTISAMPLE_NONE, 0, FALSE, tex.surfaces, NULL) != D3D_OK){
			delete tex.surfaces;
		...
		if (tex.flags & CUBEMAP){
			if (device->CreateCubeTexture(tex.width, mipMapped? 0 : 1, tex.usage, formatsd3d10[tex.format], D3DPOOL_DEFAULT, (LPDIRECT3DCUBETEXTURE9 *) &tex.texture, NULL) != D3D_OK){
			...
			if (device->CreateTexture(tex.width, tex.height, mipMapped? 0 : 1, tex.usage, formatsd3d10[tex.format], D3DPOOL_DEFAULT, (LPDIRECT3DTEXTURE9 *) &tex.texture, NULL) != D3D_OK){
				ErrorMsg("Couldn't create render target");
				return false;
			}
			if (tex.surfaces == NULL) tex.surfaces = new LPDIRECT3DSURFACE9;
...
//			img.convert(FORMAT_RGB10A2);
	...
	D3D10_SUBRESOURCE_DATA texData[1024];
...
VertexFormatID RendererHLSLD3D10::addVertexFormatVrtl(const FormatDesc *formatDesc, const uint nAttribs, const ShaderID shader){
	const DXGI_FORMAT formatsd3d10[][4] = {
	...
	for (uint i = 0; i < nAttribs; i++){
		...
		desc[i].Format = formatsd3d10[formatDesc[i].format][size - 1];
...
#else
DepthStateID RendererHLSLD3D10::addDepthState(const bool depthTest, const bool depthWrite, const int depthFunc, const bool stencilTest, const uint8 stencilReadMask, const uint8 stencilWriteMask,
...
bool fillSS(ID3D10SamplerState **dest, int &min, int &max, const SamplerStateID selectedSamplerStates[], SamplerStateID currentSamplerStates[], const SamplerStateD3D10 *samplerStates){
	min = 0;
	do {
...
void RendererHLSLD3D10::setShaderConstantRaw(const char *name, const void *data, const int size){
	...
	ConstantD3D10 *constants = &shaders[selectedShader].constants[0];
...
void RendererHLSLD3D10::changeRenderTargetsVrtl(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT, const int depthSlice, const int *slices){
	...
	if (nRenderTargets > 0){
		...
		if (rt == FB_COLOR){
			vp.Width  = STX_Service::GetWindowInstance()->GetWidth();
			vp.Height = STX_Service::GetWindowInstance()->GetHeight();
		} else {
...
void RendererHLSLD3D10::changeToMainFramebuffer(){
	...
	D3D10_VIEWPORT vp = { 0, 0, (UINT) STX_Service::GetWindowInstance()->GetWidth(), (UINT) STX_Service::GetWindowInstance()->GetHeight(), 0.0f, 1.0f };
	device->RSSetViewports(1, &vp);
...
void RendererHLSLD3D10::clearRenderTarget(const TextureID renderTarget, const float4 &color, const int slice){
	if (slice == NO_SLICE){
...
unsigned int RendererHLSLD3D10::drawArrays(const Primitives primitives, const int firstVertex, const int nVertices){
    if(d3dPrim[primitives]==D3D10_PRIMITIVE_TOPOLOGY_UNDEFINED)
        return 0;
	apply();
...
{
        device->UpdateSubresource( vertexBuffers[id].vertexBuffer, 0, 0, m_pData, 0, 0 );
	return 0;
}
BYTE *RendererHLSLD3D10::fillIndexBufferVrtl(const IndexBufferID id, BYTE*  m_pData, int l)
{
        device->UpdateSubresource( indexBuffers[id].indexBuffer, 0, 0, m_pData, 0, 0 );
	return 0;
}
...
void RendererHLSLD3D10::viewport(int x, int y, int w, int h){
        D3D10_VIEWPORT vp = { x,y, w, h, 0.0f, 1.0f };
        device->RSSetViewports(1, &vp);
  m_viewport.X=x;
  m_viewport.Y=y;
  m_viewport.Width=w;
	...
	void RendererHLSLD3D10::captureScreenshot(Image3& img)
	{
		...
		if (SUCCEEDED(device->CreateTexture2D(&desc, 0, &texture)))
		{
			if (msaaSamples > 1)
			{
				...
				{
					device->ResolveSubresource(resolved, 0, m_pBackBuffer, 0, desc.Format);
			...
			{
				device->CopyResource(texture, m_pBackBuffer);
			}
...
if(!((selectedVertexBuffers[0]>-1)&&(selectedIndexBuffer>-1)))
	...
	void RendererHLSLD3D10::Clear(bool color, bool depth, D3DXFROMWINEVECTOR4 colors)
	{
		...
		if(depth) device->ClearDepthStencilView( DSV, D3D10_CLEAR_DEPTH, 1.0, 0 );
	}
...
public:
	...
	{
		stx_TechniqueHLSLD3D10(stx_EffectHLSLD3D10* pEffect = 0) { m_pTechnique = 0; m_pInputLayout = 0; }
...
public:
	...
	void BeginPass( unsigned int iPass )
	{
		...
		if(!m_vTechniques[m_iTechnique].m_pInputLayout)
			RendererHLSLD3D10::GetDevice()->CreateInputLayout( m_vTechniques[m_iTechnique].m_vPasses[iPass].m_pDesc, elementsOf( m_vTechniques[m_iTechnique].m_vPasses[iPass].m_pDesc ), PassDesc.pIAInputSignature,
...
stx_Effect* RendererHLSLD3D10::addEffectFromFile(const std::string& aFileName, const char* aDefines)
{
    return 0;//new stx_EffectHLSLD3D10(aFileName, aDefines);
}
#endif
int RendererHLSLD3D10::getSamplerIndex(ShaderID shd, const char* aName)
{
