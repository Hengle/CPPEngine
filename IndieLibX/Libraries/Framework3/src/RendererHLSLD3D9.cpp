...
void RendererHLSLD3D9::setTexture(const char *textureName, const TextureID texture)
{
...
RendererHLSLD3D9::RendererHLSLD3D9(int awidth, int aheight,
		bool InitLogFile,
			bool fullScreen,
int argc, char *argv[]
) : mOk(false), RendererHLSLCg(awidth, aheight,InitLogFile,argc, argv),m_pD3D(0),m_pd3dDevice(0)
{
	HRESULT hr = E_FAIL;
...
//
	...
	if(FAILED(hr)) return;
	memcpy(&m_caps,&d3dcaps,sizeof(D3DCAPS9));
...
RendererHLSLD3D9::~RendererHLSLD3D9()
{
	...
	for (uint i = 0; i < shaders.getCount(); i++)
	{
		...
		for (uint j = 0; j < ((ShaderHLSLNOFX3D3D9)shaders[i]).nSamplers; j++)
		{
			(shaders[i]).samplers[j].name.clear();
		}
		for (uint j = 0; j < ((ShaderHLSLNOFX3D3D9)shaders[i]).nConstants; j++)
		{
...
void RendererHLSLD3D9::resetVrtl(const uint flags)
{
	reset0(flags);
	if (flags & RESET_TEX)
	{
...
D3DPRIMITIVETYPE GetPrimitiveType(Primitives a)
{
	D3DPRIMITIVETYPE pt=D3DPT_POINTLIST;
	switch(a)
	{
		case PRIM_TRIANGLES:
...
void RendererHLSLD3D9::DrawPrimitive(Primitives PrimitiveType,UINT StartVertex,UINT PrimitiveCount, const uint flags)
{
	if(!GetPrimitiveType(PrimitiveType)) return;
        apply();
	V(m_pd3dDevice->DrawPrimitive(GetPrimitiveType(PrimitiveType),StartVertex,PrimitiveCount));
	reset(flags);
	nDrawCalls++;
}
void RendererHLSLD3D9::DrawIndexedPrimitive(Primitives PrimitiveType,INT BaseVertexIndex,UINT MinVertexIndex,UINT NumVertices,UINT startIndex,UINT primCount, const uint flags)
{
		if(!GetPrimitiveType(PrimitiveType)) return;
	apply();
	V(m_pd3dDevice->DrawIndexedPrimitive(GetPrimitiveType(PrimitiveType),BaseVertexIndex,MinVertexIndex,NumVertices,startIndex,
	primCount));
	reset(flags);
	nDrawCalls++;
}
...
memcpy(d,lockedRect.pBits,3*Desc.Width*Desc.Height);
/*
		int l=0;
...
void RendererHLSLD3D9::changeDepthStateVrtl(const DepthStateID depthState, const uint stencilRef)
{
	if (depthState != currentDepthState)
	{
		if (depthState == DS_NONE)
		{
			...
			{
				V(dev->SetRenderState(D3DRS_ZENABLE, TRUE));
				currentDepthTestEnable = true;
			}
			if (!currentDepthWriteEnable)
			{
				V(dev->SetRenderState(D3DRS_ZWRITEENABLE, TRUE));
				currentDepthWriteEnable = true;
			}
			if (currentDepthFunc != LESS)
			{
				V(dev->SetRenderState(D3DRS_ZFUNC, currentDepthFunc = LESS));
			}
		}
		else
		{
			if ((depthStates[depthState]).depthTest)
			{
				...
				{
					V(dev->SetRenderState(D3DRS_ZWRITEENABLE, (currentDepthWriteEnable = (depthStates[depthState]).depthWrite)? TRUE : FALSE));
				}
				if ((depthStates[depthState]).depthFunc != currentDepthFunc)
				{
					V(dev->SetRenderState(D3DRS_ZFUNC, currentDepthFunc = (depthStates[depthState]).depthFunc));
				}
			}
			else
			{
	...
	{
		V(dev->SetRenderState(D3DRS_STENCILREF, stencilRef));
...
ShaderID RendererHLSLD3D9::addHLSLShaderVrtl(const char *vsText, const char *gsText, const char *fsText, const char *vsName, const char *psName, const int vsLine, const int gsLine, const int fsLine,
const char *header, const char *extra, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags)
{
	if (vsText == 0 && fsText == 0) return SHADER_NONE;
	...
	if (vsText != 0)
	{
		std::string shaderString;
			...
			shaderString.append("#define ROW_MAJOR row_major\n");
			shaderString.append("#define MVPSEMANTIC\n");
			shaderString.append("#define WSIGN +\n");

		if (extra  != 0) shaderString.append(extra);
		if (header != 0) shaderString.append(header);
		shaderString.append(vsText);
        stx_snprintf(buf,65536,"\n%s\n",shaderString.c_str());
		OutputDebugStringA(buf);
		const char *profile = D3DXGetVertexShaderProfile(dev);
		if (D3DXCompileShader(shaderString.c_str(), shaderString.length(), 0, 0, mainvs, profile, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR, &shaderBuf, &errorsBuf, &shader.vsConstants) == D3D_OK)
		{
			V(dev->CreateVertexShader((DWORD *) shaderBuf->GetBufferPointer(), &shader.vertexShader));
			#ifdef DEBUG
			LPD3DXBUFFER disasm;
			D3DXDisassembleShader((DWORD *) shaderBuf->GetBufferPointer(), FALSE, 0, &disasm);
	...
	if (fsText != 0)
	{
		...
		if (extra  != 0) shaderString.append(extra);
		if (header != 0) shaderString.append(header);
		shaderString.append(fsText);
        stx_snprintf(buf,65536,"\n%s\n",shaderString.c_str());
		OutputDebugStringA(buf);
		const char *profile = D3DXGetPixelShaderProfile(dev);
		if (D3DXCompileShader(shaderString.c_str(), shaderString.length(), 0, 0, mainps, profile, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR, &shaderBuf, &errorsBuf, &shader.psConstants) == D3D_OK)
		{
			V(dev->CreatePixelShader((DWORD *) shaderBuf->GetBufferPointer(), &shader.pixelShader));
			#ifdef DEBUG
			LPD3DXBUFFER disasm;
			D3DXDisassembleShader((DWORD *) shaderBuf->GetBufferPointer(), FALSE, 0, &disasm);
			char *str = (char *) disasm->GetBufferPointer();
...
VertexFormatID RendererHLSLD3D9::addVertexFormatVrtl(const FormatDesc *formatDesc, const uint nAttribs, const ShaderID shader)
{
	...
	D3DDECLTYPE_D3DCOLOR unsigned long
	FORMAT_UBYTE
	*/
...
;
VertexBufferID RendererHLSLD3D9::addVertexBufferVrtl(const long size, const BufferAccess bufferAccess, const void *data, VertexBufferID old)//, const IDataProcessor* p)
{
	VertexBufferD3D9 vb;
	vb.size = size;
...
IndexBufferID RendererHLSLD3D9::addIndexBufferVrtl(const uint nIndices, const uint indexSize, const BufferAccess bufferAccess, const void *data, IndexBufferID old)//, const IDataProcessor* p)
{
	IndexBufferD3D9 ib;
...
SamplerStateID RendererHLSLD3D9::addSamplerStateVrtl(const Filter filter, const AddressMode s, const AddressMode t, const AddressMode r, const float lod, const uint maxAniso, const int compareFunc, const float *border_color)
{
...
TextureID RendererHLSLD3D9::addTexture(Image3* img, const bool useMips, const SamplerStateID samplerState, uint flags, float3 alpha)
{
	TextureD3D9 tex;
	memset(&tex, 0, sizeof(TextureD3D9));
	img->BindImage();
	tex.ss=samplerState;
    ...
    unsigned char *src;
	int mipMapLevel = 0;
	while ((src = (unsigned char *)(useMips? img->getPixels(mipMapLevel) : ilGetData())) != 0)
	{
		...
		else
		{
			...
			{
				memcpy(rect.pBits, ilGetData(), size);
/*
...
TextureID RendererHLSLD3D9::addRenderTargetVrtl(const int width, const int height, const int depth, const int mipMapCount, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags)
{
...
TextureID RendererHLSLD3D9::addRenderDepthVrtl(const int width, const int height, const int arraySize, const FORMAT format, const int msaaSamples, const SamplerStateID samplerState, uint flags)
{
...
void RendererHLSLD3D9::applySamplerStates()
{
	for (uint i = 0; i < MAX_SAMPLERSTATE; i++)
	{
		...
		if (samplerState != currentSamplerStates[i])
		{
			...
			if (ss.lod != css.lod)
			{
				V(dev->SetSamplerState(i, D3DSAMP_MIPMAPLODBIAS, *(DWORD *) &ss.lod))
			}
			;
...
int RendererHLSLD3D9::getSamplerUnit(const ShaderID shader, const char *samplerName) const
{
if(shaders.size()<=shader) return -1;
	std::vector<SamplerD3D9 > samplers = ((ShaderHLSLNOFX3D3D9)shaders[shader]).samplers;
	int minSampler = 0;
	int maxSampler = ((ShaderHLSLNOFX3D3D9)shaders[shader]).nSamplers - 1;
...
void RendererHLSLD3D9::changeRenderTargetsVrtl(const TextureID *colorRTs, const uint nRenderTargets, const TextureID depthRT, const int depthSlice, const int *slices)
{
...
BYTE *RendererHLSLD3D9::fillVertexBufferVrtl(const VertexBufferID id, BYTE*  m_pData, int l)
{
	BYTE* m_pLockedData;
	vertexBuffers[id].vertexBuffer->Lock( 0, 0, ( void** )&m_pLockedData, 0 );
	memcpy(m_pLockedData,m_pData,l);
	vertexBuffers[id].vertexBuffer->Unlock();
	return 0;
}
...
void RendererHLSLD3D9::changeBlendStateVrtl(const BlendStateID blendState, const uint sampleMask)
{
...
void RendererHLSLD3D9::drawArrays(const Primitives primitives, const int firstVertex, const int nVertices){
	if(!GetPrimitiveType(primitives)) return;
	dev->DrawPrimitive(GetPrimitiveType(primitives), firstVertex, nVertices);
}

void RendererHLSLD3D9::drawElements(const Primitives primitives, const int firstIndex, const int nIndices, const int firstVertex, const int nVertices){
	if(!GetPrimitiveType(primitives)) return;
	int indexSize = indexBuffers[currentIndexBuffer].indexSize;

	dev->DrawIndexedPrimitive(GetPrimitiveType(primitives), 0, firstVertex, nVertices, firstIndex, getPrimitiveCount(primitives, nIndices));
}
#endif
