...
bool Camera2::controls(D3DXFROMWINEVECTOR3* pos,D3DXFROMWINEQUATERNION* q,IInput* Input)
{
	bool r=false;
#if 1
	if( Input->OnKeyPress(KEY_HOME ))
	{
	  reset(Input);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_G ))
	{
	  speed *= 10.0f;
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_B ))
	{
	  speed /= 10.0f;
	  r=r||true;
	}
	...
	if( Input->OnKeyPress(KEY_PAGEUP))
	{
		m_pitch -= 0.5f * speed;
		pitch(m_pitch);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_PAGEDOWN))
	{
		m_pitch += 0.5f * speed;
		pitch(m_pitch);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_LEFT))
	{
		m_yaw -= 0.5f * speed;
		yaw(m_yaw);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_RIGHT))
	{
		m_yaw += 0.5f * speed;
		yaw(m_yaw);
	  r=r||true;
	}
/*	if( Input->OnKeyPress(KEY_PRIOR))
	{
		m_roll -= 0.5f * speed;
		roll(m_roll);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_NEXT))
	{
		m_roll += 0.5f * speed;
		roll(m_roll);
	  r=r||true;
	}*/
	if( Input->OnKeyPress(KEY_W))
	{
		walk(units);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_S))
	{
		walk(-units);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_R))
	{
		fly(units);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_F))
	{
		fly(-units);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_D))
	{
		strafe(units);
	  r=r||true;
	}
	if( Input->OnKeyPress(KEY_A))
	{
		strafe(-units);
	  r=r||true;
	}
